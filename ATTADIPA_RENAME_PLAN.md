...
> **Статус исполнения, 2026-08-21.** Технический rename, документация,
> бренд-материалы, GitHub repository и GitHub Pages завершены. Проверены
> новый URL, redirect старого URL, Discussions, Issues, Actions, Pages,
> CodeQL, secrets/variables, environments и webhooks. У проекта нет внешних
> сервисов, Docker/GHCR, релизов, пакетов, пользовательского домена, соцсетей
> или отдельной CI-инфраструктуры — соответствующие пункты этого плана
> неприменимы, а не пропущены. Остаются только намеренно отложенные действия:
> регистрация домена при появлении такой потребности, профессиональный
> trademark-clearance перед коммерческим запуском и контрольный внешний аудит
> через 1–2 недели после публичного переименования.
...
- [x] `Attadipa` — имя проекта, всегда с такой капитализацией.
- [x] `Lumar` — имя маскота.
- [x] Lumar — светлячок; слово `firefly` допустимо только как обычное существительное.
- [x] `Independent by design` — архитектурное кредо, раскрывающее смысл названия.
- [x] Происхождение имени: палийское `attadīpa`; в самом бренде пишем `Attadipa` без диакритики.
- [x] Attadipa не расшифровывается как аббревиатура.
- [x] Не использовать `AttadipaOS` как альтернативное официальное имя без отдельного решения.
- [x] Правила для `attadipa`, `ATTADIPA_`, `attadipa_*`.
- [x] Русское написание в обычном тексте: **Аттадипа**, рекомендуемое произношение — **атта-ДИ-па**.
- [x] В UI/логотипе бренд лучше оставлять латиницей: **Attadipa**.
...
- [x] **Не делать filter-repo/BFG по старым коммитам.**
- [x] Не пытаться удалить `FireflyOS` из исторических commit messages.
- [x] Не менять старые merge commits и PR history.
...
- [x] Создать ветку `rename/firefly-to-attadipa`.
- [x] На время rename не принимать крупные архитектурные изменения.
- [x] Не смешивать ребрендинг с рефакторингом логики.
- [x] Сначала добиться зелёного host build + tests + simulator под новым именем.
- [x] Затем слить rename.
- [x] Сразу после merge переименовать сам GitHub repository.
- [x] После rename репозитория провести второй проход проверки ссылок и внешних интеграций.
