...
- **The token that cannot be met is a documented price, not a silent one --
  ...
  `ui/lvgl/provision_face.cpp:172` — "  // Six keys, not fourteen, so a key can be a full touch target on both".
...
- **LVGL's three label traps, all three already paid for.**
  ...
  it: `ui/lvgl/mesh_face.cpp:349` — "    // `LV_LABEL_LONG_DOT` puts the dots in only where the height is fixed;".
  **A label created bare is that same trap with nothing to read.** There is no
  ...
  `ui/lvgl/mesh_face.cpp:283` — "    // content height and `LV_LABEL_LONG_WRAP`, so a name carrying a line break".
