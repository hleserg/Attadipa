...
void connect_and_handshake(MeshCoreCompanion& client)
{
    ...
    device[0] = 13;
    device[1] = 13;
    CHECK(client.receive(device, sizeof(device), at(3)));

    std::uint8_t self[62]{};
    self[0] = 5;
    std::memcpy(&self[58], "Node", 4);
    CHECK(client.receive(self, sizeof(self), at(4)));
...
void test_send_and_receive()
{
    ...
    MeshCoreFrame frame{};
    CHECK(client.next_tx(frame));
    CHECK(frame.size == 44 && frame.bytes[0] == 2);
    CHECK(std::memcmp(&frame.bytes[7], peer.id.public_key.data(), 32) == 0);
    CHECK(std::memcmp(&frame.bytes[39], "Hello", 5) == 0);
