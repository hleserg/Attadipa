...
правдоподобно. Если часы не могут сказать — они так и говорят, и говорят,
*чего именно* они не знают. Каждая из этих фраз написана на обоих языках, на
которых говорит проект, — но на панели пока отрисованы только английские.

**Гибкость к железу — конструктивно.** Две платы часов, у которых почти нет
ничего общего, работают на одном и том же коде приложений, тех же сервисах и
том же интерфейсе. Под какую плату собран образ прошивки — один выбор во время
сборки в `firmware/main/` (`choice ATTADIPA_BOARD`), а настольный симулятор
держит обе геометрии в одном бинарнике. Приложения спрашивают, что
...
| **Снимать и водить живой UI из другого процесса** — тапы, свайпы, кнопки, сценарные проходы | ✅ `MEASURED` на физических часах для захвата, удалённого тапа и путей физического тача/BOOT/PWR ([стендовая запись](docs/hardware/WATCH_CONTROL_2026-08-25.md)); свайпы и сценарные проходы пока только в симуляторе |
...
| **Компас и курс** — API есть, экран поворачивает стрелку, пока только по фикстурам симулятора. Магнитометра нет ни на одной плате, два модуля лежат на стенде и ждут врезки, и курс пока не выставляет ни одна прошивка | 🧪 [`MAGNETOMETER_RETROFIT`](docs/research/MAGNETOMETER_RETROFIT.md) |
...
покрывает один бинарник симулятора. Начните с `--no-bring-up`: это самый быстрый
