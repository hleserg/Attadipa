...
// rebuilds, which is how "day and night both checked" becomes something a
// person can do in four seconds rather than a line in a review checklist.
...
PixelCost pixel_cost() {
  ...
  case platform::PanelTechnology::Amoled:
    return PixelCost::PerPixel;
...
std::int32_t px(Space s) { return metrics().px(ui::dp_of(s)); }

// A role, resolved for the current theme, in the form LVGL wants.
//
// The substitution is the interesting part. `color()` returns nothing for a
// role the theme genuinely does not define — `color.danger` in both themes, any
// background the night table omits — and this screen must not paint a guess and
// must not paint nothing either. So it substitutes the one colour that is
// always defined, and says out loud that it did. A diagnostic that quietly
// invents a colour is a diagnostic that lies about the palette it is
// diagnosing.
lv_color_t paint(ColorRole role) {
  ...
  std::fprintf(
      stderr,
      "palette: %s is UNKNOWN in the %s theme (%s), drawing %s instead\n",
      ui::name_of(role), ui::name_of(g_theme), ui::name_of(cost),
      ui::name_of(substitute));
...
lv_obj_t *make_row(lv_obj_t *parent, const char *left, const char *right,
                   ColorRole role, const lv_font_t *font) {
...
// in tokens.h: `TypeRole` deliberately carries no sizes, because final §51
// wants licence, Cyrillic coverage, legibility at real pixel size and generated
// flash size checked before a face is adopted, and none of the four has been.
//
...
//
// The fonts are ours rather than LVGL's built-ins, and that is not a
// preference. `lv_font_montserrat_14` is generated from `-r
// 0x20-0x7F,0xB0,0x2022` — Latin only — so `×` draws as a box and every
// Cyrillic letter draws as a box. See assets/fonts/README.md.
const lv_font_t *pick_font(TypeRole role, std::uint16_t width_px) {
  const bool large = width_px >= 400;
  switch (role) {
  case TypeRole::Display:
  case TypeRole::Title:
    return large ? &attadipa_nunito_sans_28 : &attadipa_nunito_sans_16;
  default:
    return large ? &attadipa_nunito_sans_20 : &attadipa_nunito_sans_14;
  }
}
...
void build_boot_screen(const platform::HardwareInventory &inventory,
  ...
  lv_label_set_text_fmt(subtitle, tr(StringId::DiagnosticGeometry),
                        inventory.board_name(), inventory.display().width_px,
                        inventory.display().height_px,
  ...
  char counted[64];
  l10n::format_plural(counted, sizeof counted,
                      PluralId::DiagnosticCapabilityCount,
  ...
  if (const platform::RadioInfo *radio = inventory.radio()) {
    ...
    make_row(screen, tr(StringId::DiagnosticRadioChip),
             ...
             chip != nullptr ? ColorRole::TextPrimary : ColorRole::Warning,
             font);
    make_row(screen, tr(StringId::DiagnosticRadioLora),
             tr(radio->can_do_lora() ? StringId::Yes : StringId::No),
             radio->can_do_lora() ? ColorRole::Success : ColorRole::TextMuted,
             font);
