 ...
 * SHA-256 2ece178f34500cbb3755980c1520daee4d15fd2f0893fc68672958708c9c6291.
 * On an LVGL bump, diff this file against the new template rather than
 * regenerating it — the edits below are decisions, not transformations.
 *
 * This configures the *simulator*. The two device builds will get their own
 * files, because LVGL's configuration is per-build and the boards disagree
 * about the things it controls. What does not need a second file is geometry:
 * lv_sdl_window_create() takes the resolution at runtime and
 * lv_display_set_dpi() takes the density, so 240x240 and 410x502 are one
 * binary. Task T-008 requires exactly that.
 *
 * Changes from the stock template:
 *   - enabled the file (the template ships behind #if 0)
 *   - LV_USE_SDL 0 -> 1: the SDL window, mouse and keyboard drivers are the simulator
 *   - LV_SDL_ACCELERATED 1 -> 0: the software renderer is what SDL_VIDEODRIVER=dummy can do, and CI runs headless
 *   - LV_USE_LOG 0 -> 1: on the desktop a log costs nothing and an LVGL assertion without one says almost nothing
 *   - LV_MEM_SIZE 64 KiB -> 1 MiB: a placeholder, and a deliberate one — see the note below
 *   - LV_USE_ASSERT_OBJ 0 -> 1: slow, and the simulator is the one place that does not matter
 *   - LV_USE_ASSERT_STYLE 0 -> 1: same reason
 *   - LV_FONT_MONTSERRAT_16 0 -> 1: scaffolding only — Montserrat has no Cyrillic and is not the product font
 *   - LV_FONT_MONTSERRAT_20 0 -> 1: scaffolding only — Montserrat has no Cyrillic and is not the product font
 *   - LV_FONT_MONTSERRAT_28 0 -> 1: scaffolding only — Montserrat has no Cyrillic and is not the product font
 ...
 * been established. A bounded heap is kept rather than switching to the C
 * library allocator, so that the simulator can still run out of LVGL memory
 * the way a watch would; the bound is simply not yet the right number.
 */
