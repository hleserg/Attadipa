...
* **Alpha-only icons, one authored colour background.** Icons remain
  `LV_COLOR_FORMAT_A8` masks recoloured through `ColorRole`. The Clock's
  410x502 background is RGB565 source art made for that exact screen.
...
BACKGROUND_SOURCE = (ROOT / "ui" / "assets" / "source" / "backgrounds" /
...
BACKGROUND_SYMBOL = "attadipa_background_clock_meadow_night_410x502"
BACKGROUND_WIDTH = 410
BACKGROUND_HEIGHT = 502
...
def inputs() -> list:
    ...
    files.append(BACKGROUND_SOURCE)
    return files
...
def outputs() -> list:
    """Every file this pipeline is responsible for: masks, background, header.
    ...
    a correct one.
    """
    return ([OUT_DIR / f"{manifest.symbol(n, s)}.c"
             for n, s in manifest.assets()] +
            [OUT_DIR / f"{BACKGROUND_SYMBOL}.c", HEADER])
...
def convert_background() -> Path:
    check_source(BACKGROUND_SOURCE)
    from PIL import Image
    with Image.open(BACKGROUND_SOURCE) as im:
        if im.size != (BACKGROUND_WIDTH, BACKGROUND_HEIGHT):
            raise SystemExit(
                f"generate_images: {BACKGROUND_SOURCE.name} is "
                f"{im.size[0]}x{im.size[1]}; expected "
                f"{BACKGROUND_WIDTH}x{BACKGROUND_HEIGHT}.")
    subprocess.run(
        [sys.executable, str(VENDOR), "--ofmt", "C", "--cf", "RGB565",
         "--compress", "NONE", "--name", BACKGROUND_SYMBOL,
         "-o", str(OUT_DIR), str(BACKGROUND_SOURCE)],
        check=True, stdout=subprocess.DEVNULL)
    return OUT_DIR / f"{BACKGROUND_SYMBOL}.c"
...
def write_header(sizes: dict) -> None:
    ...
    out.write("//\n")
    out.write("// Icons are alpha-only masks recoloured through ColorRole. The clock\n")
    out.write("// background is an authored RGB565 raster for the 410x502 face.\n")
    out.write("//\n")
    out.write("// The number in each name is a **pixel** size, not a token and not a board.\n")
    out.write("// 33 px is icon.size.lg on the T-Watch's measured 220 dpi panel; 39 and 47\n")
    out.write("// are icon.size.md and icon.size.lg on the Waveshare's 315 dpi one.\n")
    ...
    for name in sorted(manifest.ICONS):
        ...
        out.write("\n")
    out.write("// Original Attadipa night-meadow clock background.\n")
    out.write(f"LV_IMAGE_DECLARE({BACKGROUND_SYMBOL});   // "
              f"{sizes[BACKGROUND_SYMBOL]} B of .rodata\n\n")
...
def main() -> int:
    ...
    sizes[BACKGROUND_SYMBOL] = BACKGROUND_WIDTH * BACKGROUND_HEIGHT * 2
    total += sizes[BACKGROUND_SYMBOL]
    print(f"  {background.name:38s} {BACKGROUND_WIDTH:3d}x{BACKGROUND_HEIGHT:<3d} "
          f"{sizes[BACKGROUND_SYMBOL]:6d} B")
    write_header(sizes)
    ...
    print(f"images: {len(sizes)} asset(s), {total} B of .rodata "
          f"({total / 1024.0:.1f} kB), uncompressed")
    return 0
