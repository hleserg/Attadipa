...
**Attadipa has code.** As of 2026-08-25 the repository builds seven libraries and
**thirty-four tests** — **forty-one** with the desktop simulator, which is off
...
a device will serve when T-114 adds its endpoint to the firmware.
...
- **Half of that was overstated, and it was corrected 2026-08-23.** The record
  ...
  stands (`COLMOD 3Ah = 0x55`). The transfer byte order was registered as D21
  and T-166 has now resolved the operational setting by measurement: with the
  board profile's RGB565 byte swap enabled, the physical panel showed the
  intended asymmetric red, green and blue swatches. Nothing shipped before it
  was wrong — every generated asset is an `A8` mask with no byte order — and what
  ...
  and still `UNKNOWN` in cost. Necessity is measured; performance is not.
...
device endpoint over the firmware and physical display path.

## Build and test state

| Target | State |
|---|---|
| Host / native | builds; **thirty-four tests** pass locally and in CI — and **forty-one** with `-DATTADIPA_BUILD_SIMULATOR=ON`, which is the *Simulator build and headless run* job and not this row: `CMakeLists.txt` defaults the option off and the host job configures without it — smoke, capability registry, both halves of the layer-boundary check, localization, four flash-tool checks, and the six suites this milestone added: trust, transport, power, position, diagnostics, and the replay rig with its sixteen traces — fifteen replayed and one deliberately broken, for the rig's own test — plus the
...
- **`ota_1` can never boot.** It sits at exactly `0x1000000`, and the ROM and
  ...
  **And the upper half is not "for data" either, which took a second look to
  ...
  tree. T-165 added the first one, and CI reports three partitions in one table,
  all below `0x1000000`; the check is no longer vacuous. Both boards originally
  booted a vendor table this repository did not own —
...
- **A `PURE_RAM_APP` runs fine — if the serial port is never closed.** Four
  ...
  *last* close of a `ttyACM`; those are CDC control bits on this native-USB
  board, and the USB-Serial/JTAG peripheral resets the digital core. `esptool`
  exiting was therefore itself the reset. `rst:0x15 (USB_UART_CHIP_RESET)` says
  *the host did it* and should have been read that way the first time. Driving `load-ram`
...
- **The board ran our firmware through T-165 acceptance, then returned to the
  ...
  claimed as completed. The same unit now runs the T-166 candidate at 5%
  brightness: the owner observed the intended RGB swatches and readable 28 px
  text, while six visible-target taps reached LVGL and the serial log. OD-19
  permits reversible flashing; the exact sequence is in the evidence file.

## Recently completed

- **The seven-PR queue incident is drained and its admission/claim root causes
  are closed.** #241 and #245 merged, #251 was independently current and merged,
  #217/#218 were classified parked while only #246 remains ordinary active;
  #248 is the explicit recovery PR. Normal WIP is 2, `full` starts at 2 and
  `incident` at 3. The shipping writer checks admission twice: before dispatch
  and again after acquiring the repository-wide writer lease. Unknown API/CLI
  or JSON state refuses. `agent:working` is now a visible mirror of an atomic
  GitHub ref carrying the run token, not an advisory claim; CI repair, retries,
  watchdog dispatch and local agents use the same primitive. The 27-case
  production-path suite runs stub `gh`, the workflow step, parallel claims,
  stale recovery and three mutations. No hardware was used or claimed.
- **T-168 is deployed.** The two review-failure steps now invalidate the old
  verdict before fallible notification, the parked caller patch is removed,
  and the same executable test reads the shipping workflow.
...
- **The eight agent-config files at the root were not copies of this
  ...
  `.mcp.json`, `.gemini/` and `opencode.json` were generated by `rtk` and
  `graft` and described those two tools only — so Codex, Cline, Windsurf, Kilo
  Code, Antigravity and opencode landed on a tool manual and read nothing about
  the boards, the specification or the rule about `PASS`. They are pointers to
  `CLAUDE.md` now rather than symlinks to it, because both tools rewrite their
  own marker-delimited blocks in place and a symlink would send the next
  `rtk init` into the project's rules. `CLAUDE.md` gained **The tooling around
  the work**: where `ponytail` applies and where it must not (`docs/research/`,
  where the explanation *is* the product), that `graft` indexes code and cannot
  index prose — its parsers refuse `.md`, so `STATUS.md`, `TASKS.md` and the
  research notes stay `rg`'s job — and that `rtk`'s exclusions for `esptool`,
  `idf.py`, `tools/flash/` and `tools/watch_control.py` live in
  `~/.config/rtk/config.toml`, the only file it reads. Codex also gained `rtk`,
  through `tools/agents/rtk-codex-hook.cjs`, which translates Claude Code's
  one-string payload into the argv Codex sends; its own shapes are covered by
  `--self-test` and **whether Codex fires the hook at all is NOT VERIFIED**,
  because that account is rate-limited until 2026-09-01. No hardware was
  touched by any of this and no hardware claim is made by it.
...
- **The same sweep was reading the first page of a pull request and deciding on
  it as though it were the whole one.** Issue #170, filed by the producing
  agent and confirmed against `main`. Every fact the merge gate refuses on
  arrives in a GraphQL connection, and a connection is a *page*: the query
  asked for `labels(first:50)`, `reviewThreads(first:100)`, `files(first:100)`
  and `contexts(first:100)`, never asked whether another page existed, and
  computed on what came back. A pull request with 101 review threads whose
  hundred-and-first was unresolved answered
  `[ .nodes[] | select(.isResolved | not) ] | length` with **zero**, and zero is
  the value that merges. Past the fiftieth label the same shape hid
  `ai-review:blocking`; past the hundredth context, a failing check. `first: N`
  had become a silent *permitting* condition — the more there was to read, the
  less the gate could refuse on — and `merge-candidate.sh` could not have caught
  it from its own arguments, because every one of them is a summary carrying no
  trace of how much was read before summarising. The fail-open was at the
  boundary, not in the decision.

  Completeness is now proved from `pageInfo`, and it is a fail-closed **input**
  to the rule rather than an assumption behind it: `merge-facts.graphql` asks
  every connection whether it has a next page, `merge-facts.jq` refuses on any
  answer it cannot read — missing, null, not an object, no `hasNextPage`, a
  `hasNextPage` that is a string — and `merge-candidate.sh` takes the verdict as
  a tenth argument. Never `nodes | length`, which cannot tell a full page from a
  truncated one; and never `totalCount`, which **does not count the filtered
  set** — `timelineItems(itemTypes:[LABELED_EVENT])` answered `totalCount: 15`
  beside a single node on #173, so a count-based rule would have held every pull
  request in the repository forever. Observed against the live API, not assumed.
  85 assertions became 135, including the finding reproduced end to end: the old
  extraction quoted verbatim answering `0` over the fixture, and the shipped rule
  refusing the same document.

  **Half of it is parked, and the merge sweep merges nothing until that changes.**
  Agents run as `claude[bot]`, whose installation token holds no `workflows`
  permission, so the four edits pointing the sweep at the rule are in
  `docs/automation/pending/170-merge-sweep-completeness.patch` — T-144, which
  also carries #154's identical blocker. `merge-candidate.sh` refuses the
  pre-#170 nine-argument caller **by arity**, so the live sweep now holds every
  pull request and logs the file to apply, once each per run. That is the
  fail-closed state and not the finished one: a gate that cannot prove it read
  the whole pull request must not merge it, and orchestrator sessions are
  unaffected.
...
- **T-102 — documentation consistency in CI, and the defect its own pull request
  ...
  *Check 5*, one of them wrong and neither checkable; both are now the tuple's
  ...
  because fixing the instance is not fixing the mechanism. Check 8's **four**
  ...
  register IDs would redden CI on a correct document; it reads **undecorated
  IDs only**, so a bolded or backticked row leaves the register without a word;
  and it does **not** check that a `D<NN>` cited elsewhere resolves to a live
  row — the half that cost the nineteen hand-edits, exactly as invisible now as
  the collision was. This sentence said *three* until the fifth review round, on
  the commit that made it four, forty lines above the sentence announcing the
  fourth. And the
