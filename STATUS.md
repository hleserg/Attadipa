...
- **A stranger's "ok" was an authorisation.**
  [#130](https://github.com/hleserg/Attadipa/issues/130). `pr-merge-sweep.yml`
  has to decide whether anybody dealt with a finding from the other reviewer,
  because Codex is configured on ChatGPT's side, sets no label, and reaches this
  repository only as a comment. The test it used was
  `select(.bot | not) | select(.at > $last)` — *the finding is answered if any
  later comment came from something that is not a bot.* That is a
  **classification** doing the work of an **authorisation**, on a public
  repository where anybody with an account can comment, and the "answer" needed
  no relation to the finding beyond a later timestamp. Reproduced on `7558728`
  and on current `main`, through the workflow's own `jq`: a Codex finding, one
  comment reading `ok` from an account with no permissions at all, and
  `merge-candidate.sh` prints `MERGE`. Two earlier rounds had narrowed this
  condition — first for bot replies, then for review bodies as well as issue
  comments — and neither reached the bottom of it, which is the argument for
  moving the rule out of the YAML: `.github/scripts/codex-answered.sh` is a file
  a test can execute, and `.github/tests/codex-answered-test.sh` runs 59
  assertions over it, the first of which is the bypass. An answer now needs all
  three of a `write|maintain|admin` actor established through the collaborators
  API — the same call the intake gate makes, and **not** `author_association`,
  which hands `COLLABORATOR` to read-only invitees; a **binding to the finding**,
  either a reply carrying GitHub's own `in_reply_to_id` for the finding's thread
  or the token `attadipa: codex-reviewed` outside code, stripped by the intake
  gate's own markdown stripper so that quoting the token is not typing it; and a
  date at or after the head commit, so an answer given to commit A does not
  carry to commit B. Anything it cannot establish prints `unknown`, which holds
  under a line of its own — the previous failure path was `|| CODEX=1`, so a
  read that never happened reported itself as *"1 unanswered comment(s) from the
  other reviewer"*, and an empty string became a zero and **merged**.

  **Half of it is parked, and the half that is parked is the caller.** Agents
  here run as `claude[bot]` through the Claude GitHub App —
  `ATTADIPA_AGENT_TOKEN` is unset by design — and that installation token holds
  no `workflows` permission, so GitHub refuses a push touching
  `.github/workflows/**` before any review sees it. The rule, its test and the
  documents are on the branch; `pr-merge-sweep.yml` and `ci.yml` are in
  `docs/automation/pending/130-merge-sweep-caller.patch` with the one command
  that applies them. Until they land the rule is present, tested and **not
  called**, so `merge-candidate-test.sh` — the suite CI already runs — fails two
  assertions saying exactly that, and CI is red on purpose rather than green
  over a fix that is not wired up. Applying the patch takes both suites to 86/86
  and 61/61, which was run rather than predicted. Filed as **T-127**: it is not
  this task's problem, it is every automation task's.
