...
- **A blocked run came back out of the pipeline as a success, and a repaired
  ...
  *"Done"* over the agent's `BLOCKED:` comment and added `agent:review`. The
  ...
  re-introductions of the defect. **T-169 records one thing found and not
  ...
  (Filed as T-127 and renumbered on merge — `main` had taken that number for the
  anchor check while this branch was open.)
