...
- **And it was taking the head commit's word for when the head commit
  arrived.** Issue #199, filed by the producing agent and confirmed against
  `861f5fa`. The sweep derived two separate protections — the six-hour settling
  window and *"was `ai-review:pass` reached on this commit"* — from one
  expression, `(.pushedDate // .committedDate)`. `pushedDate` is deprecated, and
  a live read against the head of #193 on 2026-08-24 answered `null` beside a
  live `committedDate`: **the fallback was not the narrow case, it was the only
  branch ever taken**, so both protections rested on the git committer clock,
  which is an input. `GIT_COMMITTER_DATE=2020-01-01 git commit` produced a head
  that cleared six hours the instant it existed and that every earlier
  `ai-review:pass` labelling post-dated, so the previous head's verdict read as
  a verdict about it. The workflow's own comment named the attack and fell back
  anyway.

  Both now come from `workflowRun.createdAt` on the head commit's own check
  suites, restricted to `pull_request` events — GitHub's stamp against that
  object id, written when it starts work, and unreachable from a commit. The
  maximum is taken, not the minimum, so a commit that existed elsewhere before
  is timed by its arrival here rather than by its earlier life. Where GitHub has
  stamped nothing, `merge-head-trust.sh` prints `HOLD`; there is no fallback, and
  `merge-facts.graphql` no longer asks for `committedDate` or `pushedDate` at
  all, so re-deriving one takes an edit to two files and turns the suite red.
  `merge-candidate.sh` also now takes the head's object id as an eleventh
  argument and refuses a caller that cannot name the commit it is deciding
  about. 135 assertions became 182, including the finding reproduced end to
  end — the old extraction quoted verbatim reading the backdated date, calling
  the stale pass current and clearing six hours, beside the shipped rules
  refusing the same document. Verified read-only against the live API: the query
  runs, and #176's real head shows a `pull_request` run created at 15:50:19Z
  with its `ai-review:pass` labelling at 16:04:43Z, which is the shape a genuine
  review has.

  **Both halves are parked in one patch, and the merge sweep merges nothing
  until it lands.** Agents run as `claude[bot]`, whose installation token holds
  no `workflows` permission, so the eight edits pointing the sweep at the two
  rules are in `docs/automation/pending/170-merge-sweep-completeness.patch` —
  T-144, which also carries #154's identical blocker. One patch and not two on
  purpose: a second patch against the same workflow is two apply orders and an
  arity that moves twice. `merge-candidate.sh` refuses the pre-#170
  nine-argument caller **by arity**, so the live sweep holds every pull request
  and logs the file to apply, once each per run — which is also why #199 is a
  finding about code and a parked patch rather than a live incident. That is the
  fail-closed state and not the finished one: a gate that cannot prove it read
  the whole pull request, or say when its head arrived, must not merge it, and
  orchestrator sessions are unaffected. **NOT DEPLOYED** until the patch is
  applied; T-126 stays open, because no run of this workflow has ever completed
  a merge and nothing local can prove one would.
