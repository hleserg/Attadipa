...
- **A pull request with a merge conflict gets no CI at all, and nothing said
  ...
  **Half of it is on `main` and half is not, and that half is blocked on a
  ...
  written down, with the three commands to land such a patch as step 8 of
  ...
  **This particular one has now been landed** — a local orchestrator session
  applied it on 2026-08-24 and the patch file is gone rather than left in
  `pending/`. Two things had gone wrong with it while it sat there: its `ci.yml`
  hunk cited an anchor at `:301` that had moved to `:351`, and it had been
  committed with **CRLF** line endings where every other patch in that directory
  is LF, so `git apply` refused it on an LF tree regardless of content. Both were
  found by running the recipe rather than reading it.
