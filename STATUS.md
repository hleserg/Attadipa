...
- **Recognising an error and disclosing the text around it were one act.**
  [#106](https://github.com/hleserg/Attadipa/issues/106).
  `failure-reason.sh` exists so a failed agent run names itself on the issue
  instead of pointing at a log `show_full_output: false` has emptied on purpose.
  Its whitelist did the recognising with patterns like
  `API Error: [0-9]{3}[^"]{0,240}` and then **printed the whole match**, so up to
  240 characters of whatever followed a status code went into a public comment:
  a token in the result string, a nested `error.message`, or — because the greps
  ran over the raw file rather than over the record that holds the verdict — an
  error prefix quoted in an unrelated **tool result**, which also became the
  run's stated cause. Eleven of the thirteen patterns had that suffix; the two
  that did not (`JavaScript heap out of memory` and the output-token maximum)
  were already safe, which is what the shape of the defect predicts. Reproduced
  on `f2b6853`, ten commits after the finding was made against `5fd2738`, so the
  staleness check said implement rather than close. The fix inverts what the log
  is for: it decides **which sentence of the script** to print, and every line is
  assembled from literal text plus captures drawn from a bounded alphabet — a
  three-digit HTTP status, a group of at most twelve digits, or a name from a
  closed list of error types. There is no path by which a byte of message text
  reaches stdout, so **widening what is recognised no longer widens what is
  disclosed** — which is the whole cost of T-108, and it is now paid. Recognition
  is **ordered, not scoped**, and the first version of this fix had that wrong —
  the independent review blocked on it and was right. The detectors read
  `.result` and `.error` of the last `result` record first, because that record
  is the verdict and a tool result is not; if nothing there matches, the whole
  file is read after all. Scoping to the record alone would have gone silent on
  run `32589375744` — the very run the script was written for, whose result
  record has neither field — and `show_full_output: false` publishes that record
  and withholds everything else, so a reader told nothing about the rest has been
  sent to a log already emptied of the answer. Each line therefore says what it
  stood on: `(found outside the result record)`, or `(no status code or error
  type beside it)` when only the words matched — `.result` is model output, and
  an agent that dies while writing about this file leaves `Credit balance is too
  low` in its final message, which the retry decision would read as a spend
  failure. The status and the type come from one bounded window, so two unrelated
  errors are never spliced into a third. `subtype` and `num_turns`, the two
  fields printed without being recognised first, are held to `[a-z_]` and `[0-9]`
  **inside the jq** as well as at the print site — the head line is `|`-joined,
  and a `subtype` of `x|false|1|no` used to shift every field right and report a
  failed run as a clean one. 96 assertions, up from 27; 25 of the 27 pass
  **unchanged**, which matters because those are the leak cases and weakening
  them to fit a fix would have been the easy failure here. The two that changed
  are the malformed-input pair, which asserted a bare `unclassified` and now
  assert which kind of nothing it was — `agent-say.sh` answers `unclassified`
  with *"widening that whitelist is the fix"*, which is the wrong instruction for
  a log nothing could parse. The 69 new ones cover each of the fourteen
  recognised prefixes with a secret behind it, and are red against the pre-fix
