...
- **Recognising an error and disclosing the text around it were one act.**
  ...
  is scoped as well as rendered: the detectors read `.result` and `.error` of the
  last `result` record and nothing else, so a tool result cannot be the verdict;
  the whole-file scan survives only where no record could be read at all — a
  truncated write — and a line found that way says where it came from, because it
  may belong to an earlier record. `subtype` and `num_turns`, the two fields
  printed without being recognised first, are held to `[a-z_]` and `[0-9]`
  instead of trusted. 80 assertions, up from 27; every one of the 27 still passes
  **unchanged**, which matters because the old ones are the leak cases and
  weakening them to fit a fix would have been the easy failure here. The 53 new
  ones cover each of the fourteen recognised prefixes with a secret behind it,
  and are red against the pre-fix copy in a way that can be *run* rather than
  claimed: `ATTADIPA_REASON_SH=/tmp/old.sh bash
  .github/tests/failure-reason-test.sh` reports 22 failures on `5fd2738`'s
  script and 0 on this one.
