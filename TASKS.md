...
A1's schematic-revision
  ...
  in ([#83](https://github.com/hleserg/Attadipa/issues/83)). **A6 is no longer
  on it either** — answered 2026-08-22 on
  [#56](https://github.com/hleserg/Attadipa/issues/56) and recorded as
  ...
  the node gets an accelerometer and probably a gyroscope for GNSS
  optimisation, and never a magnetometer. That is a different answer from A5's,
  not the same one twice — a node's magnetometer and a wrist's magnetometer
  answer different questions, and only the wrist's is coming. **A4 (the regulatory region) is no longer on this list** — closed
  ...
  **A5 and A6 are also no longer on this list** — answered 2026-08-22 on
  ...
  [OD-17](docs/research/OWNER_DECISIONS.md#od-17--a5-and-a6-an-external-magnetometer-is-coming-for-the-watch-the-node-will-never-carry-one).
- **Impact:** what is left on this list gates less than it used to. A5 and A6
  are answered and no longer gate anything: an external magnetometer is coming
  for the watch and the node will never carry one
  ([OD-17](docs/research/OWNER_DECISIONS.md#od-17--a5-and-a6-an-external-magnetometer-is-coming-for-the-watch-the-node-will-never-carry-one)),
  ...
  ([ADR-0009](docs/adr/0009-heading.md) §3). A2's answer, now given, decided the
  radio half: of the five candidate radios two cannot do LoRa at all and only
  one is supported by the pinned MeshCore
  ...
  off the physical part when it arrives. **A11** gates nothing either way; it
  changes what the fleet table may claim.
