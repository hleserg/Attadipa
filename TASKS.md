...
Recommended next action:
                ...
                unit ([OD-17](docs/research/OWNER_DECISIONS.md))
...
### T-170 · The node IMU needs a capability model of its own — **DONE** 2026-08-22
- **Filed as T-111 and renumbered on merge.** `main` took its own T-111 —
  *a third capability source, for hardware that is neither the board's nor the
  node's* — while this branch was open. `main`'s keeps the number because it
  landed first. T-170 was verified free across every remote branch, not just
  against `main`.
