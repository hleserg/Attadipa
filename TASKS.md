...
### T-106 · Four measurements and eight registers, before any cell is ordered
- **Priority:** P1 — it gates the battery decision, and every part of *the
  gate* is cheap: M1, M2, M3 and the eight registers need a caliper, a scale,
...
- **Acceptance — and it does not include the magnetometer.** M1, M2, M3, M4's
  ...
  the cell-safety registers and not part of the original five — and the sizing table in
...
- **Dependencies:** none to start — but **the first caller is already written**,
  ...
  *"nothing under `tools/` opens a serial port"* describes `main` at the moment
  ...
  `open()` — **which is `LIKELY` to help and is not established.** `cdc_acm`
  raises both lines in the kernel when the tty is activated, before any
  userspace code runs, so that sequence may only lower them again after the
  assertion. It is still the right conversion — a constructor that opens the
  port cannot even try — but do not close goal 3 on having written it.
...
- **Goal:** one shared resolver, and a check that keeps it the only route.
  ...
  3. Handle the two reset mechanisms while there: DTR and RTS low **before**
     ...
     ([WAVESHARE_RUNNING_OUR_CODE](docs/research/WAVESHARE_RUNNING_OUR_CODE.md)
     §2.2, §2.3). **Only the second of those two has a bench result.** The
     hold-open rule rests on §2.3, a recorded experiment; the pre-open set does
     not, and §2.2 now says it is `LIKELY` to be defeated by `cdc_acm` raising
     the lines in the kernel first. **Observing which it is, on the unit, is
     part of this goal and not a follow-up** — see Acceptance.
- **Acceptance:** the resolver exists and has host tests covering no-match,
  ...
  **And goal 3 is not met by host tests.** Whether the board resets when a port
  is opened with DTR and RTS pre-set is a hardware observation: this task closes
  with it recorded as `MEASURED` on the unit, or as
  `NOT EXECUTED — HARDWARE REQUIRED` with goal 3 explicitly left open. Without
  that clause every criterion here is satisfiable with no board in the room —
  resolver, three tests, green lint, task `DONE` — after which `main` records
  the rail as built, `BENCH_HANDLING` stops saying no tool implements it, and
  every caller still resets the board on open. The `Hardware required` line
  below mentions this and is not a gate; this is.
...
- **What got through:**
  ...
  3. **`OD-nn` is a sequence with no allocator and no check**, and it has
     already failed in three different ways at once. The sweep is of every
     remote branch, not just the open pull requests, and it has now been run
     twice — the second time after the first collision actually landed:
