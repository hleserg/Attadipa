...
### T-169 · The two-attempt repair bound appears to reset itself
- **This task was filed as T-127 and renumbered on merge**, the same way T-149
  was and for the same reason. `main` took its own T-127 — *a link's `#anchor`
  is captured and then never checked*, now in DONE — while this branch was open.
  `main`'s keeps the number because it landed first; `check_docs.py`'s duplicate
  task-ID check caught the collision at merge, which is what it is for. T-169
  was verified free across every remote branch rather than against `main` alone,
  because T-158 to T-168 are allocated on branches that have not landed.
