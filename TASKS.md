...
- **Tests:** `actionlint` over seven workflows with shellcheck integration —
  ...
  A third watchdog scan is written, tested and **deployed on this branch**:
  ...
  **It waited on a permission rather than on the work,** and a local session has
  the permission. GitHub refuses to let a GitHub App create or update anything
  under `.github/workflows/` without the `workflows` scope, which `claude[bot]`
  does not hold, so the job was parked as a patch. On 2026-08-24 a local
  orchestrator session applied it — step 8 of
  ...
  — and the patch file is deleted rather than left behind.

  **Two things were wrong with the parked copy, and both are worth recording.**
  Its `ci.yml` hunk no longer applied: the anchor it cited at `:301` had moved to
  `:351` as that file grew, so the hunk was re-derived against the current file
  rather than forced. And the patch had been **committed with CRLF line
  endings** while every other patch in that directory is LF — so `git apply`
  refused it on an LF tree whatever its content said. A patch parked for a day is
  a patch that may no longer apply, and only running it says which.

  Whether to grant a GitHub App the `workflows` scope permanently is still an
  owner decision with a real argument on each side, and it is still open. What
  has changed is that it no longer blocks this particular guard.
- **Tests:** `actionlint` over seven workflows with shellcheck integration —
