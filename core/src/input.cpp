...
void InputQueue::clear()
{
    ...
    // `pushed == popped + flushed + size()` is only true if a deliberate
    // discard is as visible as a pop. (`dropped` is not a term in it -- a
    // refused push never entered the ring. See the note in `input.h`.)
