...
// `firmware/main/meshcore_ble.cpp:1430` — "provider.begin(now());"
// followed immediately by `firmware/main/meshcore_ble.cpp:1431` — "if (configured.load()) start_scan();",
// and that scan is neither passive nor bounded —
// `firmware/main/meshcore_ble.cpp:703` — "params.passive = 0;" and `:707`
...
// session (`firmware/main/meshcore_ble.cpp:1921` — "(void)attadipa::firmware::quiesce_gap(gap);"),
// and the lifecycle's fault step is reached only on paths where no scan is
// running (`firmware/main/meshcore_ble.cpp:1433` — "case SessionStep::Fault:").
...
// (`firmware/main/physical_input.cpp:519` — "sleep_requested_ = true;").
