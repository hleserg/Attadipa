...
// `firmware/main/meshcore_ble.cpp:1405` — "provider.begin(now());"
// followed immediately by `firmware/main/meshcore_ble.cpp:1406` — "if (configured.load()) start_scan();",
// and that scan is neither passive nor bounded —
// `firmware/main/meshcore_ble.cpp:678` — "params.passive = 0;" and `:682`
...
// session (`firmware/main/meshcore_ble.cpp:1872` — "(void)attadipa::firmware::quiesce_gap(gap);"),
// and the lifecycle's fault step is reached only on paths where no scan is
// running (`firmware/main/meshcore_ble.cpp:1408` — "case SessionStep::Fault:").
