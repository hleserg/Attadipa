...
// `firmware/main/meshcore_ble.cpp:1306` — "provider.begin(now());"
// followed immediately by `firmware/main/meshcore_ble.cpp:1307` — "if (configured.load()) start_scan();",
// and that scan is neither passive nor bounded —
// `firmware/main/meshcore_ble.cpp:587` — "params.passive = 0;" and `:591`
...
// `firmware/main/meshcore_ble.cpp:1719` — "                    provider.fault(now());"
// — nor does the lifecycle's fault step,
// `firmware/main/meshcore_ble.cpp:1309` — "        case SessionStep::Fault:".
...
// started at `firmware/main/meshcore_ble.cpp:1307` — "if (configured.load())
