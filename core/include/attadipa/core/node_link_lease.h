...
// `firmware/main/meshcore_ble.cpp:1420` — "provider.begin(now());"
// followed immediately by `firmware/main/meshcore_ble.cpp:1421` — "if (configured.load()) start_scan();",
// and that scan is neither passive nor bounded —
// `firmware/main/meshcore_ble.cpp:693` — "params.passive = 0;" and `:697`
...
// session (`firmware/main/meshcore_ble.cpp:1903` — "(void)attadipa::firmware::quiesce_gap(gap);"),
// and the lifecycle's fault step is reached only on paths where no scan is
// running (`firmware/main/meshcore_ble.cpp:1423` — "case SessionStep::Fault:").
