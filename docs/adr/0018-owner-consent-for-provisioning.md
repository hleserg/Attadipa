...
2. **The passkey was RAM-only when this was decided, and the storage it
   ...
   the passkey:
   `firmware/main/meshcore_ble.cpp:2215` — "bool configure_meshcore_ble(std::uint32_t passkey)"
   reaches `firmware/main/meshcore_ble.cpp:1718` — "secure_pairing.store(event.passkey != 0);"
   and nothing else, and the two flags a scan waits on are plain atomics:
   `firmware/main/meshcore_ble.cpp:195` — "std::atomic_bool configured{false};"
   and `firmware/main/meshcore_ble.cpp:197` — "std::atomic_bool reconnect_allowed{false};".
   But the seam that would hold it is already in this translation unit, put
   there by #304: `firmware/main/meshcore_ble.cpp:259` — "constexpr const char* kMeshNvsNamespace = ",
   read at `firmware/main/meshcore_ble.cpp:349` — "const esp_err_t err = nvs_get_blob(handle, kNodeKeyNvsKey,"
   and written at `firmware/main/meshcore_ble.cpp:464` — "esp_err_t err = nvs_set_blob(handle, kNodeKeyNvsKey, id.public_key.data(),",
   behind an `nvs_flash_init()` at `firmware/main/meshcore_ble.cpp:2162` —
   ...
   (`firmware/main/meshcore_ble.cpp:1739` — "const bool stored = !event.persist_passkey ||"),
   ...
   (`firmware/main/meshcore_ble.cpp:1788` — "if (!erase_passkey()) {").
   ...
   **Correction, 2026-09-02 ([#409](https://github.com/hleserg/Attadipa/issues/409)):
   ...
   writer of `firmware/main/meshcore_ble.cpp:260` — "constexpr const char* kNodeKeyNvsKey"
...
`docs/testing/WATCH_CONTROL.md:128` — "so on a device a held power key may be a shutdown rather than an event".
...
`firmware/main/meshcore_ble.cpp:2042` — "ble_hs_cfg.sm_io_cap = BLE_HS_IO_KEYBOARD_ONLY;".
...
six digits, not a key: `firmware/main/meshcore_ble.cpp:2215` —
...
- There is one bond slot, and it is spoken for.
  ...
  set at `firmware/main/meshcore_ble.cpp:1718` — "secure_pairing.store(event.passkey != 0);"
  and read at `firmware/main/meshcore_ble.cpp:938` — "if (secure_pairing.load()) {". An image nobody has provisioned
