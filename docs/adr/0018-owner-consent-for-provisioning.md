...
1. **The RTC write path exists and is compiled out, not missing.** When this
   ...
   next to the HIL-only one that `firmware/main/waveshare_board.cpp:481` — "#if CONFIG_ATTADIPA_WATCH_CONTROL"
   still gates,
   `firmware/main/waveshare_board.cpp:482` — "class BoardTimeSink final : public attadipa::debug::TimeSink {".
...
- **Both interfaces exist and are the right shape, and a product image
  ...
  by `firmware/main/waveshare_board.cpp:482` — "class BoardTimeSink final : public attadipa::debug::TimeSink {"
...
- Puts the face in `ui/lvgl/`, which is what subjects it to the theme-token
  ...
  `firmware/main/waveshare_board.cpp:642` — "void build_mesh_screen() {" — is
