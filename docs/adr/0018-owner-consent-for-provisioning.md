...
1. **The RTC write path exists and is compiled out, not missing.** When this
   ...
   `firmware/main/waveshare_board.cpp:295` — "esp_err_t restore_time_metadata() {". Every option therefore cost *re-gating
...
chip: `firmware/main/waveshare_board.cpp:295` — "esp_err_t restore_time_metadata() {".
