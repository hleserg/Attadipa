...
3. **Touch already works, as an LVGL pointer device.**
   `firmware/main/physical_input.cpp:90` — "lv_indev_set_type(indev, LV_INDEV_TYPE_POINTER);".
...
with nothing gating it: `firmware/main/physical_input.cpp:56` —
"    buttons.pin_bit_mask = 1ULL << GPIO_NUM_0;",
`firmware/main/physical_input.cpp:509` —
"  PhysicalButton physical_buttons_[1] = {{GPIO_NUM_0, false, 1}};", and
`firmware/main/physical_input.cpp:552` —
