...
2. **The passkey was RAM-only when this was decided, and the storage it
   ...
   the passkey:
   `firmware/main/meshcore_ble.cpp:2483` — "bool configure_meshcore_ble(std::uint32_t passkey)"
   reaches `firmware/main/meshcore_ble.cpp:1884` — "secure_pairing.store(event.passkey != 0);"
   and nothing else, and the two flags a scan waits on are plain atomics:
   `firmware/main/meshcore_ble.cpp:221` — "std::atomic_bool configured{false};"
   and `firmware/main/meshcore_ble.cpp:223` — "std::atomic_bool reconnect_allowed{false};".
   But the seam that would hold it is already in this translation unit, put
   there by #304: `firmware/main/meshcore_ble.cpp:285` — "constexpr const char* kMeshNvsNamespace = ",
   read at `firmware/main/meshcore_ble.cpp:374` — "const esp_err_t err = nvs_get_blob(handle, kNodeKeyNvsKey,"
   and written at `firmware/main/meshcore_ble.cpp:551` — "esp_err_t err = nvs_set_blob(handle, kNodeKeyNvsKey, id.public_key.data(),",
   behind an `nvs_flash_init()` at `firmware/main/meshcore_ble.cpp:2400` —
   ...
   (`firmware/main/meshcore_ble.cpp:1915` — "const bool stored = !event.persist_passkey ||"),
   ...
   (`firmware/main/meshcore_ble.cpp:1973` — "if (!erase_passkey()) {").
   ...
   until the owner enters the digits again.
   ...
   **Correction, 2026-09-02 ([#409](https://github.com/hleserg/Attadipa/issues/409)):
   ...
   writer of `firmware/main/meshcore_ble.cpp:286` — "constexpr const char* kNodeKeyNvsKey"
...
`docs/research/VERIFIED_FACTS.md:1359` — "button presses arrive as PMU interrupts"
...
`docs/testing/WATCH_CONTROL.md:150` — "so on a device a held power key may be a shutdown rather than an event".
...
`firmware/main/meshcore_ble.cpp:2243` — "ble_hs_cfg.sm_io_cap = BLE_HS_IO_KEYBOARD_ONLY;".
...
six digits, not a key: `firmware/main/meshcore_ble.cpp:2483` —
...
- There is one bond slot, and it is spoken for.
  ...
  `docs/research/VERIFIED_FACTS.md:500` — "### A wrong MeshCore node's bond evicts the pinned node's",
  ...
  set at `firmware/main/meshcore_ble.cpp:1884` — "secure_pairing.store(event.passkey != 0);"
  and read at `firmware/main/meshcore_ble.cpp:1046` — "if (secure_pairing.load()) {". An image nobody has provisioned
...
chip: `firmware/main/waveshare_board.cpp:324` — "esp_err_t restore_time_metadata() {".
...
- **Both interfaces exist and are the right shape, and a product image
  ...
  `core::` is not one yet: `core/include/attadipa/core/mesh_service.h:224` —
