...
2. **The passkey was RAM-only when this was decided, and the storage it
   ...
   the passkey:
   `firmware/main/meshcore_ble.cpp:2535` — "bool configure_meshcore_ble(std::uint32_t passkey)"
   reaches `firmware/main/meshcore_ble.cpp:1895` — "secure_pairing.store(event.passkey != 0);"
   ...
   and written at `firmware/main/meshcore_ble.cpp:554` — "esp_err_t err = nvs_set_blob(handle, kNodeKeyNvsKey, id.public_key.data(),",
   behind an `nvs_flash_init()` at `firmware/main/meshcore_ble.cpp:2452` —
   ...
   (`firmware/main/meshcore_ble.cpp:1926` — "const bool stored = !event.persist_passkey ||"),
   ...
   (`firmware/main/meshcore_ble.cpp:1985` — "if (!erase_passkey()) {").
   ...
   **Correction, 2026-09-02 ([#409](https://github.com/hleserg/Attadipa/issues/409)):
   ...
   writer of `firmware/main/meshcore_ble.cpp:289` — "constexpr const char* kNodeKeyNvsKey"
...
4. **Both sink interfaces live in the same header as the forbidden symbol.**
   ...
   "class MeshSink {", while `tools/flash/firmware_elf_check.py:63` — "# Bridge::handle is the single function every privileged opcode is dispatched"
...
`firmware/main/meshcore_ble.cpp:2295` — "ble_hs_cfg.sm_io_cap = BLE_HS_IO_KEYBOARD_ONLY;".
...
six digits, not a key: `firmware/main/meshcore_ble.cpp:2535` —
...
- There is one bond slot, and it is spoken for.
  ...
  set at `firmware/main/meshcore_ble.cpp:1895` — "secure_pairing.store(event.passkey != 0);"
  and read at `firmware/main/meshcore_ble.cpp:1049` — "if (secure_pairing.load()) {". An image nobody has provisioned
