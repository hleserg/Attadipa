...
2. **The passkey was RAM-only when this was decided, and the storage it
   ...
   the passkey:
   `firmware/main/meshcore_ble.cpp:2543` — "bool configure_meshcore_ble(std::uint32_t passkey)"
   reaches `firmware/main/meshcore_ble.cpp:1902` — "secure_pairing.store(event.passkey != 0);"
   ...
   behind an `nvs_flash_init()` at `firmware/main/meshcore_ble.cpp:2460` —
   ...
   (`firmware/main/meshcore_ble.cpp:1933` — "const bool stored = !event.persist_passkey ||"),
   ...
   (`firmware/main/meshcore_ble.cpp:1993` —
...
`firmware/main/meshcore_ble.cpp:2303` — "ble_hs_cfg.sm_io_cap = BLE_HS_IO_KEYBOARD_ONLY;".
...
six digits, not a key: `firmware/main/meshcore_ble.cpp:2543` —
...
- There is one bond slot, and it is spoken for.
  ...
  set at `firmware/main/meshcore_ble.cpp:1902` — "secure_pairing.store(event.passkey != 0);"
  and read at `firmware/main/meshcore_ble.cpp:1056` — "if (secure_pairing.load()) {". An image nobody has provisioned
