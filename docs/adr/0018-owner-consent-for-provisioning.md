...
1. **The RTC write path exists and is compiled out, not missing.** When this
   ...
   next to the HIL-only one that `firmware/main/waveshare_board.cpp:607` — "#if CONFIG_ATTADIPA_WATCH_CONTROL"
   still gates,
   `firmware/main/waveshare_board.cpp:608` — "class BoardTimeSink final : public attadipa::debug::TimeSink {".
   The restore side was always unconditional:
   `firmware/main/waveshare_board.cpp:277` — "esp_err_t restore_time_metadata() {". Every option therefore cost *re-gating
...
2. **The passkey was RAM-only when this was decided, and the storage it
   ...
   the passkey:
   `firmware/main/meshcore_ble.cpp:2174` — "bool configure_meshcore_ble(std::uint32_t passkey)"
   reaches `firmware/main/meshcore_ble.cpp:1677` — "secure_pairing.store(event.passkey != 0);"
   and nothing else, and the two flags a scan waits on are plain atomics:
   `firmware/main/meshcore_ble.cpp:188` — "std::atomic_bool configured{false};"
   and `firmware/main/meshcore_ble.cpp:190` — "std::atomic_bool reconnect_allowed{false};".
   But the seam that would hold it is already in this translation unit, put
   there by #304: `firmware/main/meshcore_ble.cpp:252` — "constexpr const char* kMeshNvsNamespace = ",
   read at `firmware/main/meshcore_ble.cpp:342` — "const esp_err_t err = nvs_get_blob(handle, kNodeKeyNvsKey,"
   and written at `firmware/main/meshcore_ble.cpp:457` — "esp_err_t err = nvs_set_blob(handle, kNodeKeyNvsKey, id.public_key.data(),",
   behind an `nvs_flash_init()` at `firmware/main/meshcore_ble.cpp:2121` —
   ...
   (`firmware/main/meshcore_ble.cpp:1698` — "const bool stored = !event.persist_passkey ||"),
   ...
   (`firmware/main/meshcore_ble.cpp:1747` — "if (!erase_passkey()) {").
   ...
   **Correction, 2026-09-02 ([#409](https://github.com/hleserg/Attadipa/issues/409)):
   ...
   writer of `firmware/main/meshcore_ble.cpp:253` — "constexpr const char* kNodeKeyNvsKey"
   ...
   **Note, 2026-09-02 ([#411](https://github.com/hleserg/Attadipa/issues/411)):
   the pin has an eraser now, and a product image can forget its node on its
   ...
   *current* passkey is the next thing typed:
   ...
   provisioning. The correction above stands as the record of what declining
   cost until then.

3. **Touch already works, as an LVGL pointer device.**
   `firmware/main/physical_input.cpp:97` — "lv_indev_set_type(indev, LV_INDEV_TYPE_POINTER);".
...
`firmware/main/meshcore_ble.cpp:2001` — "ble_hs_cfg.sm_io_cap = BLE_HS_IO_KEYBOARD_ONLY;".
...
six digits, not a key: `firmware/main/meshcore_ble.cpp:2174` —
...
- There is one bond slot, and it is spoken for.
  ...
  set at `firmware/main/meshcore_ble.cpp:1677` — "secure_pairing.store(event.passkey != 0);"
  and read at `firmware/main/meshcore_ble.cpp:931` — "if (secure_pairing.load()) {". An image nobody has provisioned
...
chip: `firmware/main/waveshare_board.cpp:277` — "esp_err_t restore_time_metadata() {".
...
- **Commits to a seam that reports what the hardware did, and the passkey's half
  ...
  `core/include/attadipa/core/provisioning.h:28` — "Pending,   // Taken by hardware that answers on another task, and not" —
  and the screen stays on the field until
  `core/include/attadipa/core/provisioning.h:83` — "virtual ProvisionOutcome mesh_passkey_outcome() = 0;"
...
- **Both interfaces exist and are the right shape, and a product image
  ...
  by `firmware/main/waveshare_board.cpp:608` — "class BoardTimeSink final : public attadipa::debug::TimeSink {"
  ...
  `firmware/main/meshcore_ble.h:20` — "bool configure_meshcore_ble(std::uint32_t passkey);"
...
- Puts the face in `ui/lvgl/`, which is what subjects it to the theme-token
  ...
  `firmware/main/waveshare_board.cpp:776` — "void build_mesh_screen() {" — is
