...
2. **The passkey was RAM-only when this was decided, and the storage it
   ...
   the passkey:
   `firmware/main/meshcore_ble.cpp:2205` — "bool configure_meshcore_ble(std::uint32_t passkey)"
   reaches `firmware/main/meshcore_ble.cpp:1708` — "secure_pairing.store(event.passkey != 0);"
   and nothing else, and the two flags a scan waits on are plain atomics:
   `firmware/main/meshcore_ble.cpp:193` — "std::atomic_bool configured{false};"
   and `firmware/main/meshcore_ble.cpp:195` — "std::atomic_bool reconnect_allowed{false};".
   But the seam that would hold it is already in this translation unit, put
   there by #304: `firmware/main/meshcore_ble.cpp:257` — "constexpr const char* kMeshNvsNamespace = ",
   read at `firmware/main/meshcore_ble.cpp:347` — "const esp_err_t err = nvs_get_blob(handle, kNodeKeyNvsKey,"
   and written at `firmware/main/meshcore_ble.cpp:462` — "esp_err_t err = nvs_set_blob(handle, kNodeKeyNvsKey, id.public_key.data(),",
   behind an `nvs_flash_init()` at `firmware/main/meshcore_ble.cpp:2152` —
   ...
   (`firmware/main/meshcore_ble.cpp:1729` — "const bool stored = !event.persist_passkey ||"),
   ...
   (`firmware/main/meshcore_ble.cpp:1778` — "if (!erase_passkey()) {").
   ...
   **Correction, 2026-09-02 ([#409](https://github.com/hleserg/Attadipa/issues/409)):
   ...
   writer of `firmware/main/meshcore_ble.cpp:258` — "constexpr const char* kNodeKeyNvsKey"
...
`firmware/main/meshcore_ble.cpp:2032` — "ble_hs_cfg.sm_io_cap = BLE_HS_IO_KEYBOARD_ONLY;".
...
six digits, not a key: `firmware/main/meshcore_ble.cpp:2205` —
...
- There is one bond slot, and it is spoken for.
  ...
  set at `firmware/main/meshcore_ble.cpp:1708` — "secure_pairing.store(event.passkey != 0);"
  and read at `firmware/main/meshcore_ble.cpp:936` — "if (secure_pairing.load()) {". An image nobody has provisioned
