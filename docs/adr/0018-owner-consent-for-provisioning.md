...
2. **The passkey was RAM-only when this was decided, and the storage it
   ...
   the passkey:
   `firmware/main/meshcore_ble.cpp:2211` — "bool configure_meshcore_ble(std::uint32_t passkey)"
   reaches `firmware/main/meshcore_ble.cpp:1714` — "secure_pairing.store(event.passkey != 0);"
   ...
   behind an `nvs_flash_init()` at `firmware/main/meshcore_ble.cpp:2158` —
   ...
   (`firmware/main/meshcore_ble.cpp:1735` — "const bool stored = !event.persist_passkey ||"),
   ...
   (`firmware/main/meshcore_ble.cpp:1784` — "if (!erase_passkey()) {").
...
4. **Both sink interfaces live in the same header as the forbidden symbol.**
   ...
   "class MeshSink {", while `tools/flash/firmware_elf_check.py:55` — "# Bridge::handle is the single function every privileged opcode is dispatched"
...
`firmware/main/meshcore_ble.cpp:2038` — "ble_hs_cfg.sm_io_cap = BLE_HS_IO_KEYBOARD_ONLY;".
...
six digits, not a key: `firmware/main/meshcore_ble.cpp:2211` —
...
- There is one bond slot, and it is spoken for.
  ...
  set at `firmware/main/meshcore_ble.cpp:1714` — "secure_pairing.store(event.passkey != 0);"
...
- **Both interfaces exist and are the right shape, and a product image
  ...
  `core::` is not one yet: `core/include/attadipa/core/mesh_service.h:219` —
