...
2. **The passkey was RAM-only when this was decided, and the storage it
   ...
   the passkey:
   `firmware/main/meshcore_ble.cpp:2486` — "bool configure_meshcore_ble(std::uint32_t passkey)"
   reaches `firmware/main/meshcore_ble.cpp:1887` — "secure_pairing.store(event.passkey != 0);"
   ...
   there by #304: `firmware/main/meshcore_ble.cpp:286` — "constexpr const char* kMeshNvsNamespace = ",
   read at `firmware/main/meshcore_ble.cpp:375` — "const esp_err_t err = nvs_get_blob(handle, kNodeKeyNvsKey,"
   and written at `firmware/main/meshcore_ble.cpp:552` — "esp_err_t err = nvs_set_blob(handle, kNodeKeyNvsKey, id.public_key.data(),",
   behind an `nvs_flash_init()` at `firmware/main/meshcore_ble.cpp:2403` —
   ...
   (`firmware/main/meshcore_ble.cpp:1918` — "const bool stored = !event.persist_passkey ||"),
   ...
   (`firmware/main/meshcore_ble.cpp:1976` — "if (!erase_passkey()) {").
   ...
   **Correction, 2026-09-02 ([#409](https://github.com/hleserg/Attadipa/issues/409)):
   ...
   writer of `firmware/main/meshcore_ble.cpp:287` — "constexpr const char* kNodeKeyNvsKey"
...
`firmware/main/meshcore_ble.cpp:2246` — "ble_hs_cfg.sm_io_cap = BLE_HS_IO_KEYBOARD_ONLY;".
...
six digits, not a key: `firmware/main/meshcore_ble.cpp:2486` —
...
- There is one bond slot, and it is spoken for.
  ...
  set at `firmware/main/meshcore_ble.cpp:1887` — "secure_pairing.store(event.passkey != 0);"
  and read at `firmware/main/meshcore_ble.cpp:1047` — "if (secure_pairing.load()) {". An image nobody has provisioned
