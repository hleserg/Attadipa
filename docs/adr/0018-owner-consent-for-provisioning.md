...
1. **The RTC write path exists and is compiled out, not missing.** When this
   ...
   `firmware/main/waveshare_board.cpp:442` — "class BoardProvisioner final : public attadipa::core::Provisioner {",
   next to the HIL-only one that `firmware/main/waveshare_board.cpp:502` — "#if CONFIG_ATTADIPA_WATCH_CONTROL"
   still gates,
   `firmware/main/waveshare_board.cpp:503` — "class BoardTimeSink final : public attadipa::debug::TimeSink {".
   The restore side was always unconditional:
   `firmware/main/waveshare_board.cpp:268` — "esp_err_t restore_time_metadata() {". Every option therefore cost *re-gating
...
chip: `firmware/main/waveshare_board.cpp:268` — "esp_err_t restore_time_metadata() {".
...
- **Both interfaces exist and are the right shape, and a product image
  ...
  by `firmware/main/waveshare_board.cpp:503` — "class BoardTimeSink final : public attadipa::debug::TimeSink {"
...
- Puts the face in `ui/lvgl/`, which is what subjects it to the theme-token
  ...
  `firmware/main/waveshare_board.cpp:663` — "void build_mesh_screen() {" — is
