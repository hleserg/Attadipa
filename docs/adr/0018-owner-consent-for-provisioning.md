...
2. **The passkey was RAM-only when this was decided, and the storage it
   ...
   and written at `firmware/main/meshcore_ble.cpp:475` — "esp_err_t err = nvs_set_blob(handle, kNodeKeyNvsKey, id.public_key.data(),",
   behind an `nvs_flash_init()` at `firmware/main/meshcore_ble.cpp:2236` —
   ...
   (`firmware/main/meshcore_ble.cpp:1767` — "const bool stored = !event.persist_passkey ||"),
   ...
   (`firmware/main/meshcore_ble.cpp:1816` — "if (!erase_passkey()) {").
...
`firmware/main/meshcore_ble.cpp:2080` — "ble_hs_cfg.sm_io_cap = BLE_HS_IO_KEYBOARD_ONLY;".
...
- There is one bond slot, and it is spoken for.
  ...
  set at `firmware/main/meshcore_ble.cpp:1746` — "secure_pairing.store(event.passkey != 0);"
  and read at `firmware/main/meshcore_ble.cpp:949` — "if (secure_pairing.load()) {". An image nobody has provisioned
