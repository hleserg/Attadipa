...
2. **The passkey was RAM-only when this was decided, and the storage it
   ...
   the passkey:
   `firmware/main/meshcore_ble.cpp:2424` — "bool configure_meshcore_ble(std::uint32_t passkey)"
   reaches `firmware/main/meshcore_ble.cpp:1840` — "secure_pairing.store(event.passkey != 0);"
   ...
   behind an `nvs_flash_init()` at `firmware/main/meshcore_ble.cpp:2341` —
   ...
   (`firmware/main/meshcore_ble.cpp:1871` — "const bool stored = !event.persist_passkey ||"),
   ...
   (`firmware/main/meshcore_ble.cpp:1920` — "if (!erase_passkey()) {").
...
`firmware/main/meshcore_ble.cpp:2184` — "ble_hs_cfg.sm_io_cap = BLE_HS_IO_KEYBOARD_ONLY;".
...
six digits, not a key: `firmware/main/meshcore_ble.cpp:2424` —
...
- There is one bond slot, and it is spoken for.
  ...
  set at `firmware/main/meshcore_ble.cpp:1840` — "secure_pairing.store(event.passkey != 0);"
  and read at `firmware/main/meshcore_ble.cpp:1024` — "if (secure_pairing.load()) {". An image nobody has provisioned
