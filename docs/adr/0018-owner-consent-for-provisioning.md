...
1. **The RTC write path exists and is compiled out, not missing.**
   `firmware/main/waveshare_board.cpp:405` — "#if CONFIG_ATTADIPA_WATCH_CONTROL"
   gates `firmware/main/waveshare_board.cpp:406` — "class BoardTimeSink final : public attadipa::debug::TimeSink {",
   and a second one — `firmware/main/waveshare_board.cpp:228` — "#if CONFIG_ATTADIPA_WATCH_CONTROL"
   — gates `firmware/main/waveshare_board.cpp:282` — "esp_err_t save_time_metadata(std::int16_t offset, std::int64_t last_sync_utc) {".
   ...
   `firmware/main/waveshare_board.cpp:202` — "esp_err_t restore_time_metadata() {". Every option therefore costs *re-gating
...
`RTC`, and it is built: `firmware/main/waveshare_board.cpp:436` —
...
chip: `firmware/main/waveshare_board.cpp:202` — "esp_err_t restore_time_metadata() {".
...
- **Both interfaces exist and are the right shape, and a product image
  ...
  by `firmware/main/waveshare_board.cpp:406` — "class BoardTimeSink final : public attadipa::debug::TimeSink {"
  — which validates the request, tags it `firmware/main/waveshare_board.cpp:436`
...
- Puts the face in `ui/lvgl/`, which is what subjects it to the theme-token
  ...
  `firmware/main/waveshare_board.cpp:636` — "void build_mesh_screen() {" — is
