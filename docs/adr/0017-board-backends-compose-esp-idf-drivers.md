...
1. **The shipping tree already has the seam.** `waveshare_board.cpp:106-107`
   holds an `esp_lcd_panel_handle_t` and an `esp_lcd_touch_handle_t` and hands
   them on; `physical_input.cpp:483-486` takes exactly those two types. Nothing
   above that line knows which board it is. A second backend either reuses that
