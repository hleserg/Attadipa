...
**1c. A boundary frame no walk owns ends nothing.** An `END` after the walk
that sent it is already over — swept, ended once, or re-read — is dropped, not
...
- **On the one node this project has measured, the upper bound of that cost is
  ...
  measured bench drops — `firmware/main/meshcore_ble.cpp:1188` — "            // sessions out of three -- it is the last frame of the burst, so it"
