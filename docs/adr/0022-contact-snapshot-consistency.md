...
`link/src/meshcore_companion.cpp:1502` — "status_.peers_complete = true;" is set
...
frame it loses — `link/src/meshcore_companion.cpp:524` — "            status_.peers_complete = true;".
...
both completion flags — `link/src/meshcore_companion.cpp:1421` — "        peer_count_ = 0;".
...
- **On the one node this project has measured, the upper bound of that cost is
  ...
  measured bench drops — `firmware/main/meshcore_ble.cpp:1028` — "            // sessions out of three -- it is the last frame of the burst, so it"
