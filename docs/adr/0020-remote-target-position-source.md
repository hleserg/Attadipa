...
148 — `link/src/meshcore_companion.cpp:875` — "        if (size < 148) { ++malformed_frames_; return false; }" —
...
dispatcher owns no shared one — `link/src/meshcore_companion.cpp:763` — "    if (data == nullptr || size == 0 || size > kMeshCoreFrameBytes ||" — rejects
...
`link/src/meshcore_companion.cpp:892` — "        accept_custom_vars(&data[1], size - 1);" — and an arm
...
`link/src/meshcore_companion.cpp:500` — "    if (peer_count_ < peers_.size()) {". The primary read is
