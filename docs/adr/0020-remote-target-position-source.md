...
148 — `link/src/meshcore_companion.cpp:856` — "        if (size < 148) { ++malformed_frames_; return false; }" —
...
dispatcher owns no shared one — `link/src/meshcore_companion.cpp:744` — "    if (data == nullptr || size == 0 || size > kMeshCoreFrameBytes ||" — rejects
...
`link/src/meshcore_companion.cpp:878` — "        accept_custom_vars(&data[1], size - 1);" — and an arm
...
`link/src/meshcore_companion.cpp:481` — "    if (peer_count_ < peers_.size()) {". The primary read is
