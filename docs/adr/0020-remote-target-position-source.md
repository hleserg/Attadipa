...
148 — `link/src/meshcore_companion.cpp:1381` — "        if (size < 148) { ++malformed_frames_; return false; }" —
...
dispatcher owns no shared one — `link/src/meshcore_companion.cpp:1249` — "    if (data == nullptr || size == 0 || size > kMeshCoreFrameBytes ||" — rejects
...
`link/src/meshcore_companion.cpp:1439` — "        accept_custom_vars(&data[1], size - 1);" — and an arm
...
`link/src/meshcore_companion.cpp:656` — "    if (count < table.size()) {". The primary read is
