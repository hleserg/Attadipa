...
148 — `link/src/meshcore_companion.cpp:644` — "        if (size < 148) { ++malformed_frames_; return false; }" —
...
dispatcher owns no shared one — `link/src/meshcore_companion.cpp:539` — "    if (data == nullptr || size == 0 || size > kMeshCoreFrameBytes ||" — rejects
...
`link/src/meshcore_companion.cpp:682` — "        accept_custom_vars(&data[1], size - 1);" — and an arm
...
sixteen peers —
`link/include/attadipa/link/meshcore_companion.h:168` — "    static constexpr std::size_t kRetainedPeers = 16;" —
...
`link/src/meshcore_companion.cpp:328` — "    if (peer_count_ < peers_.size()) {". The primary read is
