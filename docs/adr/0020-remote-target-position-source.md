...
148 — `link/src/meshcore_companion.cpp:672` — "        if (size < 148) { ++malformed_frames_; return false; }" —
...
dispatcher owns no shared one — `link/src/meshcore_companion.cpp:567` — "    if (data == nullptr || size == 0 || size > kMeshCoreFrameBytes ||" — rejects
...
`link/src/meshcore_companion.cpp:710` — "        accept_custom_vars(&data[1], size - 1);" — and an arm
...
sixteen peers —
`link/include/attadipa/link/meshcore_companion.h:170` — "    static constexpr std::size_t kRetainedPeers = 16;" —
...
`link/src/meshcore_companion.cpp:340` — "    if (peer_count_ < peers_.size()) {". The primary read is
