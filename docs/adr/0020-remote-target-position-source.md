...
148 — `link/src/meshcore_companion.cpp:1346` — "        if (size < 148) { ++malformed_frames_; return false; }" —
...
dispatcher owns no shared one — `link/src/meshcore_companion.cpp:1214` — "    if (data == nullptr || size == 0 || size > kMeshCoreFrameBytes ||" — rejects
...
`link/src/meshcore_companion.cpp:1395` — "        accept_custom_vars(&data[1], size - 1);" — and an arm
...
`link/src/meshcore_companion.cpp:621` — "    if (count < table.size()) {". The primary read is
