...
- The first real input is the existing physical USB debug connection:
  ...
  **That is the bench path, and has been only that since #346.**
  ...
  (`apps/include/attadipa/apps/provisioning.h:152` — "class ProvisioningEntry {"); a Leave key
  goes out at any point, and nothing reaches the board before Next on the
  review step that spells the whole instant out
  (`apps/include/attadipa/apps/provisioning.h:52` —
  ...
  `firmware/main/waveshare_board.cpp:466` — "set_wall_clock(const attadipa::core::WallClockEntry &entry) override {"
...
- Default NVS is initialised once per boot and its verdict kept
  (`firmware/main/waveshare_board.cpp:290` — "state.metadata_storage = nvs_flash_init();").
