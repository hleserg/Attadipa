...
`core/include/attadipa/core/power_owner.h:379` — "// does not yet have and which nothing in the current firmware needs, because"
...
enum exists for is that `Unchanged` is a *claim*: a step may only say it when
the state it touched was genuinely put back, which means a device whose
postcondition can be read or a setting the host itself holds. A write over a bus
to a panel that has no getter cannot say it, because an error there does not
separate a command that never left the host from one whose acknowledgement did
not come back.
...
could read is still open — and **H1 is `PARTIAL`**, one idle state now measured
on each board, at the USB input upstream of the PMU: the Waveshare 2026-09-05
(S16) and the T-Watch 2026-09-08 (S17,
[VERIFIED_FACTS](../research/VERIFIED_FACTS.md)). An input figure separates no
rail, so neither says what any rail costs. Usable wake sources and
