...
parses today — `link/src/meshcore_companion.cpp:1047` — "bool MeshCoreCompanion::accept_message(const std::uint8_t* data," —
...
`link/src/meshcore_companion.cpp:724` — "        if (std::memcmp(peers_[i].id.public_key.data(), prefix, 6) == 0) {" —
...
unnamed one, because `link/src/meshcore_companion.cpp:1074` — "    const core::MeshPeer* sender = find_peer_prefix(&data[prefix]);" —
