...
parses today — `link/src/meshcore_companion.cpp:995` — "bool MeshCoreCompanion::accept_message(const std::uint8_t* data," —
...
`link/src/meshcore_companion.cpp:672` — "        if (std::memcmp(peers_[i].id.public_key.data(), prefix, 6) == 0) {" —
...
unnamed one, because `link/src/meshcore_companion.cpp:1017` — "    const core::MeshPeer* sender = find_peer_prefix(&data[prefix]);" —
...
`link/src/meshcore_companion.cpp:662` — "    // A seventeenth distinct contact is dropped and nothing is flagged for it." —
