...
parses today — `link/src/meshcore_companion.cpp:1051` — "bool MeshCoreCompanion::accept_message(const std::uint8_t* data," —
...
unnamed one, because `link/src/meshcore_companion.cpp:1078` — "    const core::MeshPeer* sender = find_peer_prefix(&data[prefix]);" —
