...
parses today — `link/src/meshcore_companion.cpp:1043` — "bool MeshCoreCompanion::accept_message(const std::uint8_t* data," —
...
unnamed one, because `link/src/meshcore_companion.cpp:1065` — "    const core::MeshPeer* sender = find_peer_prefix(&data[prefix]);" —
