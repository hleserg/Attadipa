...
| G-06 | Hard-iron calibration | BLOCKED | placement (T-109) **and a rail** (G-14). Not one thing: a hard-iron offset is **stored**, and [ADR-0009](../adr/0009-heading.md) invalidates a calibration record only *"when the provider changes"* — a new load appearing on a shared rail is not a provider change, so an offset fitted on the wrong rail outlives the session that produced it. Placing the sensor does not unblock it |
| G-07 | Soft-iron calibration | BLOCKED | placement (T-109) **and a rail** (G-14), for the same reason as G-06 and more so: a soft-iron matrix is fitted against whatever fields were present while it ran, stored, and never retracted by a rail change. Placing the sensor does not unblock it |
