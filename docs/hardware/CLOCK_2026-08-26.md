...
- The ESP-IDF flash build completed at `1,079,584` bytes (`0x107920`) against
...
- A debug-injected tap at `(300, 300)` on the physical device produced the
  expanding ring and pulled the animated points and firefly mark toward the
  tap. The screen remained controllable after the interaction.
...
- The tap above was injected through the debug protocol. It exercises the same
