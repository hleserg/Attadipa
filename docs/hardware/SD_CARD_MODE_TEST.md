...
The connector here is **micro-SD**, whose contact numbering is not the full-size
one, so these are the micro-SD numbers: contact 3 is `CMD` in SD mode and
`DI`/`MOSI` in SPI mode, contact 5 is `CLK`/`SCK`, and contact 7 is `DAT0` in SD
mode and `DO`/`MISO` in SPI. SPI adds a chip select on contact **2**
(`CD`/`DAT3`) and nothing else. So `MOSI` and `CMD` do not merely *may*
...
**These were the full-size numbers until the eighth review round of
[#155](https://github.com/hleserg/Attadipa/pull/155).** On a full-size card the
four signals are contacts 2 (`CMD`), 5 (`CLK`), 7 (`DAT0`) and 1 (`CS`); on
micro-SD they are 3, 5, 7 and 2. `CLK` and `DAT0` coincide, which is how the
error survived seven rounds — the two that move are `CMD` and **the chip
select**, and the chip select is the one contact the argument turns on. Source:
the micro-SD and full-size pin-assignment tables of the **SD Physical Layer
Simplified Specification** (`1 DAT2 · 2 CD/DAT3 · 3 CMD · 4 VDD · 5 CLK ·
6 VSS · 7 DAT0 · 8 DAT1`), read 2026-08-24 from a reproduction of the 9.00
tables rather than from the specification PDF, whose text this session could not
extract. The numbering is `VERIFIED`; that the socket on the received unit is a
micro-SD socket is recorded in prose here and in `REUSE_LEDGER`, and has **not**
been read off a connector datasheet or the schematic.
...
- SDMMC 1-bit works and GPIO 17 **follows both pulls** → as far as this test can
  see the pin is floating. **That is not evidence it fails to reach the
  connector.** An SD card's `CD`/`DAT3` contact is a high-impedance input, so a
  GPIO 17 that reaches it and a GPIO 17 that reaches nothing read identically
  under an internal pull — and with no card inserted the contact is not
  connected to anything at all. The reading excludes a strap, a divider or a
  rail on that pin, and nothing more. Nothing has been learnt about the
  connector, only about the host driver.
...
3. **Steps 0–5 write nothing to the card.** Enumeration reads the CID and CSD
...
([WAVESHARE_RUNNING_OUR_CODE](../research/WAVESHARE_RUNNING_OUR_CODE.md) §3.3).
...
it; a pin that follows both is floating as far as this test can tell — and §0
says why that second reading does *not* mean the pin misses the connector. That
is the whole of the read, and it is the observation §0 says the success path was
missing. **Do not set it as an output.** §1 rule 7.

**Then disarm both pulls before anything else runs.** This is not tidiness:

```c
gpio_set_pull_mode(GPIO_NUM_17, GPIO_FLOATING);   /* both pulls off */
```

`gpio_set_direction(…, GPIO_MODE_INPUT)` does **not** clear a pull already
latched in the pad, and loading a RAM image does not reset the peripherals
([WAVESHARE_RUNNING_OUR_CODE](../research/WAVESHARE_RUNNING_OUR_CODE.md) §5, and
§8 below), so a pull left on here is still on through steps 1–3. If GPIO 17 does
reach the card's `CD`/`DAT3` — the hypothesis this procedure exists to test —
then **a card that samples CMD0 with `CS` low enters SPI mode, one-way until the
card is power-cycled.** It then ignores the native host and step 1 fails at
`send_op_cond` with **`0x107`**: the empty-slot line this whole procedure exists
to disown, produced with a card in the slot, by this step. Whether the internal
pull-down (~45 kΩ) would win against the pull-up on `DAT3` is **UNKNOWN** — that
pull-up is on the far side of the connector this file cannot trace, which is
exactly why rule 7 covers holding an unidentified pin at an indeterminate level
and not only driving it.
...
**Read back from the source, 2026-08-24.** The eighth review round had no
network and flagged these numbers as possibly the ESP32-classic slot-1 IO_MUX
pinout, misread out of the wrong `#elif` arm the way the P4 numbers had been.
They are not. The header was fetched at tag **v5.4** — whose tag object is
`8e27ea72c6688b79348b123ff40d556cfe16c8c3`, the SHA cited above — and lines
99–115 are the `#elif CONFIG_IDF_TARGET_ESP32S3` arm, reading `clk 14, cmd 15,
d0 2, d1 4, d2 12, d3 13, d4 33, d5 34, d6 35, d7 36`, exactly as given here.
The resemblance to ESP32-classic slot 1 is real and is not an error: Espressif
reused those numbers for the S3 arm. **`VERIFIED` against the fetched header.**

**And on this board `clk = 14` and `cmd = 15` are the I2C bus.**
[HARDWARE_MATRIX](../research/HARDWARE_MATRIX.md)'s main-I2C row gives SDA 15,
SCL 14, **VERIFIED by measurement**, carrying five devices — including the
AXP2101 at `0x34`, the PMU that owns this board's rails. Those two are also the
only slot defaults ESP-IDF actually configures at `width = 1`, by the citation
below, while `d4`–`d7` are the ones it never touches. The snippet overrides both
at `slot.clk`/`slot.cmd`, so the step is safe as written — but the nearest
reason to write those two overrides explicitly is the PMU's bus, not the memory
rail, and an operator adapting this file to another probe is the person who
needs to know it.
...
and check that step 0's `GPIO_FLOATING` actually ran, because a pull still
latched from step 0 is inside this step's measurement rather than outside it.
...
1. **Step 1 failed** with a card in the slot. Not *"step 1 and step 2 both
   failed"*, which is what this precondition said until the eighth review round
   of [#155](https://github.com/hleserg/Attadipa/pull/155): step 2 runs **only
   if step 1 enumerated** (§5), so the two can never both be false, no path
   opened this gate, and both failure rows of §4's table route here. A slot that
