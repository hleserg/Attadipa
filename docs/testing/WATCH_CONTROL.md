...
(`meshcore_ble.cpp:2394` "bool configure_meshcore_ble") had exactly one caller,
...
(`meshcore_ble.cpp:206` "std::atomic_bool configured", `meshcore_ble.cpp:208`
"std::atomic_bool reconnect_allowed"), the `Configure` event is the only thing
that sets `configured` **true** (`meshcore_ble.cpp:1848`
"configured.store(true)", `meshcore_ble.cpp:1849`
"reconnect_allowed.store(true)" — every other write clears them), and
`start_scan()` returns unless both are true (`meshcore_ble.cpp:664`
...
(`meshcore_ble.cpp:1849` "reconnect_allowed.store(true)"), which is #325's
...
written to NVS by the worker (`meshcore_ble.cpp:1842`
"attadipa::firmware::persist_passkey(persist_ops, event.passkey);") and boot replays it through the same
`Configure` event (`meshcore_ble.cpp:2361` "restore_passkey();"). The zero of
...
(`meshcore_ble.cpp:1890` "if (!erase_passkey()) {"), so a watch told to stop
...
`nimble_port_init()` (`meshcore_ble.cpp:2146` "nimble_port_init()"), brings the
controller up and creates the `meshcore` task with a 6,144-byte stack
(`meshcore_ble.cpp:2167` "xTaskCreate(mesh_task") for a subsystem that scans
...
starts an active scan with no deadline (`firmware/main/meshcore_ble.cpp:675` —
...
(`firmware/main/board_power.cpp:498` — "esp_light_sleep_start();") without a PM
