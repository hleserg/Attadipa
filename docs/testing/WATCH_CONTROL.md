...
(`meshcore_ble.cpp:1024` "bool configure_meshcore_ble") has exactly one caller,
...
(`meshcore_ble.cpp:129` "std::atomic_bool configured", `meshcore_ble.cpp:131`
"std::atomic_bool reconnect_allowed"), the `Configure` event is the only thing
that sets either of them **true** (`meshcore_ble.cpp:909`
"configured.store(true)", `meshcore_ble.cpp:910`
"reconnect_allowed.store(true)" — every other write clears them), and
`start_scan()` returns unless both are true (`meshcore_ble.cpp:206`
...
`nimble_port_init()` (`meshcore_ble.cpp:1009` "nimble_port_init()"), brings the
controller up and creates the `meshcore` task with a 6,144-byte stack
(`meshcore_ble.cpp:1004` "xTaskCreate(mesh_task") for a subsystem that can never
