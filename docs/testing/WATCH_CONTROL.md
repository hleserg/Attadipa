...
(`waveshare_board.cpp:1056` "restore_time_metadata()") is outside the `#if` — and,
...
outside the `#if` and `BoardTimeSink` (`waveshare_board.cpp:595`
...
`BoardMeshSink::configure` (`waveshare_board.cpp:618`
...
"std::atomic_bool mesh_screen_requested") is set only at `waveshare_board.cpp:621`
