...
(`meshcore_ble.cpp:2543` "bool configure_meshcore_ble") had exactly one caller,
...
that sets `configured` **true** (`meshcore_ble.cpp:1940`
"configured.store(true)", `meshcore_ble.cpp:1942`
"reconnect_allowed.store(true)" — every other write clears them), and
`start_scan()` returns unless both are true (`meshcore_ble.cpp:696`
...
(`meshcore_ble.cpp:1942` "reconnect_allowed.store(true)"), which is #325's
...
written to NVS by the worker (`meshcore_ble.cpp:1934`
"attadipa::firmware::persist_passkey(persist_ops, event.passkey);") and boot replays it through the same
`Configure` event (`meshcore_ble.cpp:2510` "restore_passkey();"). The zero of
...
`nimble_port_init()` (`meshcore_ble.cpp:2295` "nimble_port_init()"), brings the
controller up and creates the `meshcore` task with a 6,144-byte stack
(`meshcore_ble.cpp:2316` "xTaskCreate(mesh_task") for a subsystem that scans
...
starts an active scan with no deadline (`firmware/main/meshcore_ble.cpp:707` —
