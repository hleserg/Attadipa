...
(`waveshare_board.cpp:269` "restore_time_metadata()") is outside the `#if` — and,
...
outside the `#if` and `BoardTimeSink` (`waveshare_board.cpp:609`
...
(`meshcore_ble.cpp:1987` "bool configure_meshcore_ble") had exactly one caller,
`BoardMeshSink::configure` (`waveshare_board.cpp:632`
...
(`meshcore_ble.cpp:168` "std::atomic_bool configured", `meshcore_ble.cpp:170`
"std::atomic_bool reconnect_allowed"), the `Configure` event is the only thing
that sets `configured` **true** (`meshcore_ble.cpp:1576`
"configured.store(true)", `meshcore_ble.cpp:1577`
"reconnect_allowed.store(true)" — every other write clears them), and
`start_scan()` returns unless both are true (`meshcore_ble.cpp:509`
...
(`meshcore_ble.cpp:1577` "reconnect_allowed.store(true)"), which is #325's
...
written to NVS by the worker (`meshcore_ble.cpp:1570`
"store_passkey(event.passkey)") and boot replays it through the same
`Configure` event (`meshcore_ble.cpp:1972` "restore_passkey();"). The zero of
...
(`meshcore_ble.cpp:1618` "if (!erase_passkey()) {"), so a watch told to stop
...
press on the clock opens ends on a passkey field, and its `waveshare_board.cpp:474`
...
`mesh_screen_requested` (`waveshare_board.cpp:147`
"std::atomic_bool mesh_screen_requested") is set only at `waveshare_board.cpp:635`
...
`nimble_port_init()` (`meshcore_ble.cpp:1820` "nimble_port_init()"), brings the
controller up and creates the `meshcore` task with a 6,144-byte stack
(`meshcore_ble.cpp:1841` "xTaskCreate(mesh_task") for a subsystem that scans
...
starts an active scan with no deadline (`firmware/main/meshcore_ble.cpp:520` —
