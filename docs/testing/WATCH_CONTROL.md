...
the same `0` as *expire immediately*: `debug/src/bridge.cpp:754` releases when
`now_ms - pointer_down_at_ > limits_.max_hold_ms`, which is already true one
millisecond after the `PointerDown`, and `:740` does the same for buttons.
...
(`waveshare_board.cpp:674` "restore_time_metadata()") is outside the `#if` — but
...
(`meshcore_ble.cpp:1210` "bool configure_meshcore_ble") has exactly one caller,
...
that sets `configured` **true** (`meshcore_ble.cpp:1034`
"configured.store(true)", `meshcore_ble.cpp:1035`
...
(`meshcore_ble.cpp:1101` "reconnect_allowed.store(true)"), which is #325's
...
`nimble_port_init()` (`meshcore_ble.cpp:1195` "nimble_port_init()"), brings the
controller up and creates the `meshcore` task with a 6,144-byte stack
(`meshcore_ble.cpp:1190` "xTaskCreate(mesh_task") for a subsystem that can never
