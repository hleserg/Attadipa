...
the same `0` as *expire immediately*: `debug/src/bridge.cpp:913` —
...
`debug/src/bridge.cpp:899` — "now_ms - button_down_at_[i] > limits_.max_hold_ms"
...
`bridge.cpp:479` — "caps.max_hold_ms" — copies the enforced limit into the
...
| `the device is already doing one of those` | two overlapping requests of the same kind: a second screenshot mid-transfer, or a second `mesh-forget-bond` while one is running. **For forget-bond it also answers a first and only request** — see the next row | let the first finish. Still **never** the input queue — that is `QueueFull`, its own code and its own subsystem |
| `a MeshCore forget-bond is already running on the watch` | `mesh-forget-bond` sent twice — **or sent once**, when the previous invocation's host went away while its deletion was still running and the worker still holds the slot | wait and re-run; the operation in flight is somebody's, not necessarily a request you sent. This says nothing about the bond: this request never reached the bond store, and it used to be reported as a deletion that failed, which sent the operator to re-run a command that had already worked |
...
outside the `#if` and `BoardTimeSink` (`waveshare_board.cpp:637`
...
(`meshcore_ble.cpp:2607` "bool configure_meshcore_ble") had exactly one caller,
`BoardMeshSink::configure` (`waveshare_board.cpp:660`
...
(`meshcore_ble.cpp:222` "std::atomic_bool configured", `meshcore_ble.cpp:224`
"std::atomic_bool reconnect_allowed"), the `Configure` event is the only thing
that sets `configured` **true** (`meshcore_ble.cpp:1991`
"configured.store(true)", `meshcore_ble.cpp:1993`
"reconnect_allowed.store(true)" — every other write clears them), and
`start_scan()` returns unless both are true (`meshcore_ble.cpp:749`
...
(`meshcore_ble.cpp:1993` "reconnect_allowed.store(true)"), which is #325's
...
written to NVS by the worker (`meshcore_ble.cpp:1985`
"attadipa::firmware::persist_passkey(persist_ops, event.passkey);") and boot replays it through the same
`Configure` event (`meshcore_ble.cpp:2574` "restore_passkey();"). The zero of
...
(`meshcore_ble.cpp:2044`
"if (!attadipa::firmware::erase_passkey(persist_ops)) {"), so a watch told to stop
...
press on the clock opens ends on a passkey field, and its `waveshare_board.cpp:500`
...
"std::atomic_bool mesh_screen_requested") is set only at `waveshare_board.cpp:663`
...
`nimble_port_init()` (`meshcore_ble.cpp:2359` "nimble_port_init()"), brings the
controller up and creates the `meshcore` task with a 6,144-byte stack
(`meshcore_ble.cpp:2380` "xTaskCreate(mesh_task") for a subsystem that scans
...
starts an active scan with no deadline (`firmware/main/meshcore_ble.cpp:735` —
...
(`firmware/main/board_power.cpp:503` — "esp_light_sleep_start();") without a PM
