...
(`meshcore_ble.cpp:2424` "bool configure_meshcore_ble") had exactly one caller,
...
that sets `configured` **true** (`meshcore_ble.cpp:1878`
"configured.store(true)", `meshcore_ble.cpp:1879`
...
(`meshcore_ble.cpp:1879` "reconnect_allowed.store(true)"), which is #325's
...
written to NVS by the worker (`meshcore_ble.cpp:1872`
"attadipa::firmware::persist_passkey(persist_ops, event.passkey);") and boot replays it through the same
`Configure` event (`meshcore_ble.cpp:2391` "restore_passkey();"). The zero of
...
(`meshcore_ble.cpp:1920` "if (!erase_passkey()) {"), so a watch told to stop
...
`nimble_port_init()` (`meshcore_ble.cpp:2176` "nimble_port_init()"), brings the
controller up and creates the `meshcore` task with a 6,144-byte stack
(`meshcore_ble.cpp:2197` "xTaskCreate(mesh_task") for a subsystem that scans
