...
(`waveshare_board.cpp:1149` "restore_time_metadata()") is outside the `#if` — and,
...
outside the `#if` and `BoardTimeSink` (`waveshare_board.cpp:545`
...
(`meshcore_ble.cpp:1877` "bool configure_meshcore_ble") had exactly one caller,
`BoardMeshSink::configure` (`waveshare_board.cpp:568`
...
`Configure` event (`meshcore_ble.cpp:1862` "restore_passkey();"). The zero of
...
"std::atomic_bool mesh_screen_requested") is set only at `waveshare_board.cpp:571`
