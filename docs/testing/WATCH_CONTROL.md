...
(`meshcore_ble.cpp:2200` "bool configure_meshcore_ble") had exactly one caller,
...
that sets `configured` **true** (`meshcore_ble.cpp:1731`
"configured.store(true)", `meshcore_ble.cpp:1732`
...
(`meshcore_ble.cpp:1732` "reconnect_allowed.store(true)"), which is #325's
...
written to NVS by the worker (`meshcore_ble.cpp:1725`
"store_passkey(event.passkey) && clear_reprovision_pending())") and boot replays it through the same
`Configure` event (`meshcore_ble.cpp:2185` "restore_passkey();"). The zero of
...
(`meshcore_ble.cpp:1773` "if (!erase_passkey()) {"), so a watch told to stop
...
`nimble_port_init()` (`meshcore_ble.cpp:2019` "nimble_port_init()"), brings the
controller up and creates the `meshcore` task with a 6,144-byte stack
(`meshcore_ble.cpp:2040` "xTaskCreate(mesh_task") for a subsystem that scans
