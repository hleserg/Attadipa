...
(`meshcore_ble.cpp:2214` "bool configure_meshcore_ble") had exactly one caller,
...
that sets `configured` **true** (`meshcore_ble.cpp:1745`
"configured.store(true)", `meshcore_ble.cpp:1746`
...
(`meshcore_ble.cpp:1746` "reconnect_allowed.store(true)"), which is #325's
...
written to NVS by the worker (`meshcore_ble.cpp:1739`
"store_passkey(event.passkey) && clear_reprovision_pending())") and boot replays it through the same
`Configure` event (`meshcore_ble.cpp:2199` "restore_passkey();"). The zero of
...
(`meshcore_ble.cpp:1787` "if (!erase_passkey()) {"), so a watch told to stop
...
`nimble_port_init()` (`meshcore_ble.cpp:2033` "nimble_port_init()"), brings the
controller up and creates the `meshcore` task with a 6,144-byte stack
(`meshcore_ble.cpp:2054` "xTaskCreate(mesh_task") for a subsystem that scans
