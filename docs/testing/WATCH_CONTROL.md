...
(`waveshare_board.cpp:331` "restore_time_metadata()") is outside the `#if` — and,
since #356's second change, writes one too: `provision_time()` has two callers,
`BoardProvisioner` (`waveshare_board.cpp:515` "provision_time(ops, request,")
outside the `#if` and `BoardTimeSink` (`waveshare_board.cpp:673`
...
`BoardMeshSink::configure` (`waveshare_board.cpp:696`
...
press on the clock opens ends on a passkey field, and its `waveshare_board.cpp:536`
...
`mesh_screen_requested` (`waveshare_board.cpp:209`
"std::atomic_bool mesh_screen_requested") is set only at `waveshare_board.cpp:699`
...
(`firmware/main/board_power.cpp:428` — "esp_light_sleep_start();") without a PM
