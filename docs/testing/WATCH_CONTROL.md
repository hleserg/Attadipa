...
outside the `#if` and `BoardTimeSink` (`waveshare_board.cpp:610`
...
(`meshcore_ble.cpp:2084` "bool configure_meshcore_ble") had exactly one caller,
`BoardMeshSink::configure` (`waveshare_board.cpp:633`
...
that sets `configured` **true** (`meshcore_ble.cpp:1660`
"configured.store(true)", `meshcore_ble.cpp:1661`
"reconnect_allowed.store(true)" — every other write clears them), and
`start_scan()` returns unless both are true (`meshcore_ble.cpp:555`
...
(`meshcore_ble.cpp:1661` "reconnect_allowed.store(true)"), which is #325's
...
written to NVS by the worker (`meshcore_ble.cpp:1654`
"store_passkey(event.passkey) && clear_reprovision_pending())") and boot replays it through the same
`Configure` event (`meshcore_ble.cpp:2069` "restore_passkey();"). The zero of
...
(`meshcore_ble.cpp:1702` "if (!erase_passkey()) {"), so a watch told to stop
...
"std::atomic_bool mesh_screen_requested") is set only at `waveshare_board.cpp:636`
...
`nimble_port_init()` (`meshcore_ble.cpp:1904` "nimble_port_init()"), brings the
controller up and creates the `meshcore` task with a 6,144-byte stack
(`meshcore_ble.cpp:1925` "xTaskCreate(mesh_task") for a subsystem that scans
