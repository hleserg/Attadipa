...
| `the device answered the handshake without a session generation…` | the firmware or simulator was built before #348 and speaks debug protocol v1: its `HELLO_OK` has no generation to echo, so this tool cannot tell its replies from a previous invocation's | rebuild the simulator, or reflash the watch, from a checkout that has #348. Not a timeout and not retried: the device answered, and the answer is that it cannot open a session |
...
(`waveshare_board.cpp:939` "restore_time_metadata()") is outside the `#if` — but
...
`BoardMeshSink::configure` (`waveshare_board.cpp:555`
...
"std::atomic_bool mesh_screen_requested") is set only at `waveshare_board.cpp:558`
