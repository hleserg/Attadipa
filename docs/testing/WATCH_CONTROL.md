...
(`meshcore_ble.cpp:2483` "bool configure_meshcore_ble") had exactly one caller,
...
(`meshcore_ble.cpp:221` "std::atomic_bool configured", `meshcore_ble.cpp:223`
"std::atomic_bool reconnect_allowed"), the `Configure` event is the only thing
that sets `configured` **true** (`meshcore_ble.cpp:1922`
"configured.store(true)", `meshcore_ble.cpp:1923`
"reconnect_allowed.store(true)" — every other write clears them), and
`start_scan()` returns unless both are true (`meshcore_ble.cpp:686`
...
(`meshcore_ble.cpp:1923` "reconnect_allowed.store(true)"), which is #325's
...
written to NVS by the worker (`meshcore_ble.cpp:1916`
"attadipa::firmware::persist_passkey(persist_ops, event.passkey);") and boot replays it through the same
`Configure` event (`meshcore_ble.cpp:2450` "restore_passkey();"). The zero of
...
(`meshcore_ble.cpp:1973` "if (!erase_passkey()) {"), so a watch told to stop
...
`nimble_port_init()` (`meshcore_ble.cpp:2235` "nimble_port_init()"), brings the
controller up and creates the `meshcore` task with a 6,144-byte stack
(`meshcore_ble.cpp:2256` "xTaskCreate(mesh_task") for a subsystem that scans
...
starts an active scan with no deadline (`firmware/main/meshcore_ble.cpp:697` —
