...
(`meshcore_ble.cpp:1040` "bool configure_meshcore_ble") has exactly one caller,
...
that sets either of them **true** (`meshcore_ble.cpp:925`
"configured.store(true)", `meshcore_ble.cpp:926`
"reconnect_allowed.store(true)" — every other write clears them), and
`start_scan()` returns unless both are true (`meshcore_ble.cpp:209`
...
`nimble_port_init()` (`meshcore_ble.cpp:1025` "nimble_port_init()"), brings the
controller up and creates the `meshcore` task with a 6,144-byte stack
(`meshcore_ble.cpp:1020` "xTaskCreate(mesh_task") for a subsystem that can never
