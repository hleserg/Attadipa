...
(`waveshare_board.cpp:291` "restore_time_metadata()") is outside the `#if` — and,
since #356's second change, writes one too: `provision_time()` has two callers,
`BoardProvisioner` (`waveshare_board.cpp:475` "provision_time(ops, request,")
outside the `#if` and `BoardTimeSink` (`waveshare_board.cpp:633`
...
`BoardMeshSink::configure` (`waveshare_board.cpp:656`
...
press on the clock opens ends on a passkey field, and its `waveshare_board.cpp:496`
...
`mesh_screen_requested` (`waveshare_board.cpp:169`
"std::atomic_bool mesh_screen_requested") is set only at `waveshare_board.cpp:659`
