...
([`MESHCORE_COMPANION_PROTOCOL.md:392`](MESHCORE_COMPANION_PROTOCOL.md)
...
`link/src/meshcore_companion.cpp:624` —
"(void)copy_text(status_.node_name, &data[58], size - 58);" — with the public
key at 4, `link/src/meshcore_companion.cpp:621` — "std::memcpy(status_.node_id.public_key.data(), &data[4],".
...
builds. [`MESHCORE_COMPANION_PROTOCOL.md:431`](MESHCORE_COMPANION_PROTOCOL.md)
...
which the companion owns: `link/src/meshcore_companion.cpp:612` — "if (size < 58) { ++malformed_frames_; return false; }"
...
(`tests/test_meshcore_companion.cpp:901` — "CHECK(client.malformed_frames() == 1);")
...
handshake fails), `Failed` (the transport fault: `firmware/main/meshcore_ble.cpp:1279` — "provider.fault(now());"
lands as `link/src/meshcore_companion.cpp:279` — "status_.availability = core::Availability::Failed;")
...
`link/src/meshcore_companion.cpp:612` — "if (size < 58) { ++malformed_frames_; return false; }"
