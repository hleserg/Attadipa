...
`link/src/meshcore_companion.cpp:1348` —
"(void)copy_text(status_.node_name, &data[58], size - 58);" — with the public
key at 4, `link/src/meshcore_companion.cpp:1345` — "std::memcpy(status_.node_id.public_key.data(), &data[4],".
...
which the companion owns: `link/src/meshcore_companion.cpp:1331` — "if (size < 58) { ++malformed_frames_; return false; }"
...
handshake fails), `Failed` (the transport fault: `firmware/main/meshcore_ble.cpp:1436` — "provider.fault(now());"
...
`link/src/meshcore_companion.cpp:1331` — "if (size < 58) { ++malformed_frames_; return false; }"
