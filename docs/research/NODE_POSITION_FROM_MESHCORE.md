...
`link/src/meshcore_companion.cpp:1039` —
"(void)copy_text(status_.node_name, &data[58], size - 58);" — with the public
key at 4, `link/src/meshcore_companion.cpp:1036` — "std::memcpy(status_.node_id.public_key.data(), &data[4],".
...
which the companion owns: `link/src/meshcore_companion.cpp:1022` — "if (size < 58) { ++malformed_frames_; return false; }"
...
lands as `link/src/meshcore_companion.cpp:403` — "status_.availability = core::Availability::Failed;")
...
`link/src/meshcore_companion.cpp:1022` — "if (size < 58) { ++malformed_frames_; return false; }"
