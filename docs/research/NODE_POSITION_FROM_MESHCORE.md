...
`link/src/meshcore_companion.cpp:1067` —
"(void)copy_text(status_.node_name, &data[58], size - 58);" — with the public
key at 4, `link/src/meshcore_companion.cpp:1064` — "std::memcpy(status_.node_id.public_key.data(), &data[4],".
...
which the companion owns: `link/src/meshcore_companion.cpp:1050` — "if (size < 58) { ++malformed_frames_; return false; }"
...
`link/src/meshcore_companion.cpp:1050` — "if (size < 58) { ++malformed_frames_; return false; }"
