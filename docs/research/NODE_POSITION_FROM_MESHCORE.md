...
`link/src/meshcore_companion.cpp:1204` —
"(void)copy_text(status_.node_name, &data[58], size - 58);" — with the public
key at 4, `link/src/meshcore_companion.cpp:1201` — "std::memcpy(status_.node_id.public_key.data(), &data[4],".
...
which the companion owns: `link/src/meshcore_companion.cpp:1187` — "if (size < 58) { ++malformed_frames_; return false; }"
...
lands as `link/src/meshcore_companion.cpp:486` — "status_.availability = core::Availability::Failed;")
...
`link/src/meshcore_companion.cpp:1187` — "if (size < 58) { ++malformed_frames_; return false; }"
