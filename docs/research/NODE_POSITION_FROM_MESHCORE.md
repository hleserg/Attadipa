...
`link/src/meshcore_companion.cpp:1352` —
"(void)copy_text(status_.node_name, &data[58], size - 58);" — with the public
key at 4, `link/src/meshcore_companion.cpp:1349` — "std::memcpy(status_.node_id.public_key.data(), &data[4],".
...
which the companion owns: `link/src/meshcore_companion.cpp:1335` — "if (size < 58) { ++malformed_frames_; return false; }"
...
handshake fails), `Failed` (the transport fault: `firmware/main/meshcore_ble.cpp:1496` — "provider.fault(now());"
lands as `link/src/meshcore_companion.cpp:586` — "status_.availability = core::Availability::Failed;")
...
`link/src/meshcore_companion.cpp:1335` — "if (size < 58) { ++malformed_frames_; return false; }"
