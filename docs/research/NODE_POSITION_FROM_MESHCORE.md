...
`link/src/meshcore_companion.cpp:567` —
"(void)copy_text(status_.node_name, &data[58], size - 58);" — with the public
key at 4, `link/src/meshcore_companion.cpp:564` — "std::memcpy(status_.node_id.public_key.data(), &data[4],".
...
which the companion owns: `link/src/meshcore_companion.cpp:555` — "if (size < 58) { ++malformed_frames_; return false; }"
...
(`tests/test_meshcore_companion.cpp:705` — "CHECK(client.malformed_frames() == 1);")
...
lands as `link/src/meshcore_companion.cpp:239` — "status_.availability = core::Availability::Failed;")
...
`link/src/meshcore_companion.cpp:555` — "if (size < 58) { ++malformed_frames_; return false; }"
