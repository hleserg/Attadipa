...
| Current draw per power state, on either board | **PARTIAL** — H1. **One state is MEASURED**: the Waveshare at its USB input, cell disconnected, screen on at minimum brightness on the provisioning entry screen, idle and unprovisioned — a median 82.83 mA at a median 4.990 V, **413 mW median** (415 mW mean), 2026-09-05, S16 ([VERIFIED_FACTS](VERIFIED_FACTS.md)). That is input power at VBUS, upstream of the PMU, not a rail split. Every other state, and the whole T-Watch, is still **NOT EXECUTED — HARDWARE REQUIRED** |
