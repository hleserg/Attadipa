...
| Current draw per power state, on either board | **PARTIAL** — H1. **One idle state on the Waveshare is MEASURED**, 2026-09-05, S16 — the figure, its conditions and its three residual unknowns are in [VERIFIED_FACTS](VERIFIED_FACTS.md) and are not restated here. It is input power at VBUS, upstream of the PMU, so it separates no rail. Every other state, and the whole T-Watch, is still **NOT EXECUTED — HARDWARE REQUIRED** |
