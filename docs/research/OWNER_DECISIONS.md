...
> **The prescription beside it — "select by address" (`:1138`) — is withdrawn:
> the one BLE address this repository has measured is a *random* one.**
> `peer_addr_type=1`,
> [`MESHCORE_T114_FIRST_CONTACT.md:47`](MESHCORE_T114_FIRST_CONTACT.md)
> "random", `MEASURED` 2026-08-28 — a random address rotates, so it identifies
> a node for the length of a connection and no longer. The public key is the
> only measured identity that outlives one, and even it is regenerated by a
> factory reset (`:50`). **What to select by is therefore open, and it is
