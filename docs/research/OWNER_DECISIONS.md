...
**Numbering — and the honest version, because the tidy one was wrong.** This
entry took **OD-21** to stay clear of five open pull requests, and an earlier
draft said OD-16 through OD-20 were *allocated* to them. They are not. Read off
the branches on 2026-08-23:

| Pull request | `## OD-` heading it adds |
|---|---|
| #92 | `OD-16` — A1, A2 and A3 |
| #94 | `OD-17` — A5 and A6, the external magnetometer |
| #97 | `OD-16` — A10, the display wake sources |
| #95 | `OD-16` — A9, the day theme on an emissive panel |
| #112 | `OD-16` — A5 and A6, **the same heading text as #94** |

**Four pull requests claim OD-16.** Two numbers are in use across the five, not
five, so OD-21 leaves a hole at OD-18, OD-19 and OD-20 that no decision will
ever fill — and a later reader would go hunting for three that were never
written. The first of the four to merge takes OD-16; the other three must be
renumbered by whoever merges them, not by a map written in advance, because
merge order is not something this file can predict. #94 and #112 recording the
same decision under different numbers is a separate problem and belongs to those
two pull requests.

**There is no CI gate on this yet.** An earlier draft said
`tools/docs/check_docs.py` refuses a duplicate `## OD-` heading, in the present
tense. It does not: on `main` that checker runs five checks — links, code spans,
duplicate task IDs, task bodies, and the root-file allowlist — and the string
`OD-` does not appear in it. **#92 adds the duplicate-decision check; until #92
merges, a collision is silent** and two OD-16 headings reach `main` with CI
green and every `#od-16--…` anchor resolving to whichever one GitHub renders
first. Found in review, in the entry written to be relied on without
re-checking — which is the reason this paragraph says what is true rather than
what would be tidy.
...
3. **Owner-facing blocks on a public surface are bilingual**, English first and
   ...
   **This rule ships already breached, and it is filed rather than narrowed.**
   Three owner-facing texts on that channel are generated by workflows and are
   English only: `attadipa_receipt` and `attadipa_outcome` in
   `.github/scripts/agent-say.sh` — the second posted by `Hand over` on every
   exit path — and the missing-credential `BLOCKED` block generated in bash in
   `.github/workflows/claude-agent.yml`. They are literal strings in shell and
   YAML with no model in the loop, so no agent can comply from inside a run.
   **T-131** does the work. Review found the contradiction and offered the
   alternative of scoping this item to text an agent writes by hand; that was
   refused, because the unattended fleet is exactly where the owner has no other
   channel, and narrowing the rule would fix the wording by giving up the point.
   The shape is fixed even where the text is not: the English half stays first,
   which is what keeps `^\s*BLOCKED:` and the marker matchers reading what they
   expect.
...
- The reading that the exception list is "the two specification documents".
  Since #118 it is those two **and the owner's own two files** under
  `docs/ideas/`, which `docs/ideas/README.md` places on the same footing —
  *"**they** are kept here verbatim and unedited"*, meaning
  `idei-i-dorabotki.md` and `nosimye-ustroystva.md`. **Not the directory:**
  `docs/ideas/README.md` is itself an agent-written index and is English,
  correctly so. An earlier draft of this entry and of `CLAUDE.md` said
  *"everything under `docs/ideas/`"* and cited that very file as the link, which
  would have had the next agent adding a Russian row to an English table. Found
  in review.
