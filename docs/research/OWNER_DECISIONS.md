...
answer.
## OD-20 — A10: the display wakes on raise, button or touch; the raise gesture reads the accelerometer only

**Decided:** 2026-08-22, on [issue #53](https://github.com/hleserg/Attadipa/issues/53).

**Filed as OD-16 and renumbered on merge, 2026-08-24.** Four pull requests each
claimed OD-16 for a different decision while they sat conflicted; `main` took the
A1/A2/A3 one, so it keeps the number. This is a different decision and needed a
free one. **OD-20** was verified free across every remote branch rather than
against `main` alone — OD-17 through OD-19 and OD-21 are each allocated on a
branch that has not landed. The duplicate-decision check in `check_docs.py` is
what caught the collision, on the merged tree, which is the only place it exists.
