...
| **Waveshare's BSP component configures the charger not at all**, and the image actually running is opaque. **Correction, 2026-08-23:** this row used to name that image `xiaozhi`. The flash dump says otherwise — `otadata` is blank, so the bootloader falls through to `factory`, which holds **`phone_s3_box_3`**; `xiaozhi 1.8.5` sits unselected in `ota_0`. The opacity is unchanged and the name was wrong, and it had been borrowed onward. | Vendor repo; [WAVESHARE_FLASH_LAYOUT](WAVESHARE_FLASH_LAYOUT.md) §2.1 |
