...
- **Independently corroborated:** the same arithmetic puts `node_name` at offset
  ...
  `link/src/meshcore_companion.cpp:524` — "(void)copy_text(status_.node_name, &data[58], size - 58);".
...
- **Checked:** 2026-09-02. A fact about the toolchain; an ESP-IDF upgrade
  re-reads it.
- **Consequence:** `firmware/main/waveshare_board.cpp:278` —
...
- **Consequence:** the boot log at `firmware/main/waveshare_board.cpp:281` —
