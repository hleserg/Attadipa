...
fact into a semantic claim — `link/src/meshcore_companion.cpp:1138` —
...
1. `RESP_CODE_CONTACTS_START` records the node's count, clears the retained
   peers and clears both completion flags — `link/src/meshcore_companion.cpp:1126` —
...
6. Every other valid push — including all four invalidating ones — reaches the
   ...
   `link/src/meshcore_companion.cpp:1370` — "// A response code this build does not know is a frame we did not".
...
set: `link/src/meshcore_companion.cpp:1124` — "        peer_count_ = 0;" — and with
