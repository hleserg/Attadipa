...
fact into a semantic claim — `link/src/meshcore_companion.cpp:1068` —
...
1. `RESP_CODE_CONTACTS_START` records the node's count, clears the retained
   peers and clears both completion flags — `link/src/meshcore_companion.cpp:1056` —
...
6. Every other valid push — including all four invalidating ones — reaches the
   ...
   `link/src/meshcore_companion.cpp:1300` — "// A response code this build does not know is a frame we did not".
...
set: `link/src/meshcore_companion.cpp:1054` — "        peer_count_ = 0;" — and with
