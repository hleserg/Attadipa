...
boolean is asked to: `core/include/attadipa/core/mesh_service.h:211` —
...
1. `RESP_CODE_CONTACTS_START` records the node's count, clears the retained
   peers and clears both completion flags — `link/src/meshcore_companion.cpp:1369` —
...
2. Each `RESP_CODE_CONTACT` is length-checked and accepted into the 16-entry
   ...
   doubles — `link/src/meshcore_companion.cpp:650` — "    if (count < table.size()) {".
3. `RESP_CODE_END_OF_CONTACTS` sets both flags —
   `link/src/meshcore_companion.cpp:715` — "contacts_complete_ = true;" — and
...
4. **Since #567 the boundary frame is no longer the only way that happens.** A
   ...
   `tick()` — `link/src/meshcore_companion.cpp:470` — "            status_.peers_complete = true;" — because a
...
- **The re-read must not re-arm the message drain.** Ending a contact walk is
  ...
  `CMD_SYNC_NEXT_MESSAGE` — `link/src/meshcore_companion.cpp:714` — "    if (!request_next_message(now)) return false;"
...
set: `link/src/meshcore_companion.cpp:1367` — "        peer_count_ = 0;" — and with
...
| **an incoming message loses its sender's name** | `find_peer_prefix()` walks only `peer_count_` — `link/src/meshcore_companion.cpp:665` — "    for (std::size_t i = 0; i < peer_count_; ++i) {" — and an unresolved prefix blanks the field | the wearer is shown a message from nobody |
