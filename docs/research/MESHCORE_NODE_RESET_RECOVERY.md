...
[`MESHCORE_T114_FIRST_CONTACT.md:739`](MESHCORE_T114_FIRST_CONTACT.md) —
...
[`firmware/main/meshcore_ble.cpp:957`](../../firmware/main/meshcore_ble.cpp) —
...
| The pinned node public key | NVS, `attadipa_mesh` / [`firmware/main/meshcore_ble.cpp:258`](../../firmware/main/meshcore_ble.cpp) — "constexpr const char* kNodeKeyNvsKey" | **untouched, and untouchable** — section 4 | **yes** |
...
passkey. That sets
[`firmware/main/meshcore_ble.cpp:1736`](../../firmware/main/meshcore_ble.cpp) —
...
[`firmware/main/meshcore_ble.cpp:1833`](../../firmware/main/meshcore_ble.cpp) —
...
[`firmware/main/meshcore_ble.cpp:2032`](../../firmware/main/meshcore_ble.cpp) —
...
confirm value, [`firmware/main/meshcore_ble.cpp:659`](../../firmware/main/meshcore_ble.cpp) —
...
- [`link/src/meshcore_companion.cpp:811`](../../link/src/meshcore_companion.cpp) —
...
`store_node_pin()` — [`firmware/main/meshcore_ble.cpp:462`](../../firmware/main/meshcore_ble.cpp) —
...
- [`firmware/main/meshcore_ble.cpp:250`](../../firmware/main/meshcore_ble.cpp) —
  "What the image has since #411 is the reverse";
- [`core/include/attadipa/core/mesh_service.h:76`](../../core/include/attadipa/core/mesh_service.h) —
...
1. A recovery that *replays* the stored passkey is wrong: it is stale the
   ...
   replaces the value —
   [`firmware/main/meshcore_ble.cpp:435`](../../firmware/main/meshcore_ble.cpp) —
   ...
   [`firmware/main/meshcore_ble.cpp:2101`](../../firmware/main/meshcore_ble.cpp) —
   "no MeshCore passkey stored; BLE stays unconfigured", and
   [`firmware/main/meshcore_ble.cpp:1307`](../../firmware/main/meshcore_ble.cpp) —
...
| The RAM copy of that pin — `pinned_set_`, `pinned_`, `status_.pinned_id`, `has_pinned`; **`pinned_set_` is the one that decides**: [`link/src/meshcore_companion.cpp:1173`](../../link/src/meshcore_companion.cpp) — "if (!pinned_set_) return false;" is the whole of `pinned()`, and a clear that zeroes the key but leaves it true refuses *every* node, since [`link/src/meshcore_companion.cpp:811`](../../link/src/meshcore_companion.cpp) — "if (pinned_set_ && !(status_.node_id == pinned_)) {" is then true for any key and the adopt path behind [`firmware/main/meshcore_node_pin.h:188`](../../firmware/main/meshcore_node_pin.h) — "if (!ops.pinned(expected)) {" is unreachable | the worker-owned `MeshCoreCompanion` singleton, [`firmware/main/meshcore_ble.cpp:133`](../../firmware/main/meshcore_ble.cpp) — "attadipa::link::MeshCoreCompanion provider;" | it is what `settle_node_pin()` actually asks — [`firmware/main/meshcore_ble.cpp:1558`](../../firmware/main/meshcore_ble.cpp) — "return provider.pinned(out);" — never NVS. It has two writers — boot, [`:2166`](../../firmware/main/meshcore_ble.cpp) — "provider.pin(pinned);", and the adopt path, [`:1562`](../../firmware/main/meshcore_ble.cpp) — "void adopt(const attadipa::core::MeshPeerId& id) { provider.pin(id); }" from [`firmware/main/meshcore_node_pin.h:190`](../../firmware/main/meshcore_node_pin.h) — "ops.adopt(seen);" — and the second is what completes a recovery: the next handshake with no pin stores the new key to NVS ([`:189`](../../firmware/main/meshcore_node_pin.h) — "if (!ops.store(seen)) return PinOutcome::AdoptFailed;" → `nvs_set_blob`, a set, not an erase-then-set) and pins it in RAM. `begin()` keeps it across sessions on purpose ([`link/src/meshcore_companion.cpp:161`](../../link/src/meshcore_companion.cpp) — "// `status_.pinned_id` and `status_.refused_id` are deliberately NOT cleared"), and there is no unpin: [`link/include/attadipa/link/meshcore_companion.h:91`](../../link/include/attadipa/link/meshcore_companion.h) — "void pin(const core::MeshPeerId& node);" is the whole write side. Clear NVS alone and the next handshake refuses the node exactly as before, until the watch reboots — the reboot this whole report exists to remove. The mesh screen reads the same copy: [`apps/src/mesh.cpp:56`](../../apps/src/mesh.cpp) — "    if (status.has_refused && status.has_pinned && !is_terminal(status)) {" |
| The live session and `reconnect_allowed` | RAM | the deletion cannot happen under a live encrypted session — the existing worker already terminates first, [`firmware/main/meshcore_ble.cpp:1833`](../../firmware/main/meshcore_ble.cpp) "const int deleted = ble_store_util_delete_peer(&address);" is preceded by a terminate |
...
- *The NVS eraser is a crash-recovery guard, not what makes the clear take
  ...
  [`firmware/main/meshcore_ble.cpp:2166`](../../firmware/main/meshcore_ble.cpp) —
...
- *Between the clear and the adoption the watch is unpinned*, in the state
  [`firmware/main/meshcore_ble.cpp:2183`](../../firmware/main/meshcore_ble.cpp) —
...
[`../../firmware/main/meshcore_ble.cpp:1737`](../../firmware/main/meshcore_ble.cpp) —
...
([`../../firmware/main/meshcore_ble.cpp:405`](../../firmware/main/meshcore_ble.cpp) —
...
([`../../firmware/main/meshcore_ble.cpp:1730`](../../firmware/main/meshcore_ble.cpp) —
