...
[`MESHCORE_T114_FIRST_CONTACT.md:749`](MESHCORE_T114_FIRST_CONTACT.md) —
...
[`firmware/main/meshcore_ble.cpp:1081`](../../firmware/main/meshcore_ble.cpp) —
"BLE_HS_HCI_ERR(BLE_ERR_PINKEY_MISSING)) {", which calls
[`firmware/main/meshcore_ble.cpp:1082`](../../firmware/main/meshcore_ble.cpp) —
"(void)record_stale_bond(event->enc_change.conn_handle," and then falls into
[`firmware/main/meshcore_ble.cpp:1085`](../../firmware/main/meshcore_ble.cpp) —
...
| The pinned node public key | NVS, `attadipa_mesh` / [`firmware/main/meshcore_ble.cpp:289`](../../firmware/main/meshcore_ble.cpp) — "constexpr const char* kNodeKeyNvsKey" | **untouched, and untouchable** — section 4 | **yes** |
| The stored passkey | NVS, [`firmware/main/meshcore_ble.cpp:292`](../../firmware/main/meshcore_ble.cpp) — "constexpr const char* kPasskeyNvsKey" | untouched | **yes**, replayed at boot: [`firmware/main/meshcore_ble.cpp:2521`](../../firmware/main/meshcore_ble.cpp) — "restore_passkey();" |
| `reconnect_allowed` | RAM atomic | cleared: [`firmware/main/meshcore_ble.cpp:779`](../../firmware/main/meshcore_ble.cpp) — "reconnect_allowed.store(false);" inside `disconnect_fault()`. This is the fail-closed stop | **no** |
...
([`firmware/main/meshcore_forget_outcome.h:52`](../../firmware/main/meshcore_forget_outcome.h) —
...
passkey. That sets
[`firmware/main/meshcore_ble.cpp:1945`](../../firmware/main/meshcore_ble.cpp) —
"configured.store(true);" and
[`firmware/main/meshcore_ble.cpp:1947`](../../firmware/main/meshcore_ble.cpp) —
"reconnect_allowed.store(true);", and scanning resumes. The watch connects,
sees `secure_pairing` at
[`firmware/main/meshcore_ble.cpp:1060`](../../firmware/main/meshcore_ble.cpp) — "if (secure_pairing.load()) {",
...
[`firmware/main/meshcore_ble.cpp:2085`](../../firmware/main/meshcore_ble.cpp) —
"const int deleted = ble_store_util_delete_peer(&address);" and re-arms one
attempt at [`firmware/main/meshcore_ble.cpp:2123`](../../firmware/main/meshcore_ble.cpp) —
...
[`firmware/main/meshcore_ble.cpp:2314`](../../firmware/main/meshcore_ble.cpp) —
...
confirm value, [`firmware/main/meshcore_ble.cpp:779`](../../firmware/main/meshcore_ble.cpp) —
...
`store_node_pin()` — [`firmware/main/meshcore_ble.cpp:561`](../../firmware/main/meshcore_ble.cpp) —
...
that one — [`firmware/main/meshcore_ble.cpp:1998`](../../firmware/main/meshcore_ble.cpp) —
...
- [`firmware/main/meshcore_ble.cpp:281`](../../firmware/main/meshcore_ble.cpp) —
...
1. A recovery that *replays* the stored passkey is wrong: it is stale the
   ...
   replaces the value —
   [`firmware/main/meshcore_ble.cpp:483`](../../firmware/main/meshcore_ble.cpp) —
   ...
   [`firmware/main/meshcore_ble.cpp:2419`](../../firmware/main/meshcore_ble.cpp) —
   "no MeshCore passkey stored; BLE stays unconfigured", and
   [`firmware/main/meshcore_ble.cpp:1435`](../../firmware/main/meshcore_ble.cpp) —
...
| The RAM copy of that pin — `pinned_set_`, `pinned_`, `status_.pinned_id`, `has_pinned`; **`pinned_set_` is the one that decides**: [`link/src/meshcore_companion.cpp:1991`](../../link/src/meshcore_companion.cpp) — "if (!pinned_set_) return false;" is the whole of `pinned()`, and a clear that zeroes the key but leaves it true refuses *every* node, since [`link/src/meshcore_companion.cpp:1353`](../../link/src/meshcore_companion.cpp) — "if (pinned_set_ && !(status_.node_id == pinned_)) {" is then true for any key and the adopt path behind [`firmware/main/meshcore_node_pin.h:209`](../../firmware/main/meshcore_node_pin.h) — "if (!ops.pinned(expected)) {" is unreachable | the worker-owned `MeshCoreCompanion` singleton, [`firmware/main/meshcore_ble.cpp:162`](../../firmware/main/meshcore_ble.cpp) — "attadipa::link::MeshCoreCompanion provider;" | it is what `settle_node_pin()` actually asks — [`firmware/main/meshcore_ble.cpp:1719`](../../firmware/main/meshcore_ble.cpp) — "return provider.pinned(out);" — never NVS. It has two writers — boot, [`firmware/main/meshcore_ble.cpp:2489`](../../firmware/main/meshcore_ble.cpp) — "provider.pin(pinned);", and the adopt path, [`firmware/main/meshcore_ble.cpp:1725`](../../firmware/main/meshcore_ble.cpp) — "void adopt(const attadipa::core::MeshPeerId& id) { provider.pin(id); }" from [`firmware/main/meshcore_node_pin.h:213`](../../firmware/main/meshcore_node_pin.h) — "ops.adopt(seen);" — and the second is what completes a recovery: the next handshake with no pin stores the new key to NVS ([`:210`](../../firmware/main/meshcore_node_pin.h) — "if (!(ops.begin_pin_write() && ops.store(seen)))" → `nvs_set_blob`, a set, not an erase-then-set, between a durable gate's raise and its erase since #647) and pins it in RAM. `begin()` keeps it across sessions on purpose ([`link/src/meshcore_companion.cpp:185`](../../link/src/meshcore_companion.cpp) — "// `status_.pinned_id` and `status_.refused_id` are deliberately NOT cleared"), and there is no unpin: [`link/include/attadipa/link/meshcore_companion.h:93`](../../link/include/attadipa/link/meshcore_companion.h) — "void pin(const core::MeshPeerId& node);" is the whole write side. Clear NVS alone and the next handshake refuses the node exactly as before, until the watch reboots — the reboot this whole report exists to remove. The mesh screen reads the same copy: [`apps/src/mesh.cpp:56`](../../apps/src/mesh.cpp) — "    if (status.has_refused && status.has_pinned && !is_terminal(status)) {" |
| The live session and `reconnect_allowed` | RAM | the deletion cannot happen under a live encrypted session — the existing worker already terminates first, [`firmware/main/meshcore_ble.cpp:2085`](../../firmware/main/meshcore_ble.cpp) "const int deleted = ble_store_util_delete_peer(&address);" is preceded by a terminate |
...
- *The NVS eraser is a crash-recovery guard, not what makes the clear take
  ...
  [`firmware/main/meshcore_ble.cpp:2489`](../../firmware/main/meshcore_ble.cpp) —
...
- *Between the clear and the adoption the watch is unpinned*, in the state
  [`firmware/main/meshcore_ble.cpp:2506`](../../firmware/main/meshcore_ble.cpp) —
...
[`../../firmware/main/meshcore_node_forget.h:129`](../../firmware/main/meshcore_node_forget.h) —
...
([`../../firmware/main/meshcore_node_forget.h:135`](../../firmware/main/meshcore_node_forget.h) —
...
([`../../firmware/main/meshcore_ble.cpp:475`](../../firmware/main/meshcore_ble.cpp) —
"    return PasskeyReplay::Inhibited;"). The old digits remain stored but are
not armed after a power loss; an owner-entered replacement raises its own
write gate, commits, and only then lowers the forget marker and that gate
([`../../firmware/main/meshcore_passkey.h:108`](../../firmware/main/meshcore_passkey.h) —
...
[`docs/adr/0018-owner-consent-for-provisioning.md:83`](../adr/0018-owner-consent-for-provisioning.md) —
