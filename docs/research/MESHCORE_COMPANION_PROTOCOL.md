...
| Whether the first-party JS and Python clients agree with this reading | **narrowed twice, and still open in general.** `meshcore.js` at `9e76c514` was read for the 148-byte contact frame ([REUSE_LEDGER](REUSE_LEDGER.md)), and read again on 2026-09-14 for the two send frames: `onSentResponse` and `onSendConfirmedPush` parse `RESP_CODE_SENT` and `PUSH_CODE_SEND_CONFIRMED` field for field as §5 records them — [OUTBOUND_MESHCORE_MESSAGES](OUTBOUND_MESHCORE_MESSAGES.md) §2.2–2.3. Three frames of fifty-odd, so the row stands for the rest |
