...
[`sim/options.cpp:248`](../../sim/options.cpp) — "out.child_mode = true;" and
[`sim/options.cpp:259`](../../sim/options.cpp) — "out.clock_time_set = true;"; and the tick period
...
| Axis | Today | Evidence |
|---|---|---|
| geometry | **yes** — `--board t-watch-s3-plus` / `waveshare-amoled-206` | `sim/options.cpp:316` — "out.board = *found;" |
| theme | **yes** — `--theme`, and `T` at runtime | `sim/options.cpp:348-362` — "out.theme = ui::Theme::Day;" |
| locale | **yes** — `--locale`, and `L` at runtime | `sim/options.cpp:334-348` — "out.locale = l10n::Locale::En;" |
| Adult/Child | **yes** — `--child` | [`sim/options.cpp:248`](../../sim/options.cpp) — "out.child_mode = true;", rendered at [`sim/main.cpp:247`](../../sim/main.cpp) — "apps::ClockMode::Child" |
| a specific time | **yes** — `--clock-time <unix seconds>` | [`sim/options.cpp:259`](../../sim/options.cpp) — "out.clock_time_set = true;", consumed at [`sim/main.cpp:249`](../../sim/main.cpp) — "!options.clock_time_set" and passed to `build_clock_screen` as its `live` argument |
| **battery / charging** | no injection | final §57 also asks for *"simulated battery"* |
| node attached / detached | **yes** — `--node` | `sim/options.cpp:190-193` — "out.node_attached = true;" |
