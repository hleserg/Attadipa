...
[`sim/options.cpp:167`](../../sim/options.cpp) — "out.child_mode = true;" and
[`sim/options.cpp:178`](../../sim/options.cpp) — "out.clock_time_set = true;"; and the tick period
...
definition.** `sim/main.cpp:342` — "core::Capability::Time" — already lists
...
| Axis | Today | Evidence |
|---|---|---|
| geometry | **yes** — `--board t-watch-s3-plus` / `waveshare-amoled-206` | `sim/options.cpp:228-240` — "out.board = *found;" |
...
| Adult/Child | **yes** — `--child` | [`sim/options.cpp:167`](../../sim/options.cpp) — "out.child_mode = true;", rendered at [`sim/main.cpp:492`](../../sim/main.cpp) — "apps::ClockMode::Child" |
| a specific time | **yes** — `--clock-time <unix seconds>` | [`sim/options.cpp:178`](../../sim/options.cpp) — "out.clock_time_set = true;", consumed at [`sim/main.cpp:470`](../../sim/main.cpp) — "g_clock_live = !options.clock_time_set" |
...
| screenshot | **yes**, but the **first frame only** | `sim/main.cpp:583` — "if (options.screenshot != nullptr)" — takes the snapshot, then the frame loop runs |
