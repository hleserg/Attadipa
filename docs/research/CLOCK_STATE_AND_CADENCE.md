...
definition.** `sim/main.cpp:351` — "core::Capability::Time" — already lists
...
| Adult/Child | **yes** — `--child` | [`sim/options.cpp:171`](../../sim/options.cpp) — "out.child_mode = true;", rendered at [`sim/main.cpp:501`](../../sim/main.cpp) — "apps::ClockMode::Child" |
| a specific time | **yes** — `--clock-time <unix seconds>` | [`sim/options.cpp:182`](../../sim/options.cpp) — "out.clock_time_set = true;", consumed at [`sim/main.cpp:479`](../../sim/main.cpp) — "g_clock_live = !options.clock_time_set" |
...
| screenshot | **yes**, but the **first frame only** | `sim/main.cpp:592` — "if (options.screenshot != nullptr)" — takes the snapshot, then the frame loop runs |
