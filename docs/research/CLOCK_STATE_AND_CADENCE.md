...
| Adult/Child | **yes** — `--child` | [`sim/options.cpp:189`](../../sim/options.cpp) — "out.child_mode = true;", rendered at [`sim/main.cpp:243`](../../sim/main.cpp) — "apps::ClockMode::Child" |
| a specific time | **yes** — `--clock-time <unix seconds>` | [`sim/options.cpp:200`](../../sim/options.cpp) — "out.clock_time_set = true;", consumed at [`sim/main.cpp:245`](../../sim/main.cpp) — "!options.clock_time_set" and passed to `build_clock_screen` as its `live` argument |
...
| screenshot | **yes**, but the **first frame only** | `sim/main.cpp:328` — "if (options.screenshot != nullptr)" — takes the snapshot, then the frame loop runs |
