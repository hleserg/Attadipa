...
The tool reads `assets/fonts/generated/attadipa_montserrat_*.c` — the exact
bytes the firmware and the simulator link — and applies LVGL v9.5.0's own
...
(`tools/font/generate_ui_fonts.py:44`). **This is scaffolding, not the product
typeface** — D16 is open and neither Inter nor Nunito Sans is pinned
([FONT_MEASUREMENTS](FONT_MEASUREMENTS.md)). Every number below therefore
measures *the font a Clock built today would use*, and would have to be re-run
after D16.
...
| which typeface, and therefore every number in §7 | D16, open since T-032 |
