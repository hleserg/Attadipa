...
[`sim/options.cpp:189`](../../sim/options.cpp) — "out.child_mode = true;" and
[`sim/options.cpp:200`](../../sim/options.cpp) — "out.clock_time_set = true;"; and the tick period
...
| Axis | Today | Evidence |
|---|---|---|
| geometry | **yes** — `--board t-watch-s3-plus` / `waveshare-amoled-206` | `sim/options.cpp:250-262` — "out.board = *found;" |
...
| Adult/Child | **yes** — `--child` | [`sim/options.cpp:189`](../../sim/options.cpp) — "out.child_mode = true;", rendered at [`sim/main.cpp:208`](../../sim/main.cpp) — "apps::ClockMode::Child" |
| a specific time | **yes** — `--clock-time <unix seconds>` | [`sim/options.cpp:200`](../../sim/options.cpp) — "out.clock_time_set = true;", consumed at [`sim/main.cpp:210`](../../sim/main.cpp) — "!options.clock_time_set" and passed to `build_clock_screen` as its `live` argument |
...
| screenshot | **yes**, but the **first frame only** | `sim/main.cpp:293` — "if (options.screenshot != nullptr)" — takes the snapshot, then the frame loop runs |
