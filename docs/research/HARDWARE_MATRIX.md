...
| Buttons | **two case keys: PWR and BOOT.** The current official schematic/product page resolve the old text-extraction ambiguity. PWR pulls AXP2101 `PWRON`; BOOT pulls GPIO0 low. `SYS_OUT/GPIO10` reports the resulting power state, not the key level. The vendor BSP still declares none. | physical BOOT: two debounced `down/up` pairs measured through `core::InputQueue`; PWR is read through AXP2101 edge status | `power` remotely injectable; `boot` not injectable because it is a reset strap | D5 CLOSED 2026-08-25 | VERIFIED / PWR event observation pending |
