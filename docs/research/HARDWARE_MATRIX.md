...
| Battery | **940 mAh — confirmed off the cell's own label** on a physical unit, 2026-08-27 (`TERKDELL`, date code `20250922`). **The nominal voltage is now settled at 3.8 V, high-voltage chemistry.** Two independent unit-specific sources agree — the cell's own label, and the AXP2101's factory charge target read off this unit on 2026-08-28: `REG 0x64` = `0x04` = `100b` = **4.35 V**, decode traced to XPowersLib ([TWATCH_S3_PLUS_DOWNLOAD_MODE](TWATCH_S3_PLUS_DOWNLOAD_MODE_2026-08-28.md) §8). The vendor document's 3.7 V is the outlier. It decides charge termination — 4.35 V against 4.2 V — and no code sets an AXP2101 charge target today — the repository's only prescription for that register is [BATTERY_UPGRADE](BATTERY_UPGRADE.md) §8, written for the other board's cell — so nothing is wrong yet, and the first change to set it here must honour D22's answer — 4.35 V — rather than that section's 4.2 V | Capacity and voltage both **VERIFIED (MEASURED)** — [TWATCH_S3_PLUS_BRINGUP_2026-08-27](TWATCH_S3_PLUS_BRINGUP_2026-08-27.md) §5a for the label, [TWATCH_S3_PLUS_DOWNLOAD_MODE](TWATCH_S3_PLUS_DOWNLOAD_MODE_2026-08-28.md) §8 for the charge target. **D22 closed** |
...
| Main I2C bus | — | SDA 10, SCL 11 | **Scanned 2026-08-28: four devices** — `0x19`, `0x34`, `0x51`, `0x5A`, exactly the four rows above. `0x38` correctly absent (separate touch bus); `0x42` absent but ambiguous, D9 | — | **VERIFIED by measurement** — [TWATCH_S3_PLUS_DOWNLOAD_MODE](TWATCH_S3_PLUS_DOWNLOAD_MODE_2026-08-28.md) §8 |
...
| Says | Source | Verdict |
|---|---|---|
| **1.54"** | the schematic LCD sheet: part `QT154C2408`, symbol `LCD_1.54-TOUCH` | **right**, and the part number decodes — see below |
...
| IMU | QMI8658, 6-axis. **The silicon reports `REVISION_ID = 0x7C`**, which is the value in `13-52-27 ∙ QMI8658C Datasheet ∙ Rev A` (© 2022 QST, 20 June 2022) — the document whose chapter 11 has a hardware pedometer. **The name here used to be `13-52-25`, unsourced; corrected under [#341](https://github.com/hleserg/Attadipa/issues/341).** Both Rev A documents exist and **both have been read**: they give `0x7C` in their register-description sections and `0b01101000` = `0x68` in their register-map summaries, so this byte does not choose between them. `13-52-27` is named because the schematic prints `QMI8658C` ([`VERIFIED_FACTS.md:829`](VERIFIED_FACTS.md) "no register tells them apart"). The QMI8658C Rev 0.6 document gives `0x79`, so **that document does not describe this part**, despite the schematic printing `QMI8658C` twice. **Board-frame axes are silkscreened beside it**: X toward the battery edge, Y toward the USB-C edge, Z as ⊙ out of the back face (H15, half answered) | main I2C; SDO/SA0 to GND, CS to VCC3V3 | **`0x6B`, MEASURED** — `0x6A` does not acknowledge. The schematic and revs 0.8/0.9/A are right; Rev 0.6's SA0-low → `0x6A` mapping is not this board | D13 | **VERIFIED** (was address CONFLICTING) — S13, bus scan and register read from a RAM app, [WAVESHARE_RUNNING_OUR_CODE](WAVESHARE_RUNNING_OUR_CODE.md) §3.1–3.2 |
| RTC | PCF85063ATL | main I2C; in S6, `CLKOUT` has a no-connect marker and `CLKOE` has no destination. Physical-unit continuity/revision is `UNKNOWN` (D20) | `0x51` — datasheet-fixed, NXP PCF85063A Rev. 7 §9.5.1 reserves `1010001` | D13 | VERIFIED for S6; physical route `UNKNOWN` — [RTC_SLOW_CLOCK](RTC_SLOW_CLOCK.md) |
...
| Buttons | **two case keys: PWR and BOOT.** PWR pulls the AXP2101 `PWRON` input low; `SYS_OUT/GPIO10` is PMU system state, not a key mirror. BOOT pulls GPIO0 low. The AXP IRQ net ends at `EXIO5`, not an ESP32-S3 GPIO. | physical BOOT and PWR edge pairs measured through `core::InputQueue`; awake PWR events use AXP2101 status. GPIO10 Light-sleep wake was physically disproved; corrected firmware wakes briefly every 100 ms to poll the latched PMU edge, leaving the panel off on misses | `power` remotely injectable; `boot` not injectable because it is a reset strap | D5 CLOSED 2026-08-25 | old GPIO10 wake FAILED; corrected PMU-poll PWR wake and direct GPIO38 touch wake **MEASURED** — [SLEEP_WAKE_2026-08-26](../hardware/SLEEP_WAKE_2026-08-26.md) |
| SD card | — | the BSP configures SDMMC 1-bit: CLK 2, CMD 1, D0 3. The schematic names the same three nets `MOSI`/`SCK`/`MISO` and adds a chip-select near **GPIO 17** that this pin map does not have | — | D13 | **The slot is VERIFIED; the mode is PARTIAL.** The vendor's shipping firmware drives it through the **SDMMC host driver** — S13. That is the vendor's *software* choice: the slot was **empty**, so the `send_op_cond` timeout it ends on is what every wiring produces, and no card has ever enumerated here. On the ESP32-S3 the pin numbers settle nothing either, because the SDMMC slots route through the GPIO matrix. **D14 does not close** — [#131](https://github.com/hleserg/Attadipa/issues/131), [SD_CARD_MODE_TEST](../hardware/SD_CARD_MODE_TEST.md), T-127 |
...
| **Expansion pad row** | ten plated pads along the board's bottom edge, individually silkscreened | `VBUS · GND · D+/IO20 · D-/IO19 · IO15 · IO14 · RXD · TXD · GND · 3V3` — **`IO15`/`IO14` are the main I2C bus, not spare GPIO**; the only uncommitted channel here is `RXD`/`TXD`, which is **UART0: `RXD` = GPIO 44, `TXD` = GPIO 43** | see the bus rows | `3V3` sourced, rail not identified | VERIFIED — S9, [WAVESHARE_BOARD_RECEIVED](WAVESHARE_BOARD_RECEIVED.md) §1.5. The GPIO numbers are traced from the schematic test points `TP13`/`TP6` to `U2` pins 49/50 and thence to ESP-IDF `soc/esp32s3/include/soc/uart_pins.h` — [GNSS_MODULES_READOFF_2026-09-04](GNSS_MODULES_READOFF_2026-09-04.md) §4 |
...
| S15 | **the received Waveshare unit itself, on 2026-08-28**, running the `pedo` probe as a **RAM boot** (`tools/flash/ramhold.py`; every segment loads to a RAM address, **nothing is written to flash**, so the unit still carries the T-166 candidate in flash — two of the five captures record the loader step itself, and **none of the five prints `esp_image: segment` or `Loaded app from partition`**, which is what a flash boot on this board prints — [WAVESHARE_RUNNING_OUR_CODE](WAVESHARE_RUNNING_OUR_CODE.md) `:73-77`) with the owner holding and moving the watch. The probe writes `CTRL2`, `CTRL7`, `CTRL8`, `CTRL9` and `CAL1_L..CAL4_H` on the IMU at `0x6B` and nothing else on any device — **in the shake and walk runs. The three desk runs are an earlier build that wrote `CTRL2`, `CTRL7` and `CTRL8` only**: they carry no `CTRL9 0x0D` block and no `CAL` writes — one of **two** sufficient causes for those runs reading zero, the other being that the board never moved ([PEDOMETER_BENCH_2026-08-28](PEDOMETER_BENCH_2026-08-28.md) `:275-276`, *"Zero is the correct answer for a stationary board"*) — so `CAL1_L..CAL4_H` residue on this part cannot be attributed to them — **plus `CTRL3` in the shake run only**, which that run's build added to raise the gyro for 6DOF and which is **neither captured by `--- before ---` nor restored**, so `CTRL3 = 0x36` is residue this session knowingly left on the part; the QMI8658 has no non-volatile configuration, and the probe restores the `CTRL2`/`CTRL7`/`CTRL8` values it found and verifies the restore — **verified on the three desk runs; the shake run's capture ends with neither the restore block nor the END banner the three desk runs both print, so for that run it is unverified and what it left is `UNKNOWN`: a clean cut with no traceback admits both a stopped logger over a running image that restored into a console nobody read — and what it restored to is the walk probe's `CTRL2 = 0x27, CTRL7 = 0x01, CTRL8 = 0x90`, which is `Pedo_EN` and `aEN` set, so on that branch the engine has been counting since 15:08 and the next `--- before ---` prints a nonzero found count with no walk behind it — and a power loss, which clears the part and prints `0`**. **The unit no longer carries the factory image** — T-166 replaced it on 2026-08-25 ([BENCH_DEVICES](BENCH_DEVICES.md)) — so register state *found* on this board is whatever the last program left, and **cannot be attributed**: four of the five runs found `CTRL2 = 0x24, CTRL7 = 0x03, CTRL8 = 0x00`, which is exactly the 2026-08-23 exit state and which an SoC restart alone would not have disturbed — whether the IMU actually kept power for those five days is not recorded — while the shake run found the walk probe's own armed state from 29 minutes earlier. Found state here is therefore **not** evidence about vendor firmware. What the vendor firmware wrote is a separate, answered question, measured under S13. [PEDOMETER_BENCH_2026-08-28](PEDOMETER_BENCH_2026-08-28.md) |

| S16 | **the received Waveshare unit, measured at its USB-C input 2026-09-05** by a FNIRSI **FNB-58** inline power meter — sold as an FNB-C5, but the USB identity is `2e3c:5558` `FNIRSI`/`FNB-58`, one of the four the `baryluk/fnirsi-usb-power-data-logger` protocol covers. **Data comes off the HID interface; the `ttyACM` the meter also exposes returns nothing, passively or to the vendor's start sequence.** The meter measures the *input*, upstream of the AXP2101: conversion losses are inside the number and rail-level draw is not. The **cell was disconnected**, which is the only reason the reading is board consumption rather than board-plus-charger. Trigger and PD were kept off at plain 5 V — AXP2101 datasheet V1.4 §5.1 Table 5-1 puts `VBUS` absolute maximum at **12 V**, and §6.5.4.3 "Power Off" says *"When VSYS<VOFF or VBUS>7V, AXP2101 will be powered off"*; the SWcharge V1.0 datasheet carries both unchanged. The meter's rated accuracy is `UNKNOWN`; a **2.484 mA** zero offset, self-measured with the output open, was subtracted. Raw logs are bench-only and not committed |
