...
| Main I2C bus | — | SDA 10, SCL 11. **Pull-up value and designators: `UNKNOWN`** — not read from S3/S4, and unlike the Waveshare row below nobody has needed them. It matters here for a different reason than a retrofit: `SDA` reaches the detachable GNSS daughterboard (D9), so this bus can change length and capacitance without a soldering iron | — | — | VERIFIED for the pins; pull-ups unread |
...
| Main I2C bus | — | SDA 15, SCL 14 — **and both are brought out on the expansion pad row below**, labelled there as bare `IO15`/`IO14`. **Pull-ups: `R49` 2.2 kΩ on `SDA`, `R23` 2.2 kΩ on `SCL`, both to `VCC3V3`** — plus **`C35` and `C34`, 22 pF from each line to `AGND`**. Read 2026-08-24 and previously unrecorded; there is exactly one pull-up per line on the whole drawing. **2.2 kΩ is stiffer than the 4.7–10 kΩ a retrofit would assume**, which is what makes the parallel-pull-up arithmetic in [MAGNETOMETER_RETROFIT](MAGNETOMETER_RETROFIT.md) §4.3.3 worth doing rather than eyeballing | **Scanned, 2026-08-23: five devices** — `0x18`, `0x34`, `0x40`, `0x51`, `0x6B`. The predicted sixth, `0x38` (touch), answers only after its reset is pulsed on GPIO 9 — touch row; `0x6A` is free, and so are `0x0C`, `0x0D` and `0x1E`, which is where a magnetometer retrofit would sit (T-109). **The scan ran at 100 kHz**, and at that speed neither the pull-up value nor the bus capacitance constrains anything; 400 kHz is a separate question with `Cb` `UNKNOWN` — [MAGNETOMETER_RETROFIT](MAGNETOMETER_RETROFIT.md) §4.3.4 | — | **VERIFIED by measurement** — S13, [WAVESHARE_RUNNING_OUR_CODE](WAVESHARE_RUNNING_OUR_CODE.md) §3.1. The earlier six-device list was predicted from documentation and is corrected here. **The pull-up and capacitor values are VERIFIED from the schematic** (S6, re-read by rendering rather than text extraction — see the note under Sources), not measured on the board. **The designator-to-net mapping was then re-derived independently from the drawing's vector paths and agrees** — `R23` on `SCL`, `R49` on `SDA`, both dots and both endpoints named in [MAGNETOMETER_RETROFIT](MAGNETOMETER_RETROFIT.md) §4.3.1 |
...
| S6 | `waveshareteam/ESP32-S3-Touch-AMOLED-2.06`, `Schematic/ESP32-S3-Touch-AMOLED-2.06-Schematic-V1.0.pdf` — **read**, 3 sheets, by text extraction; pin-to-net adjacency partially recoverable, see D3 and D13. md5 `b0cdcac0afb0c8605896d995676c4468`, sha256 `6d531fb458863c666210c92294a07204d675bcb7997a54fc219d92fadbbacf9d`. **Partially re-read 2026-08-24 by a second method** — rendering the sheet and reading the junction dots, rather than extracting text — which is how the main I2C pull-ups were recovered. A text extraction gives designators and values with no wires between them, so *every* row sourced to S6 alone that depends on what connects to what is weaker than one that names a rendered region. The pull-up row does; several older rows do not say which they are. **Read a third way on the same day, and this is the strongest of the three: extracting the PDF's vector paths**, in which wires are line segments and junction dots are filled curves, so connectivity comes out as coordinates rather than as a judgement about a picture. It reproduced the pull-up row exactly, in a context that had not made the rendered reading. **It has not been tried on D5 or D13**, which are the two questions still open against this file and are connectivity questions of the same shape — D13's own recorded next action is "read the schematic sheets visually", which is the weaker of the two methods. (Not D3: that one is `CLOSED — mis-stated`.) [MAGNETOMETER_RETROFIT](MAGNETOMETER_RETROFIT.md) §4.3.1 |
