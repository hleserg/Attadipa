...
| S15 | **the received Waveshare unit itself, on 2026-08-28**, running the `pedo` probe from flash with the owner holding and moving the watch. The probe writes `CTRL2`, `CTRL7`, `CTRL8`, `CTRL9` and `CAL1_L..CAL4_H` on the IMU at `0x6B` and nothing else on any device; the QMI8658 has no non-volatile configuration, and the probe restores the `CTRL2`/`CTRL7`/`CTRL8` values it found and verifies the restore. **The unit no longer carries the factory image** — T-166 replaced it on 2026-08-25 ([BENCH_DEVICES](BENCH_DEVICES.md)) — so register state *found* on this board is residue from Attadipa firmware and is **not** evidence about vendor firmware. [PEDOMETER_BENCH_2026-08-28](PEDOMETER_BENCH_2026-08-28.md) |
