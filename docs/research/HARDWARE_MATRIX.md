...
| S13 | **the assembled case of that same unit, under a hand** — the buttons counted by pressing them, 2026-08-23. A separate source from S9 because S9 is photographs and is defined as *silkscreen and populated-or-not only*: how many protrusions are *pressable* is tactile, and none of the four S9 frames is a side view of the case. Same unit, third method, on the S10/S11/S12 precedent. Reported by the owner on [#99](https://github.com/hleserg/Attadipa/issues/99); no separate write-up, and this row is the record |
