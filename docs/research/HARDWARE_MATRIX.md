...
| Buttons | **two case keys: PWR and BOOT.** PWR pulls the AXP2101 `PWRON` node low; that node also drives T1 through R16, and T1 plus R11 invert it onto `SYS_OUT/GPIO10` as an active-high key mirror. BOOT pulls GPIO0 low. The vendor BSP still declares none. | physical BOOT and PWR edge pairs measured through `core::InputQueue`; awake PWR events use AXP2101 status, while Light-sleep arms GPIO10 high because I2C cannot be polled asleep | `power` remotely injectable; `boot` not injectable because it is a reset strap | D5 CLOSED 2026-08-25 | schematic route VERIFIED; physical GPIO10 wake **NOT EXECUTED — HARDWARE REQUIRED** — [SLEEP_WAKE_2026-08-26](../hardware/SLEEP_WAKE_2026-08-26.md) |
