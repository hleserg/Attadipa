...
| S13 | **the received unit running, and running our own code at it** — an owner-authorised bench session on 2026-08-23. The vendor bootloader's and factory application's own boot log captured from 62 ms, plus a diagnostic written into the free OTA slot and a `PURE_RAM_APP` loaded over USB. Both routes to running our code closed; the flash was restored and re-verified byte-for-byte against the T-099 backup. [WAVESHARE_RUNNING_OUR_CODE](WAVESHARE_RUNNING_OUR_CODE.md) |
