...
| A11 | **Do one or two of the T114s carry GNSS?** [TEST_FLEET](TEST_FLEET.md) §1 records one headless node with no screen and no GNSS, and one with both. The indoor-fix observation recorded in [OWNER_DECISIONS](OWNER_DECISIONS.md) OD-16 says *"either unit"*, which reads as two | **UNKNOWN** | the project owner — **asked as [#124](https://github.com/hleserg/Attadipa/issues/124)**. Nothing is blocked either way — no node here gets an indoor fix under either reading — but [#90](https://github.com/hleserg/Attadipa/issues/90) and [#91](https://github.com/hleserg/Attadipa/issues/91) both read the fleet table, so a plan built on it should not be built on a number nobody has confirmed |
...
| H5 | Which wake sources are usable in practice, and what does each cost? **The Waveshare's QMI8658 INT1 is now known to reach a wake-capable GPIO** — GPIO21, inside the ESP32-S3's RTC-IO range 0–21 — so a raise gesture is an interrupt wake candidate, not necessarily a poll, on both boards ([WAVESHARE_ARRIVAL §3.2a](WAVESHARE_ARRIVAL.md), OD-20) | UNKNOWN — routing VERIFIED, cost still unmeasured | measurement; vendor table gives the shape |
...
| H16 | **The QMI8658's INT1 electrical configuration** — push-pull or open-drain, active level. The schematic gives the net (`QMI_INT1` → GPIO 21, [WAVESHARE_ARRIVAL §3.2a](WAVESHARE_ARRIVAL.md)) but draws no pull resistor, and the setting is a `CTRL1` register field, not a schematic fact | UNKNOWN | `CTRL1` in `13-52-25 ∙ QMI8658A ∙ Rev A` — **not** the QMI8658C Rev 0.6 document, which H14 above resolved does not describe this silicon (`REVISION_ID = 0x7C`, and Rev 0.6 calls `CTRL8` *"Reserved: Not Used"* where this part accepts `0x90`). The two disagree about control-register bits, and `CTRL1` is where INT1's drive type and active level live. Then confirm against the board on a bench before firmware relies on a polarity: a wrong active level on a level-triggered `ext0` leaves INT1 asserted at idle, so the SoC wakes the instant it sleeps — which presents as battery life, not as a wake bug |
...
| D5 | Waveshare button/wake inputs — BSP declares none; is that the board or the BSP? | **PARTIAL — it is the BSP, and the case carries two buttons** | The owner counted **two** pressable buttons on the assembled case, 2026-08-23 (**S13**, pressed). What that does *not* settle is which named input each one is: the schematic lists `Key1`, `Key3` and `PWRON`, a key may sit on `PWRON` **and** a GPIO at once, and `PWRON` may reach **neither** button — the T-Watch wires it to SW7 with no GPIO at all (`:112`). So both the GPIO assignment and **whether `PWRON` is under a finger on this board** are open, and the second is not a wiring detail: `PWRON` is a PMU input that can bring the system up from a state in which the SoC is not running, which a GPIO cannot. Bears on **H5** and on `core/include/attadipa/core/power_state.h:45`, where `Button` is one bit for both kinds. **Resolved by:** dump the GPIOs while pressing each button ([WAVESHARE_ARRIVAL](WAVESHARE_ARRIVAL.md) step 9), then read the AXP2101 `PWRON` and IRQ registers to see whether either press reaches the PMU |
...
| D7 | Exact ST7789V3 and CO5300 init sequences and their timing | UNKNOWN | vendor driver source. **Half of it is now also D21's**: the CO5300 datasheet's `3Ah`/`2Ch` 16-bit bit packing would close the controller half of the byte-order question without a board, and the same document answers this one |
...
| D13 | Waveshare: which loads sit on ALDO1, ALDO2 and ALDO3 — all three are 3.3 V — and **what runs on the 1.8 V ALDO4 rail**? | UNKNOWN | read the schematic sheets visually. The **live register state** is now recorded — [WAVESHARE_RUNNING_OUR_CODE](WAVESHARE_RUNNING_OUR_CODE.md) §3.4, read from the powered board without writing anything — so the voltages each rail is *set to* are known; what is on the other end of them is not |
| ~~D14~~ | ~~Waveshare SD card: the BSP uses SDMMC 1-bit on GPIO 1/2/3, but the schematic labels those nets `MOSI`/`SCK`/`MISO`. Which mode is the board actually wired for?~~ **SDMMC.** The vendor's shipping firmware calls into `sdmmc_common` and `vfs_fat_sdmmc` — the SDMMC host driver, not `sdspi` — and fails at `send_op_cond`, which is what an empty slot looks like. The schematic's SPI-style net names are labels, not a mode. Whether SPI would *also* work is untested and nothing needs it to be | **RESOLVED** | the unit's own factory firmware, 2026-08-23 — S13, [WAVESHARE_RUNNING_OUR_CODE](WAVESHARE_RUNNING_OUR_CODE.md) §3.3 |
