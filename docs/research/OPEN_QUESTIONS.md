...
- **The clock could not be set.** `write_rtc()` was reached only through
  ...
  `firmware/main/waveshare_board.cpp:444` — "class BoardProvisioner final : public attadipa::core::Provisioner {"
  is ungated and is reached from the entry screen a long press on the clock
  opens (`firmware/main/waveshare_board.cpp:906` — "void long_press(lv_event_t *) {").
