...
- **MeshCore never scanned.** `configure_meshcore_ble()` is the only writer of
  ...
  `firmware/main/meshcore_ble.cpp:1320` — "if (configured.load()) start_scan();"
...
- **A changed node cannot be recovered from, and this bullet understated it.**
  ...
  `firmware/main/meshcore_ble.cpp:475` — "nvs_set_blob(handle, kNodeKeyNvsKey"
...
([`core/include/attadipa/core/power_owner.h:352`](../../core/include/attadipa/core/power_owner.h) —
...
(`core/include/attadipa/core/power_owner.h:361` — "// That decision is made, in ADR-0016: such a consumer does not call `acquire()`").
