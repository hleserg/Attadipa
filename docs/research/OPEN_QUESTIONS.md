...
| # | Question | Status | Resolved by |
|---|---|---|---|
| H1 | Real power draw of Attadipa firmware per state, per board. **One state is now measured, 2026-09-05 (S16):** the Waveshare at its USB-C input with the cell disconnected, screen on at minimum brightness on the provisioning entry screen, idle and unprovisioned — a median 82.83 mA at a median 4.990 V, **413 mW median** (the mean is 415 mW). That is input power at VBUS, upstream of the AXP2101, so it is not a rail split, and it is board consumption only because the cell was off. [VERIFIED_FACTS](VERIFIED_FACTS.md) carries its provenance and its three residual unknowns — which build was on the board, the meter's rated accuracy, and whether the BLE radio was powered | **PARTIAL** (was UNKNOWN) | every other state, and the other board: sleep, screen off, per-rail split, and the whole T-Watch, which is micro-USB and needs an adapter the bench does not have. External instrumentation stays the only route — H2 established the PMU has no current channel |
