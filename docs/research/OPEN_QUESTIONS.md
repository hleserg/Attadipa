...
| # | Question | Status | Resolved by |
|---|---|---|---|
| H1 | Real power draw of Attadipa firmware per state, per board. **One idle state on the Waveshare is now measured, 2026-09-05 (S16)**. It is input power at VBUS, upstream of the AXP2101, so it separates no rail. The figure itself, its conditions and its residual unknowns live in [VERIFIED_FACTS](VERIFIED_FACTS.md) and are deliberately not copied here | **PARTIAL** (was UNKNOWN) | every other state, and the other board: sleep, screen off, per-rail split, and the whole T-Watch, which is micro-USB and needs an adapter the bench does not have. H2 established the PMU has no current channel, so no register read will ever supply them; **failing a shunt** — H2's open half, and the row below is where it lives — external instrumentation is the route |
...
- **The clock could not be set.** `write_rtc()` was reached only through
  ...
  opens (`firmware/main/waveshare_board.cpp:910` — "void long_press(lv_event_t *) {").
