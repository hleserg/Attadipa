...
| D8 | Is the T-Watch main I2C bus shared with anything timing-sensitive? | PARTIAL | schematic read: **four** addressable devices on SDA 10 / SCL 11 — `0x34`, `0x51`, `0x19`, `0x5A` — and **no fifth**: D9 is closed and the GNSS never reaches this bus. (This cell said *five* and *sixth*; it was counting the FT6336U at `0x38`, which [HARDWARE_MATRIX](HARDWARE_MATRIX.md):97 puts on the separate touch bus. All four answered a scan on 2026-08-28, [TWATCH_S3_PLUS_DOWNLOAD_MODE](TWATCH_S3_PLUS_DOWNLOAD_MODE_2026-08-28.md) §8.) Timing sensitivity still needs driver review |
...
| D10 | **What is radio `DIO3` (GPIO 6) for on this board — TCXO supply or a second interrupt?** | **PARTIAL — vendor firmware configures `DIO3` as TCXO control and never as an interrupt; whether the module contains a TCXO, and at what voltage, is UNKNOWN** | The vendor firmware answers the *binary* by omission, which is the strongest evidence available without a module datasheet. `LilyGoLib` builds its driver with `newModule()` = `new Module(LORA_CS, LORA_IRQ, LORA_RST, LORA_BUSY, SPI)` (`LilyGoWatchS3.h:33`) — **`DIO3` is not among the pins RadioLib is given**, so it is not an interrupt the driver could read — and then calls `radio.begin()` with no arguments (`LilyGoWatchS3.cpp:995`). `SX1262::begin()` defaults `tcxoVoltage = 1.6`, documented at `SX1262.h:52,58` as *"TCXO reference voltage to be set on DIO3"*, so every LilyGO build silently issues a `setDio3AsTcxoCtrl` at 1.6 V. The pin itself was already recorded — [HARDWARE_MATRIX](HARDWARE_MATRIX.md):102 carries `DIO3 6`, and the rendered radio sheet confirms all eight SPI and control nets. ⚠️ **`GPIO 6` must never be driven as an output**: it is the SX1262's DIO3 *regulator output*. That rule belongs in a `platform/` pin-role table where a wrong `gpio_set_direction` fails a build, next to the strapping pins — prose fails on a wrist. **Two things stay UNKNOWN and do not follow from the above:** whether the `HPD16B3` contains a TCXO at all (its datasheet is published nowhere reachable), and what voltage the module wants. Recorded as a **hypothesis, not a finding**: 1.6 V may be under-volting it — LilyGO's own Pager and all three upstream SX1262 examples pass 3.0 V, and a comparable module's published range is 1.7–3.3 V. [#326](https://github.com/hleserg/Attadipa/issues/326) |
