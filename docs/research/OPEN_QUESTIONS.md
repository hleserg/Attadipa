...
- **The clock could not be set.** `write_rtc()` was reached only through
  ...
  `firmware/main/waveshare_board.cpp:512` — "class BoardProvisioner final : public attadipa::core::Provisioner {"
  is ungated and is reached from the entry screen a long press on the clock
  opens (`firmware/main/waveshare_board.cpp:1092` — "void long_press(lv_event_t *) {").
...
- **MeshCore never scanned.** `configure_meshcore_ble()` is the only writer of
  ...
  (`firmware/main/waveshare_board.cpp:1136` —
  "  state.entry.emplace(provisioner, attadipa::apps::EntryTask::All, seed);"):
  `firmware/main/waveshare_board.cpp:543` — "set_mesh_passkey(std::uint32_t passkey) override {".
