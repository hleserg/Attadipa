...
| # | Question | Status | Resolved by |
|---|---|---|---|
| M36 | **Why did the T114's private send never confirm in 120 s?** One occurrence, §6b of [MESHCORE_T114_FIRST_CONTACT](MESHCORE_T114_FIRST_CONTACT.md): `RESP_CODE_SENT` arrived, nothing followed, and the recipient had no message. The node cannot have said more — its send-timeout hook is an empty function body — so no amount of reading closes this, and there is no air capture. It is the concrete case that makes `Failed` indefensible and `Unconfirmed` exactly right | **UNKNOWN** | two pin-matched nodes, a sniffer or a second receiver, and the same send repeated enough times to say whether it is a path, a range or a one-off. `NOT EXECUTED — HARDWARE REQUIRED` |
| M37 | **Can the node's own suggested timeout exceed this client's 15-second clamp?** `est_timeout` is `500 + 16.0 × airtime` flooded and `500 + (6.0 × airtime + 250) × (hops + 1)` direct, so it grows with payload and hops; the one value observed is 2406 ms for a short single-hop text. Arithmetic says a three-hop direct send passes 15 000 ms at about 563 ms of airtime. Whether a 128-byte MeshCore packet ever takes that long on this fleet's radio configuration is the missing term — and where it does, the watch publishes a verdict while the node is still legitimately waiting | **UNKNOWN**; the formulas are `VERIFIED` from source and the airtime is not | a bench send to a multi-hop recipient with a full-length body, logging the `est_timeout` the node returns. Costs one send and no write. `NOT EXECUTED — HARDWARE REQUIRED` |
