...
| # | Question | Status | Resolved by |
|---|---|---|---|
| H1 | Real power draw of Attadipa firmware per state, per board. **One idle state is now measured on each board — the Waveshare 2026-09-05 (S16), the T-Watch 2026-09-08 (S17)**. Both are input power at VBUS, upstream of the AXP2101, so neither separates a rail. The figures themselves, their conditions and their residual unknowns live in [VERIFIED_FACTS](VERIFIED_FACTS.md) and are deliberately not copied here | **PARTIAL** (was UNKNOWN) | every other state on both boards: sleep, screen off, per-rail split. The micro-USB adapter the T-Watch needed is on the bench since 2026-09-08 and is no longer a blocker. Two unknowns ride inside the T-Watch figure and **neither is recoverable from it**: **whether a cell was in it** (a long-press power-off while inline bounds the *VBUS-side* charge share from above and does not separate it from the loads that survive the press, so the one read that looked like a discriminator is not one; its matched cell-disconnected baseline, or a battery-side measurement once the AXP2101 variant is settled, measures the charge current of the day it runs, so **for the 2026-09-08 capture the share is unrecoverable**) and **what its powered GNSS receiver was doing**. So what is wanted next is not more captures of the same shape but **one designed capture that answers both** — cell state, the receiver's rail and sky state, and the meter's zero, each recorded on both halves of the control — which supersedes S17 rather than explaining it. A capture designed around the cell alone reproduces the GNSS unknown instead of closing it, because the firmware raises that rail itself. H2 established the PMU has no current channel, so no register read will ever supply any of them; **failing a shunt** — H2's open half, and the row below is where it lives — external instrumentation is the route |
...
| H16 | **What pull-up does each magnetometer module actually fit, and does the CJMCU-9911's `RST` pad reach the die?** Four ohmmeter readings on two bare modules, no board, no power, no soldering: `SDA`→`VCC` and `SCL`→`VCC` on each module gives the pull-up; `RST`→`VCC` and `RST`→`GND` on the CJMCU-9911 says whether the pad is tied, floating, or not connected at all. **Neither is guesswork-tolerant.** The pull-ups land in parallel with the Waveshare board's own 2.2 kΩ and the combination must stay above `UM10204`'s 3 mA sink limit — 4.7 kΩ passes, 3.3 kΩ does not — and `RSTN` left floating is forbidden by the AK09911C datasheet outright, which makes the retrofit five wires rather than four until this says otherwise. The arithmetic and both pass lines are already written, so this is a lookup rather than an analysis. **Probed on the purple module 2026-09-08** ([MAGNETOMETER_BENCH_2026-09-08](MAGNETOMETER_BENCH_2026-09-08.md)), and the first half of the procedure above is **withdrawn rather than answered**: `SDA`→`VCC` read 0.593 MΩ and `SCL`→`VCC` 0.594 MΩ, which is no pull-up at all, because continuity puts `VCC` on the single upper lead of a `662K` SOT-23 and the pull-ups on its output, point **P** — `SDA`→P reads 9.96 kΩ. `SDA`→`VCC` cannot give the pull-up on a module that regulates, which `docs/research/MAGNETOMETER_RETROFIT.md:52` — "`VCC` therefore must not be equated with sensor `VID`. Check reset against the" had already said in the reset half. The bench report's own limit is kept: 9.96 kΩ is *"evidence for that path, not three proven equal pull-ups"*. **The 3 mA pass line holds as an upper bound anyway** — 2.2 kΩ ∥ 9.96 kΩ = 1.80 kΩ sinks **1.61 mA** at `VOL` = 0.4 V, 54 % of the 3 mA — the arithmetic this repository already uses twice, `docs/research/MAGNETOMETER_RETROFIT.md:70` — "At `VOL = 0.4 V`, the board alone sinks 1.318 mA at 3.3 V. With a candidate". Taking the full 3.3 V instead prints 1.83 mA and bounds a different quantity: `UM10204`'s 3 mA is `IOL` **at** `VOL`, not at the rail. `ESTIMATED` because P's own voltage has not been measured. **The reset half is not answered:** `RST`→`VCC` and `RST`→`GND` both read OL, and *"OL at RST establishes neither a broken trace nor a valid reset-high voltage"*, so whether the pad reaches the die stays `UNKNOWN`. What is discharged is the gate this row put on the retrofit — on 2026-09-09 the module ran on the Waveshare with `CAD`/`RST` held high on the module itself and answered at `0x0D` ([MAGNETOMETER_RETROFIT](MAGNETOMETER_RETROFIT.md) §2.1), so the retrofit was four wires from the watch, not five | **PARTIAL** (was UNKNOWN — `NOT MEASURED`) | three things, none of them a gate on the module that is soldered. (a) Whether `RST` reaches the die — OL against `VCC` and `GND` cannot say it, and the reading that can is `RST` against the identified `VID` rail, which means identifying that rail first. (b) **P's voltage**, `NOT MEASURED` — the 2026-09-08 report's boundary asked for the module's internal rail before `SDA`/`SCL` were connected and the 2026-09-09 run does not record it, which is what leaves the sink arithmetic above `ESTIMATED`. (c) **The GY-271's pull-up**, `NOT MEASURED` — this row asks what *each* module fits and only the purple one was on the bench: `docs/research/MAGNETOMETER_BENCH_2026-09-08.md:42` — "not resolved sufficiently to confirm AK09911C. The blue GY-271 is not selected". OD-17 leaves the part choice open, so this half of the row is a gate again the moment the blue module is the one being wired — two 3.3 kΩ modules are what exceeds the limit, and nothing has read the blue one's value. [MAGNETOMETER_RETROFIT](MAGNETOMETER_RETROFIT.md) §2.6.3 and §4.3.3; owned by **T-109** |
| H18 | **What are the two GNSS modules on the bench, and what do they want on `VCC`?** Two receivers for the node path arrived 2026-09-02 — a "GT-U12" and a QUESCAN "AN3126", recorded from their listings only in [BENCH_DEVICES](BENCH_DEVICES.md#two-gnss-modules-delivered-2026-09-02--read-off-2026-09-04). Supply, logic level, baud, and the chip behind each were UNKNOWN. **ANSWERED on the bench 2026-09-04, except the supply half** — [GNSS_MODULES_READOFF_2026-09-04](GNSS_MODULES_READOFF_2026-09-04.md). The GT-U12 is an ALLYSTAR `HD8041D` die on an `HYT-1010` module at 115200 baud, speaking ALLYSTAR binary behind the sync word **`F1 D9`**, and it is a genuine **dual-band L1+L5** receiver — measured three ways, and the earlier single-band reading in these notes is retracted. The AN3126 is a **u-blox M10**, ROM SPG 5.10, `PROTVER=34.10`, at 38400 baud, **single band L1/E1**, with 4.07 m rms static scatter and a guarded 40.0 s cold start. The vendor tell this row predicted did not work: neither part sends `$GIGSV`, and it was the ALLYSTAR-documented `$GNTXT,01,01,01,ANT_OK*50` in the GT-U12's own output that named it, after twenty-four probed protocols were silent for want of the right sync word. **What is still open is the half this row leads with, and it is three things, not two:** (a) neither carrier has been inspected for a regulator — `UNKNOWN` on both; (b) neither module's TX idle voltage has been measured — `NOT MEASURED` on both; and (c) **the GT-U12 keeps streaming with its `VCC` lead disconnected**, across three guarded windows totalling 800 s, with the phantom path through `RX` separately disproved. The carrier back carries a round can — a backup cell or a supercapacitor — and if that holds the module up, it can drive its `TX` while the host rail is down. That is a back-drive case, and one idle-voltage reading taken with everything powered does not cover it, so (c) is part of this half rather than a curiosity beside it | **PARTIAL** (was UNKNOWN) | all three items above are what remains — the two regulators, the two TX idle voltages, and the GT-U12's back-drive path — and together they are the gate on wiring either module to the Waveshare expansion pads, whose GPIO numbers the same report resolves (§4: `RXD` = GPIO 44, `TXD` = GPIO 43) — the S3 is not 5 V tolerant, so the module's TX idle voltage has to be on a meter before it reaches GPIO 44. The GT-U12 also has no accuracy figure and no cold-start TTFF, `NOT EXECUTED — HARDWARE REQUIRED`, which is what stops the two parts being compared where it matters — the owner's bench |
...
| H21 | **What voltage is `VRTC` actually set to on this unit?** The AXP2101 allows 1.8, 2.5, 3 or 3.3 V, chosen by EFUSE/OTP with no documented user register, and it is the pull-up rail for the PMU IRQ into GPIO21. At 1.8 V the idle level sits in the SoC's indeterminate band; at 3.3 V the question disappears | **UNKNOWN** | bench step **B3** of [TWATCH_RTC_INPUT_WAKE](TWATCH_RTC_INPUT_WAKE.md) §4, opened by [#422](https://github.com/hleserg/Attadipa/issues/422) — measure the net. No document answers this for a specific unit. `NOT EXECUTED — HARDWARE REQUIRED` |
| H23 | **What PCB revision is `DC:B4:D9:18:49:40`, and does the `25-03-24` schematic describe it?** Every net fact in the report is sourced to that drawing, so this is the question the others rest on | **UNKNOWN** | bench step **B0** of [TWATCH_RTC_INPUT_WAKE](TWATCH_RTC_INPUT_WAKE.md) §4, opened by [#422](https://github.com/hleserg/Attadipa/issues/422) — photograph the board marking. `NOT EXECUTED — HARDWARE REQUIRED` |
| H24 | **Does this unit's coin cell hold the PCF8563 oscillator across a main-battery removal?** VL reads as invalid either way, so a watch that loses time on every disconnect is indistinguishable from one whose backup is simply absent — until it is measured | **UNKNOWN** | bench step **B2** of [TWATCH_RTC_INPUT_WAKE](TWATCH_RTC_INPUT_WAKE.md) §4, opened by [#422](https://github.com/hleserg/Attadipa/issues/422). `NOT EXECUTED — HARDWARE REQUIRED` |
...
| R4 | **Does a BLE link to a companion carried on the wearer's own body stay up, and is there a gap duration that separates an RF stumble from the wearer walking away?** [ADR-0019](../adr/0019-confirmed-companion-body.md) decision 6 lapses a confirmed companion on a gap longer than a named constant, which needs both halves: that the on-body link does not stumble through the wearer, and that the stumble range and the separation range do not overlap. Neither is measured; the second is load-bearing, because overlapping ranges mean no constant exists to pick | **UNKNOWN** | one bench run with no instrument: confirm a companion, carry it against the body and read whether the peer is ever lost; then leave it behind, walk out of range and back, and read how long it went unheard. The same run settles whether the watch's own light sleep drops the link. Until it runs, decision 6's gap bullet is a hypothesis and the ADR says so; if the answer is that there is no window, the rule degrades to lapsing on any loss of the peer |
...
| ~~D6~~ | ~~T-Watch: which PMU rail powers GNSS on the *specific* unit (BLDO1 vs DC3)~~ | **RESOLVED 2026-09-05 — BLDO1**, and without needing to look at the case | The rear-button inspection this row proposed was never the only route. With the module answering `UBX-MON-VER` over its UART, the PMU's own `REG 0x80` reads `0x19` — DCDC1, DCDC4 and DCDC5 enabled, **DCDC3 clear**. A rail that is off is powering nothing, so DC3 is excluded by observation rather than by argument. [TWATCH_GNSS_READOFF_2026-09-05](TWATCH_GNSS_READOFF_2026-09-05.md) §3. Two limits are recorded there and belong to the answer: `REG 0x90` bit 4 was **already set before this firmware wrote it** — the register is battery-backed and survives a CPU reset — so what is shown is that BLDO1 is enabled and the module speaks, not that our write is what enabled it; and DC4 being on says nothing about GNSS, because a u-blox needs no 850 mV core and what DC4 feeds here is **UNKNOWN** |
...
| D23 | **Does the `MS412FE` on the GNSS daughterboard reach the `MIA-M10Q`'s `V_BCKP` ball (`J5`), and is there a charge path to it from `VDD3V3`?** `docs/research/HARDWARE_MATRIX.md:244` — "S4 is one sheet: a u-blox `MIA-M10Q`, an IPEX antenna jack, an `MS412FE`" — puts the cell on that sheet, from a commit whose subject is *"Read the schematics I had only been citing"*, so the part is very likely real; **its node was what was open.** The FPC cannot decide it either way: a daughterboard-local backup domain needs no FPC pin, and the `J1` read for [#312](https://github.com/hleserg/Attadipa/issues/312) found no supply crossing but `VDD3V3` | **RESOLVED 2026-09-08 — yes to both halves.** Ball `J5` carries the net `VRTC`; the `MS412FE` (`J2` pin 1) is on it; `VDD3V3` reaches it through `D1` (`1N4148`, anode at the rail) and `R3` (`1K`) | no further action. Read from the vector segments of the LilyGO sheet, not from its text layer — see [VERIFIED_FACTS](VERIFIED_FACTS.md) *"The `MS412FE` does reach `V_BCKP`"*. It buys the 18 µA it was always worth and changes no recommendation: cutting `BLDO1` also cuts the charge path, and the cell's capacity and charge window stay **UNKNOWN** with no `MS412FE` data sheet read |
| H19 | **Is the FT6336U interrupt a pulse or a latch, and what clears it?** It decides whether touch can be a light-sleep wake at all: a latched line nothing clears re-wakes the device immediately, and a pulse can be missed entirely. The vendor also forbids sleeping the touch controller, so it cannot simply be silenced | **UNKNOWN** | the FocalTech FT6336U datasheet (the document named, per §4 of [TWATCH_RTC_INPUT_WAKE](TWATCH_RTC_INPUT_WAKE.md), opened by [#422](https://github.com/hleserg/Attadipa/issues/422)), `G_MODE` register `0xA4` and the interrupt section. A document, not a bench |
| H20 | **What is the BMA423 INT1 output driver mode, active level and latch configuration?** The net has no external pull (S3, sheet 4), so the part's own driver decides whether the line is usable. It is configuration rather than a board fact, which is why the schematic cannot answer it | **UNKNOWN** | the Bosch BMA423 datasheet (the document named, per §4 of [TWATCH_RTC_INPUT_WAKE](TWATCH_RTC_INPUT_WAKE.md), opened by [#422](https://github.com/hleserg/Attadipa/issues/422)), `INT1_IO_CTRL (0x53)` and `INT_LATCH (0x55)`. The SensorLib source is evidence level 2 and not a substitute |
| H22 | **What are the AXP2101 REG 40H and REG 42H power-on IRQ-enable defaults?** They decide whether a freshly reset PMU is already asserting IRQ before any Attadipa code runs, which is the "line already active on entry to sleep" case | **UNKNOWN** | the AXP2101 datasheet V1.4 §6.13 default columns (the document named, per §4 of [TWATCH_RTC_INPUT_WAKE](TWATCH_RTC_INPUT_WAKE.md), opened by [#422](https://github.com/hleserg/Attadipa/issues/422)). A document, not a bench |
...
whose §14 records the message wire. Four things it could not close, and the
...
### What a sender cannot learn about a message it sent

Opened 2026-09-14 by [#552](https://github.com/hleserg/Attadipa/issues/552). The
mechanism is closed and read from source at the pin —
[OUTBOUND_MESHCORE_MESSAGES](OUTBOUND_MESHCORE_MESSAGES.md) — and the decision
it produced is [ADR-0023](../adr/0023-unconfirmed-is-not-failed.md). What is
open is not what the protocol does. It is what happens on a radio.

| # | Question | Status | Resolved by |
|---|---|---|---|
| M36 | **Why did the T114's Room send never confirm, and never arrive, in 120 s?** One occurrence, §6b of [MESHCORE_T114_FIRST_CONTACT](MESHCORE_T114_FIRST_CONTACT.md), and it was a send to the "Beta Room" Room Server behind a `CMD_SEND_LOGIN`, not a private send: `RESP_CODE_SENT` arrived, nothing followed, and a person reading the room saw no message. The node cannot have said more — its send-timeout hook is an empty function body — so no amount of reading closes this, and there is no air capture. **What is open is the cause, not the outcome**: the non-delivery is `MEASURED` on the receiving end, so this case is not an argument that `Failed` would have been wrong here. It is an argument that the *client* could not have known, which is the claim [ADR-0023](../adr/0023-unconfirmed-is-not-failed.md) actually rests on | **UNKNOWN** | two pin-matched nodes, a sniffer or a second receiver, and the same send repeated enough times to say whether it is a path, a range or a one-off. `NOT EXECUTED — HARDWARE REQUIRED` |
| M37 | **Can the node's own suggested timeout exceed this client's 15-second clamp?** `est_timeout` is `500 + 16.0 × airtime` flooded and `500 + (6.0 × airtime + 250) × (hops + 1)` direct, so it grows with payload and hops; the one value observed is 2406 ms for a short text whose hop count the frame does not carry and is `UNKNOWN`. Arithmetic says a three-hop direct send passes 15 000 ms at about 563 ms of airtime. Whether a 128-byte MeshCore packet ever takes that long on this fleet's radio configuration is the missing term — and where it does, the watch publishes a verdict while the node is still legitimately waiting | **UNKNOWN**; the formulas are `VERIFIED` from source and the airtime is not | a bench send to a multi-hop recipient with a full-length body, logging the `est_timeout` the node returns. Costs one send and no write. `NOT EXECUTED — HARDWARE REQUIRED` |
| M38 | **Is a confirmation that arrives while the app is disconnected really lost?** `processAck` writes the push with no connection guard and the BLE transport drops everything while the link is down; the offline queue is reached from `queueMessage` and not from there. So source says the proof of delivery is discarded, while the node's own ack-table entry survives — meaning the *same* message confirms if the ack is slower than the reconnect and never confirms if it is faster. Nobody has disconnected mid-flight and watched | **UNKNOWN**, mechanism `VERIFIED` from source | a bench send, a deliberate disconnect inside the ack window, and a reconnect — run twice, with the disconnect on either side of the expected round trip. `NOT EXECUTED — HARDWARE REQUIRED` |
| M39 | **Is the `< 0x80` split between response codes and unsolicited pushes a documented upstream guarantee, or only the current allocation?** At `0d7ba547` responses occupy 0–31 and pushes `0x80`–`0x90`, both on the same wire with no other distinction, so the split has to hold or no client could tell an answer from an announcement. Upstream writes it down nowhere — `docs/companion_protocol.md` prints one table with both ranges in it and no rule. [ADR-0024](../adr/0024-an-unknown-answer-is-not-a-malformed-one.md) uses it to decide whether an unrecognised frame could be an answer at all | **UNKNOWN**; the allocation is `VERIFIED` from source, the rule is not | a maintainer's statement, or a protocol document that says it. Until then the conservative direction is taken: an unknown code at `0x80` or above advances nothing, so a wrong inference costs a stall and never a spin |
| M40 | **Does a node ever re-announce a message it has already announced?** Open since [#481](https://github.com/hleserg/Attadipa/pull/481) and still open. Narrowed at `0d7ba547`: `PUSH_CODE_MSG_WAITING` is emitted from the five `queue*` functions, once per frame queued, and only `if (_serial->isConnected())` — so a queue that filled while the app was away announces itself to nobody, and nothing pushes on connect. What is still unestablished is whether any path repeats an announcement for a frame still sitting in the queue | **UNKNOWN**, mechanism `VERIFIED` from source | a bench node with a non-empty queue, a client that deliberately declines to drain it, and an hour of watching for a second `0x83`. `NOT EXECUTED — HARDWARE REQUIRED` |
...
| Q5 | ~~How does a power lease taken on one task take part in a sleep decision made on another?~~ **What refuses a sleep already under way when the declaration lands after the sleeper read its snapshot?** | **half answered 2026-09-03** | The half that was asked is decided: `docs/adr/0016-one-power-owner.md:80` — "rather than the window. **A consumer that does not run on the owner's task" — such a consumer does not `acquire()` at all; it publishes a state the owner's task already reads, and that task records the declaration immediately before `sleep()`. [#367](https://github.com/hleserg/Attadipa/issues/367) item 7 shipped the first one. The second half is the declaration window that pattern **moves rather than closes**, recorded in the file that would have to change: `core/include/attadipa/core/power_owner.h:379` — "// does not yet have and which nothing in the current firmware needs, because". Not a blocker either way: consequence stays zero until a plan suspends a domain a cross-task lease holds |
...
- **Row A prices its gesture as already paid, and that is true of one of the two
  ...
  **UNKNOWN** — `docs/testing/WATCH_CONTROL.md:150` — "so on a device a held power key may be a shutdown rather than an event".
...
- **The clock could not be set.** `write_rtc()` was reached only through
  ...
  opens (`firmware/main/waveshare_board.cpp:1091` — "void long_press(lv_event_t *) {").
...
- **MeshCore never scanned.** `configure_meshcore_ble()` is the only writer of
  ...
  (`firmware/main/waveshare_board.cpp:1135` —
  ...
  `firmware/main/meshcore_ble.cpp:1435` — "if (configured.load()) start_scan();"
...
- **A changed node cannot be recovered from, and this bullet understated it.**
  ...
  `firmware/main/meshcore_ble.cpp:561` — "nvs_set_blob(handle, kNodeKeyNvsKey"
...
([`core/include/attadipa/core/power_owner.h:359`](../../core/include/attadipa/core/power_owner.h) —
...
(`core/include/attadipa/core/power_owner.h:368` — "// That decision is made, in ADR-0016: such a consumer does not call `acquire()`").
...
nothing else
([`firmware/main/physical_input.cpp:236`](../../firmware/main/physical_input.cpp) —
