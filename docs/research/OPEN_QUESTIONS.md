...
| H14 | **Which QMI8658 variant is on the Waveshare — `C`, or `A` at Rev D?** `QMI8658C` has a hardware pedometer; the `QMI8658A` Rev A datasheet documents one and **Rev D deletes the chapter**. The board is recorded only as "QMI8658 / QMI8658C" and the schematic prints no revision. This is [ADR-0003](../adr/0003-radio-not-lora.md)'s shape in a second subsystem: the part name does not tell you whether the feature exists, and OD-6 makes the pedometer mandatory | **UNKNOWN** | read `WHO_AM_I` (`0x00`) and the revision register on a powered board — §5 step 6 of [WAVESHARE_ARRIVAL.md](WAVESHARE_ARRIVAL.md). Blocks T-061's Waveshare half; if the answer is Rev D, the step counter is firmware there and the power budget changes |
| H15 | **What is the IMU's axis orientation relative to the wearer, on each board?** A step counter tolerates a wrong sign; a wrist-raise gesture and any future orientation feature do not. Neither schematic records how the part is rotated in the case, and [PEDOMETER_PARTS.md](PEDOMETER_PARTS.md) §1.9 notes that on the BMA423 axis remapping applies to the *features* only, so getting it wrong is silent rather than obviously broken | **UNKNOWN** | tilt the board through known angles and read raw axes. Cheap once a board is on the desk, impossible before |
