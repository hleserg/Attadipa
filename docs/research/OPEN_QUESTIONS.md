...
- **The clock could not be set.** `write_rtc()` was reached only through
  ...
  opens (`firmware/main/waveshare_board.cpp:903` — "void long_press(lv_event_t *) {").
...
- **MeshCore never scanned.** `configure_meshcore_ble()` is the only writer of
  ...
  `firmware/main/meshcore_ble.cpp:1210` — "if (configured.load()) start_scan();"
...
- **A changed node cannot be recovered from, and this bullet understated it.**
  ...
  `firmware/main/meshcore_ble.cpp:391` — "nvs_set_blob(handle, kNodeKeyNvsKey"
