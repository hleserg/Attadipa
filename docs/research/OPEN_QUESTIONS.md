...
- **The clock could not be set.** `write_rtc()` was reached only through
  ...
  opens (`firmware/main/waveshare_board.cpp:1044` — "void long_press(lv_event_t *) {").
...
- **MeshCore never scanned.** `configure_meshcore_ble()` is the only writer of
  ...
  (`firmware/main/waveshare_board.cpp:1082` —
  "  state.entry.emplace(provisioner, attadipa::apps::EntryTask::All, seed);"):
  `firmware/main/waveshare_board.cpp:499` — "set_mesh_passkey(std::uint32_t passkey) override {".
