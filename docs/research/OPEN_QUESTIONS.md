...
| D23 | **Does the `MS412FE` on the GNSS daughterboard reach the `MIA-M10Q`'s `V_BCKP` ball (`J5`), and is there a charge path to it from `VDD3V3`?** `docs/research/HARDWARE_MATRIX.md:244` — "S4 is one sheet: a u-blox `MIA-M10Q`, an IPEX antenna jack, an `MS412FE`" — puts the cell on that sheet, from a commit whose subject is *"Read the schematics I had only been citing"*, so the part is very likely real; **its node is what is open.** The FPC cannot decide it either way: a daughterboard-local backup domain needs no FPC pin, and the `J1` read for [#312](https://github.com/hleserg/Attadipa/issues/312) found no supply crossing but `VDD3V3` | **UNKNOWN**, and **no longer on any critical path** | render sheet S4 — `Xinyuan-LilyGO/LilyGoLib`, `schematic/T-Watch-S3-Plus-GPS V1.0 2025-04-29.pdf`, one sheet — as an image and read whether the cell's node reaches ball `J5`. The PDF text layer drops every net, so an extraction proves nothing. **Worth 18 µA on the module** plus unquantified board-side terms: with `BLDO1` up the backup domain runs off `V_IO` and this answer changes nothing, which is why [GNSS_POWER_POLICY_MIA_M10Q](GNSS_POWER_POLICY_MIA_M10Q.md) could be written without it |
