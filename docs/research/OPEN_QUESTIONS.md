...
| D19 | **Is the Waveshare PCF85063's interrupt routed to a GPIO at all?** [HARDWARE_MATRIX](HARDWARE_MATRIX.md):332 gives the part, the bus, `0x51` and the rail, and **no `INT`** — while the T-Watch's PCF8563 row (`:98`) records `INT 17` as VERIFIED. Asymmetries like that are usually a gap in the reading, not in the board. It decides whether a Clock's minute boundary can be a hardware alarm on this board or must stay a software timer the SoC has to be awake for — [CLOCK_STATE_AND_CADENCE](CLOCK_STATE_AND_CADENCE.md) §4 | **UNKNOWN** | read the schematic's RTC block visually, as D3 and D13 were. Then, separately, whether the pin it lands on is usable as a wake source in each `PowerState` — which is an ESP32-S3 RTC-GPIO question and has not been traced to the datasheet by anyone yet, on **either** board |
...
| Q4 | **Is the clock 12-hour or 24-hour, and is it a user setting at all?** | UNKNOWN | a product decision. Final §88 lists *time, date, battery* and does not say. It is not free either way: InfiniTime shows the same setting also **reordering the date** (`WatchFaceDigital.cpp:134-148`), and the 12-hour string is half again as wide — `10:00 AM` is 129 px at 28 px against `00:00`'s 82 px ([CLOCK_STATE_AND_CADENCE](CLOCK_STATE_AND_CADENCE.md) §7.3) |
| Q5 | **Which date format, per locale — long, medium or short, with the weekday or without?** | UNKNOWN | a design decision that the measurements bound rather than make: at 28 px **no long date form fits the 240 px face**, and which language is wider *flips* depending on whether the weekday is in the format (§7.2). Deciding it is choosing what the T-Watch shows, not choosing a string |
| Q6 | **Does Attadipa model time zones and DST, or carry a single offset?** | UNKNOWN | nothing in `core/` models a zone today. A watch that only ever displays local time can live on an offset; one that receives a timestamp from an Attadipa node or a phone cannot avoid the question, and the answer changes what `Timed<WallTime>` has to carry |
| Q7 | **Do the Russian catalogues need nominative month names as well as the genitive ones?** | UNKNOWN | an [ADR-0010](../adr/0010-localization.md) question. A date needs "30 **сентября**"; a month picker or a header would need "сентябрь". Two forms is two sets of strings and a rule about which is used where; one form is a grammatical error somewhere |
