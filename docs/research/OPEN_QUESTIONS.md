...
- **The clock could not be set.** `write_rtc()` was reached only through
  ...
  opens (`firmware/main/waveshare_board.cpp:1072` — "void long_press(lv_event_t *) {").
...
- **MeshCore never scanned.** `configure_meshcore_ble()` is the only writer of
  ...
  (`firmware/main/waveshare_board.cpp:1116` —
  ...
  `firmware/main/meshcore_ble.cpp:1421` — "if (configured.load()) start_scan();"
...
- **A changed node cannot be recovered from, and this bullet understated it.**
  ...
  `firmware/main/meshcore_ble.cpp:551` — "nvs_set_blob(handle, kNodeKeyNvsKey"
