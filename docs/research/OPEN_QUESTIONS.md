...
| D15 | **What is the T-Watch panel's physical diagonal — 1.3" or 1.54"?** 240 × 240 is certain; the size is not. LilyGoLib's spec tables say 1.3 Inch for the S3 *and* the S3 Plus; the schematic's LCD sheet says `QT154C2408` / `LCD_1.54-TOUCH`, and that vendor's sibling part `QT154H2201` is specified as 1.54", 240×240, ST7789V — so the `154` field decodes. It decides dpi (261 vs 220) and therefore every physical-size conversion in the design system | **CONFLICTING** | **one flat-on photograph of the running watch — asked as [#311](https://github.com/hleserg/Attadipa/issues/311)**. No ruler is needed: the active square is 23.35 mm on a 1.3" panel and 27.66 mm on a 1.54" one, which against the 42 mm case is 0.556 against 0.659 of its width — a 19 % difference that any in-frame comparison settles. The `QT154C2408` specification is not published anywhere reachable, and the vendor store page for the S3 Plus states **no display specification at all** — that silence is an established property of the sources, not an unfinished search. Working value stays 1.3", the conservative direction — [HARDWARE_MATRIX](HARDWARE_MATRIX.md#display-diagonal--conflicting) |
