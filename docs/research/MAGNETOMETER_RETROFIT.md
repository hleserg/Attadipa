...
> **Status:** research baseline, 2026-08-22; AK09911 source and raw-acquisition
> update, 2026-09-09 (§2.1). The parts arrived 2026-09-05
> ([BENCH_DEVICES](BENCH_DEVICES.md)). Source facts are `VERIFIED`; the later
> Waveshare ID/XYZ capture is `MEASURED` with its limits below. The proposed
...
The AK09911C has a real `RSTN` input on the `VID` domain. M1 specifies it as a
CMOS input, reset-active low, with a 5 µs minimum low pulse; its recommended
connection drives it from a host GPIO and says that an unused reset input is
...
fact, so keep it unwired rather than inferring a function from the silkscreen.

The Waveshare V1.0 schematic establishes one 2.2 kΩ pull-up per main I²C line:
`R23` from `VCC3V3` to `ESP32_SCL`/GPIO14 and `R49` from `VCC3V3` to
`ESP32_SDA`/GPIO15. `C34` and `C35` add 22 pF from those respective lines to
ground. The resistor/net association was checked both by a high-resolution
render and by the PDF vector paths; this is a schematic fact, not a board
measurement.

At `VOL = 0.4 V`, the board alone sinks 1.318 mA at 3.3 V. With a candidate
...
Sink current is not the only constraint. The fixed 2.2 kΩ allows about 161 pF
at 400 kHz before the Fast-mode rise-time limit, while the board already has
the two 22 pF capacitors plus device, trace and flex capacitance. The present
bus work ran at 100 kHz; choosing 400 kHz requires a measured rise time rather
than an extrapolation. All module pull-up values and total bus capacitance remain
`UNKNOWN` pending hardware measurement.
...
of ours: `docs/research/REUSE_LEDGER.md:779` — "an external magnetometer is decided"
