...
|---|---|---|
| QMI8658A Datasheet, `13-52-25`, **Rev A** | 20 June 2022 | **Present**, with a caveat this row must carry. Feature list p. 1; chapter 11 "Pedometer" pp. 64–66; `STEP_CNT_LOW/MIDL/HIGH` at `0x5A`–`0x5C`; `CTRL8.Pedo_EN`; both CTRL9 commands. **Every one of those is also true of `13-52-27`, page numbers included** — its chapter 11 opens on p. 64 and §11.6 is on p. 66 — so nothing recorded here distinguishes which of the two Rev A documents was open. The attribution is `UNKNOWN` until somebody opens `13-52-25` again; the pedometer's presence is not in doubt either way. [#341](https://github.com/hleserg/Attadipa/issues/341) |
