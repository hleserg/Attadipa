...
- `{0x80}` alone and `{0x8A}` alone ⇒ counted malformed; no key compared, no
  ...
  stack arrays — `tests/test_meshcore_companion.cpp:891` — "    const std::uint8_t short_contact[] = {3};" — so an arm that trusts its length over-reads that array by 32 bytes and by 143;
