...
`link/src/meshcore_companion.cpp:298` — "    if (size < 148 || data[33] != kAdvertTypeChat) {" —
...
- **a 6-byte prefix.** `find_peer_prefix` already matches on six bytes for
  message attribution — `link/src/meshcore_companion.cpp:326` — "        if (std::memcmp(peers_[i].id.public_key.data(), prefix, 6) == 0) {" —
...
MeshCore node on the host — `BENCH_DEVICES.md:114` — "is not ours to write to.** It is a MeshCore node somebody".
