...
`link/src/meshcore_companion.cpp:1307` — "        if (size < 148) { ++malformed_frames_; return false; }".
...
- **a 6-byte prefix.** `find_peer_prefix` already matches on six bytes for
  message attribution — `link/src/meshcore_companion.cpp:632` — "        if (std::memcmp(peers_[i].id.public_key.data(), prefix, 6) == 0) {" —
...
`link/src/meshcore_companion.cpp:952` — "        copy_text(status_.last_message, &data[text], size - text);" —
