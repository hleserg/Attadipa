...
`link/src/meshcore_companion.cpp:677` — "    if (size < 148 || data[33] != kAdvertTypeChat) {" —
...
- **a 6-byte prefix.** `find_peer_prefix` already matches on six bytes for
  message attribution — `link/src/meshcore_companion.cpp:720` — "        if (std::memcmp(peers_[i].id.public_key.data(), prefix, 6) == 0) {" —
...
`link/src/meshcore_companion.cpp:1072` — "        copy_text(status_.last_message, &data[text], size - text);" —
