...
`link/src/meshcore_companion.cpp:685` — "    if (size < 148 || data[33] != kAdvertTypeChat) {" —
...
- **a 6-byte prefix.** `find_peer_prefix` already matches on six bytes for
  message attribution — `link/src/meshcore_companion.cpp:728` — "        if (std::memcmp(peers_[i].id.public_key.data(), prefix, 6) == 0) {" —
...
| A `0x80` shorter than **33 bytes** is refused and counted malformed, in the new arm's **own** guard | `0x80` is `[0x80][pub_key×32]` (§7). The dispatcher owns no shared bound — `link/src/meshcore_companion.cpp:1305` — "size > kMeshCoreFrameBytes" — rejects only an empty or over-long frame, and **no** arm after it inherits a bound — each one that reads a fixed-size field carries its own check. An arm that inherits a guard it does not have reads 32 bytes off the end of a one-byte frame |
| A `0x8A` shorter than **148 bytes** is refused and counted malformed, in the new arm's **own** guard | It is §3.1's contact layout under a different opcode, and the coordinate is at its far end, bytes 136–143. The 148-byte guard that exists today is one `case` arm and covers `RESP_CODE_CONTACT` alone — `link/src/meshcore_companion.cpp:1437` — "        if (size < 148) { ++malformed_frames_; return false; }". A new opcode does not inherit it |
| A `0x8F` shorter than **33 bytes** is refused and counted malformed, in the new arm's **own** guard | It carries `0x80`'s `[opcode][pub_key×32]` shape and is compared against the target key the same way, so it over-reads a one-byte frame by the same 32 bytes. `0x8F` has an arm at head and so do `0x80`, `0x8A` and `0x90` — `link/src/meshcore_companion.cpp:1569` — "    case kPushAdvert:" — added by ADR-0022 decision 4 so that a push this build understands is not counted as a parse failure. So this was an arm to widen rather than one to add, and it handed its widener no bound at all. #650 widened it and gave `0x8F` its own guard; the other three still read no byte past the opcode and still carry none |
...
| The answer to `CMD_GET_CONTACT_BY_KEY`, `RESP_CODE_CONTACT` or `RESP_CODE_ERR` alike, is attributed to **that** command by the sequence this session already stamps, and is **never charged to the messaging operation** | A `RESP_CODE_ERR` is two bytes and carries nothing saying which command it answers, so attribution here is by order and not by content: each asynchronous claimant stamps `tx_seq_` as it sends — `link/src/meshcore_companion.cpp:790` — "            custom_vars_seq_ = tx_seq_;" — and the error goes to the oldest claimant still owed one — `link/src/meshcore_companion.cpp:1887` — "custom_vars_seq_ < op_seq_". That ladder has **three** claimants at head — the receiver hint, the snapshot re-read of ADR-0022 decision 6, and the send — and everything that does not enter it falls to the send — `link/src/meshcore_companion.cpp:1904` — "        if (send_busy()) {". Each claimant added to it has to be added to every earlier claimant's guard as well, which is what `(!retry_armed_ || custom_vars_seq_ < contacts_seq_)` is doing one line above that. A fourth claimant left outside it fails a message the node **accepted**: `send_busy()` stays true through `awaiting_confirm_`, where what is outstanding is a radio round trip rather than a response, and the arm's own comment records this exact defect being found and fixed once already for opcode 40 |
...
- `{0x80}` alone and `{0x8A}` alone ⇒ counted malformed; no key compared, no
  ...
  stack arrays — `tests/test_meshcore_companion.cpp:973` — "    const std::uint8_t short_contact[] = {3};" — so an arm that trusts its length over-reads that array by 32 bytes and by 143;
...
text types and none of them is a location record — `MESHCORE_COMPANION_PROTOCOL.md:689` — "`src/helpers/TxtDataHelpers.h:6-8` defines exactly three: `TXT_TYPE_PLAIN`" —
...
| placement | last thing in the message | do not require it: take the **last** match, so a quoted older message cannot win — and a last match that goes wrong must clear the answer rather than fall back to the earlier one, whether it failed a bound or the grammar. A coordinate that is not last is also the one likeliest to carry sentence punctuation, so a full stop after the decimals ends the number and only a further digit group refuses the shape |
...
`link/src/meshcore_companion.cpp:96` — "    const std::size_t copy = std::min(size, N - 1);".
...
is `kMeshTextBytes + 1`, so 128 of 131 survive and three characters of longitude
do not. That is not `(0, 0)`, it is inside ±90 and inside ±180, so every value
refusal passes it and "drop rather than clamp" never fires — and the target
lands about **650 m** away.

**131 is the longest text whose remainder still parses**, and the arithmetic is
worth spelling out because an earlier revision of this section got it wrong by
one and built a worked example on top of the error. One byte more leaves
`@55.9821,37.`, one more than that leaves `@55.9821,37`, and §14.2's grammar
refuses both for carrying no decimal place — so the 13 km figure this paragraph
used to quote was never reachable through this guard at all. 650 m is also not
the worst the cut costs, only what *this* coordinate costs: a survivor cut back
to a single decimal is under 0.1° of longitude wrong, about 6 km at this
latitude and about 11 km at the equator. The guard is sized for the class, not
for the example.

**Unlike the sender's half, ours is already flagged.** The copy reports it —
`link/src/meshcore_companion.cpp:1085` — "        copy_text(status_.last_message, &data[text], size - text);" —
