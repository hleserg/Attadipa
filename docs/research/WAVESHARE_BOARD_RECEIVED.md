...
| IMU | `IMG_20260908_223653.jpg`: `8658`, `6D111`, `KB`; printed axis triad as §1.6 | QMI8658-family marking corroborated. The absent `C` suffix settles nothing, and reading it as evidence is the exact inference H14 rejects: `docs/research/VERIFIED_FACTS.md:2200` — "the part name on the schematic (`QMI8658C`, printed twice)" — did not predict the register map, and `REVISION_ID = 0x7C` did. H14 is `docs/research/OPEN_QUESTIONS.md:90` — "**RESOLVED** (was CONFLICTING)" — since 2026-08-23, sixteen days before these photographs, and both halves are closed; what it leaves is which of the two Rev A papers applies, which is neither a variant nor a function question and which no register can decide. H15's case rotation remains open |
