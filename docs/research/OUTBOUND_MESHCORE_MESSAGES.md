...
`docs/research/MESHCORE_COMPANION_PROTOCOL.md:798` — "Whether the first-party JS and Python clients agree with this reading"
...
`link/src/meshcore_companion.cpp:1701` — "std::memcmp(&data[1], expected_ack_.data(), expected_ack_.size()) != 0"
...
ran in `reset_session()`. `None` renders as *"not sent"* / *"не отправлено"*.
[#573](https://github.com/hleserg/Attadipa/issues/573) replaced it with the
`Unknown` of §4.2, and only for a request that was still in flight.
...
- **Full 32-byte identity at the app/core boundary.** The six-byte prefix is
  ...
  application holds — `link/src/meshcore_companion.cpp:2187` — "std::memcpy(&frame[7], peer.public_key.data(), kPeerPrefixBytes);"
...
`link/src/meshcore_companion.cpp:1767` — "        if (!enqueue_private(room_peer_, std::string_view(room_text_.data()),".
...
`link/src/meshcore_companion.cpp:383` — "    if (!send_busy()) {" — which is a
...
room path it lands on `Queued`, where `Refused` says what it is from the
owner's side: nothing reached the radio, and a resend cannot duplicate anything.

**An earlier draft of this paragraph said the second producer lands after
`Accepted`, and named a defect underneath it — that `RESP_CODE_SENT` on the room
...
gated on the phase — `link/src/meshcore_companion.cpp:1636` — "if (awaiting_send_) {" — and a
...
`expected_ack_`. `test_an_answered_login_does_not_take_a_later_opcode_40s_error`
now asserts the state is `Queued` there rather than "not the failure state",
which pins it. What remains true is the sentence this replaces: only the text's
own `RESP_CODE_SENT` means `Accepted`, and it already did.
...
`docs/research/MESHCORE_COMPANION_PROTOCOL.md:730` — "This is an enumerated absence: all three definitions in" says so
...
lookup from the worker and made the provider ask the node — `firmware/main/meshcore_ble.cpp:1486` — "    const auto result = service.send_private("
...
the frame has:
`link/src/meshcore_companion.cpp:1455` — "        if (awaiting_contact_ &&". Hazard 1
...
`link/src/meshcore_companion.cpp:2089` — "    if (data[33] != kAdvertTypeChat) {". Hazard
...
the call; the re-read leaves from `tick()`, so without a term of its own it
could open a walk *after* a fetch was already outstanding and reach the same
frame from the other direction. It now stands down while one is —
`link/src/meshcore_companion.cpp:462` — "        !retry_unanswered_ && !wrong_node_ && !awaiting_contact_ &&".
...
situation it describes is unreachable — and a test that passes without
exercising anything is worse than no test. What replaces it is a row per
exclusion, in both orders, plus one for the tie.
...
without them can pass while the product lies. Rows 19 to 26 were added when §8
was implemented, and rows 23 to 25 are hazard 3 from both ends. Row 23 is the
*call* refusal, for a send that arrives while a walk is already outstanding; row
24 is the same pair in the other order at the re-read, which stands down; row 25
is the order a call refusal cannot cover at all, a walk beginning while a fetch
is outstanding and streaming the fetched key — §8.2. **So hazard 3 is handled at
