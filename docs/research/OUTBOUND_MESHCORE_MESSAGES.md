...
`link/src/meshcore_companion.cpp:1648` — "std::memcmp(&data[1], expected_ack_.data(), expected_ack_.size()) != 0"
...
- **Full 32-byte identity at the app/core boundary.** The six-byte prefix is
  ...
  application holds — `link/src/meshcore_companion.cpp:2128` — "std::memcpy(&frame[7], peer.public_key.data(), kPeerPrefixBytes);"
...
`link/src/meshcore_companion.cpp:1714` — "        if (!enqueue_private(room_peer_, std::string_view(room_text_.data()),".
...
gated on the phase — `link/src/meshcore_companion.cpp:1583` — "if (awaiting_send_) {" — and a
...
lookup from the worker and made the provider ask the node — `firmware/main/meshcore_ble.cpp:1437` — "    const auto result = service.send_private("
...
the frame has:
`link/src/meshcore_companion.cpp:1456` — "        if (awaiting_contact_ &&". Hazard 1
...
`link/src/meshcore_companion.cpp:2030` — "    if (data[33] != kAdvertTypeChat) {". Hazard
