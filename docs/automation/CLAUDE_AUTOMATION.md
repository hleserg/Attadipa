...
`docs/research/DEPENDENCIES.md:112` — "`56cf60fde42f7b19c3abfd5c9c48b69a1288461f`, 2026-09-11" —
...
| **Privileged actions run at a commit, not at a movable name** — every `uses:` in `.github/workflows/` is a 40-hex commit with the tag it came from as a trailing comment. A tag is a name its owner can move; `anthropics/claude-code-action@v1` resolved to a different commit on three consecutive days while this was being prepared, and the code it resolves to receives the Anthropic credential, a GitHub token context, OIDC and repository write permissions — so a move changes what executes here with no change to any workflow, pull request, review or required check. **No upstream compromise is claimed or observed**; the execution path is the finding. Two of the six are *annotated* tags, whose `git/ref/tags/<tag>` answer is a tag object rather than a commit — a SHA that looks identical and that GitHub will not check out, which is why the test resolves each pin as a commit rather than checking its shape | `.github/tests/action-pin-test.sh`, `docs/research/DEPENDENCIES.md`, issue [#294](https://github.com/hleserg/Attadipa/issues/294) |
| **The queue has a bounded width before work starts** — below the width admits a writer, at the width is `full`, and above it is `incident`. The width is `ATTADIPA_WIP_LIMIT` and **defaults to two**, so the shipped behaviour is unchanged: one active PR admits, two is `full`, three is `incident`. The owner raises or restores it in repository settings and the workflows pass it in, because the alternative was editing a shipped script and merging a pull request through the gate being lifted. Anything that is not a one- or two-digit number — unset, empty, `unlimited`, a negative, twenty digits — falls back to two rather than to no limit; the variable is not a way to unbound the queue by accident. Active means an open same-repository PR without `queue:parked` or `queue:emergency`; forks do not spend a slot. `writer-admission.sh` runs before dispatch and again inside the globally leased writer job, so two gate runs cannot both consume the last slot. `unknown` from any API, CLI, JSON or schema failure refuses admission. At `incident`, only an existing PR or explicitly labelled `queue:emergency` recovery can run; ordinary research/follow-up/refactor work stops | `writer-admission.sh`, `writer-start.sh`, `wip-limit.sh`, `wip-limit-test.sh`, `claude-agent.yml`, `agent-queue-watchdog.yml`, issue [#354](https://github.com/hleserg/Attadipa/issues/354) |
...
| **One writer, including local agents and CI repair** — the Actions concurrency group limits workflow writers, while an atomic `refs/tags/attadipa-claims/writer` compare-and-set lease also covers local Claude/Codex. The lease is acquired before the second admission check and released after hand-over. A queued run therefore recounts after the previous run may have opened a PR | `claim.sh`, `writer-start.sh`, `claude-agent.yml`, `claude-ci-repair.yml` |
| **One issue, one writer** — `agent:working` is only the visible mirror. Ownership is an atomic GitHub ref `refs/tags/attadipa-claims/<number>` carrying the holder's agent id; a loser re-reads the winner and exits `held` before checkout. Only that id can release it. The id is world-readable in the tag, so `claim.sh` refuses credential-shaped holders before it makes any network call. The tag also records where the claim came from — `kind=hosted` with `run` and `attempt` when the acquire ran on the workflow path, `kind=local` otherwise — and the watchdog reaps a two-hour-old ref only when *that recorded run* has completed. Provenance is never inferred from how the holder id is spelled, so a local lease named like a run is not reaped out from under a live writer and a hosted run is recoverable whatever its holder is called; local, unattributed and unreadable all fail closed. It uses the label event for pre-ref legacy claims. A DELETE that was refused is reported as a failure, never as a cleared claim | `claim.sh`, `agent-queue-watchdog.yml`, `admission-claim-test.sh` |
...
| **A refused start is not a refused request** — the two are separate answers because they were once the same silence. Admission ran *before* intake, so a `full` queue skipped `Decide`, which wrote neither an acceptance nor a refusal, so every downstream job was skipped and the run finished green in ten seconds having said nothing: run 33048001621 lost the owner's verified NVS/RTC finding on [#264](https://github.com/hleserg/Attadipa/issues/264) that way. Intake now decides for every eligible event and admission gates only the writer. A deferred request gets one receipt naming the admission state and the count, and `agent:ready` so the hourly watchdog brings it back — except where that label would be inert (`agent:working`, `agent:review`, `agent:blocked` are dropped by `queue-scan.jq` and refused by `intake-decision.sh` on dispatch), where the comment names the comment path instead. Nothing is claimed and no `agent:working` appears: the task was not started | `claude-agent.yml` (`Decide`, `Say it is deferred`), `agent-say.sh`, `.github/tests/deferred-admission-test.sh`, issue [#293](https://github.com/hleserg/Attadipa/issues/293) |
| **Turn ceilings are post-hoc assertions, not spend controls** — 200 for implementation, 100 for review, 40 for repair; job timeouts are the actual wall-clock bound. If the reviewer publishes a verdict and the action then reports `failure` for crossing its ceiling, that published verdict is retained with a workflow warning instead of being contradicted as a review that never ran | `claude_args: --max-turns`, `review-published.sh`, `review-published-test.sh` |
| **Model and effort are pinned, not defaulted** — every automated Claude role uses `claude-opus-5` at `--effort xhigh`; `max` is reserved for an explicit owner-requested exceptional manual run. Lower-effort subordinate agents remain configured independently. The action has **no `model:` input**, so the model is a string inside `claude_args` and its absence silently falls back to the CLI default. Flags read off `claude --help`: `--model` takes an alias or a full name, `--effort` takes one of `low, medium, high, xhigh, max`. The full name is pinned rather than the `opus` alias so a new Opus cannot silently change what this loop is | `claude_args: --model`, `--effort`, `.github/tests/bot-actor-test.sh` |
| **Nothing merges itself on a green tick alone** — the sweep reads `ai-review:pass` as a *condition*, so a pull request the reviewer never looked at is never merged, and one it blocked never is either. GitHub's own auto-merge cannot do this: it reads required checks, and `ai-review:blocking` is a label on purpose, because the reviewer's verdict is a judgement while a red tick is a fact. Arming native auto-merge would merge precisely the pull requests the reviewer stopped | `merge-candidate.sh`, `merge-candidate-test.sh` |
| **A commit status is not a check run** — GitHub returns both in one `statusCheckRollup`, and the sweep used to flatten them, so a third-party app's green commit status counted toward *"some check ran on this head"*. The app here is Devin, whose status on every head commit reads *"Devin Review · success · Full review skipped: trial expired and no credits remaining"* — and on a head whose own workflows were all still `action_required` it was the **only** context present, so the combined state read `success` over a pull request nothing had looked at. Statuses now arrive tagged `status:`: a red or pending one still refuses, because a red third party is information; a green one can no longer stand in for CI having run. Removing the app outright is a repository-settings act and the owner's own | `merge-candidate.sh`, `pr-merge-sweep.sh` |
...
| **The other reviewer's findings are answered by somebody entitled to, about that finding, about this tree** — Codex is configured on ChatGPT's side, sets no label, and reaches us only as a comment, so *"is one of its findings outstanding"* is the sweep's own question to answer. It answered it three wrong ways in succession, each narrower than the last and each still a bypass. *Any later comment* closed a finding — including this repository's own outcome and review comments. Then *any later non-bot comment*, which is a **classification** doing the work of an **authorisation** on a public repository: a stranger typing `ok` cleared it ([#130](https://github.com/hleserg/Attadipa/issues/130)). Then a later comment by a `write\|maintain\|admin` collaborator that either replied in the finding's own review thread or carried a written acknowledgement — better, and still bound to the head by a **timestamp**, which is not an identity: move a pull request onto a commit that already existed and the head's stamp goes *backwards*, so an acknowledgement about the previous tree clears a finding on this one. The binding is now the head's **object id**. A reply answers only where GitHub records both the finding and the reply as written against the commit that is head now; an acknowledgement has to name it, as a line of its own reading `attadipa: codex-reviewed <40-hex oid>` — and it is recognised by what **survives** markdown rather than by what a stripper removes, because five forms that show a reader the words without saying them (a ``double-backtick`` span, four spaces of indent, an HTML comment, a `> ` quotation, the words mid-sentence) all counted as saying them. Anything unreadable — a permission, a timestamp, an object id — prints `unknown` and holds | `codex-answered.sh`, `codex-answered-test.sh`, `merge-candidate.sh` |
...
| **A page is not a set.** Every merge fact arrives in a GraphQL connection, so the shipping sweep calls `merge-facts.sh` and refuses any snapshot whose `pageInfo` cannot prove completeness. Head age and review freshness come from `merge-head-trust.sh`, bound to the current object id rather than an author-controlled commit timestamp | `merge-facts.graphql`, `merge-facts.sh`, `merge-head-trust.sh`, `pr-merge-sweep.sh`, issue [#170](https://github.com/hleserg/Attadipa/issues/170) |
| **Three merges per run** — the backstop's cap, for its reason: *"a backstop that empties the queue in one go is indistinguishable from one that has gone wrong"*. It also bounds the base-moved problem — a green tick is `refs/pull/N/merge` against `main` **as it was when the checks ran**, and a base move raises no `synchronize` | `pr-merge-sweep.sh` |
| **A six-hour settling window** — measured from GitHub's observation of the current head object. An absent or unreadable observation holds; the gate never falls back to the commit author's clock or the pull request's comment-sensitive `updatedAt` | `merge-head-trust.sh`, `merge-candidate.sh` |
| **Undrafting is never a way to qualify** — every other condition holds first, and the sweep that undrafts merges nothing: the next one re-gathers every fact and asks again. It does **not** start a review, which the first version of this said it did — GitHub raises no workflow run from an event caused by `GITHUB_TOKEN`, and `claude-pr-review.yml` fires on `ready_for_review`. What protects the merge is the condition two rows above, which survives the undraft because undrafting changes no commit | `pr-merge-sweep.sh` |
...
The action can also publish its completed sticky comment and only afterwards
report `failure` because the turn count exceeded the configured ceiling. That
ordering is now explicit: a fresh trusted reviewer comment is publication even
when the step outcome is `failure`; the workflow warns, converges its verdict,
and does not post a contradictory no-review note. A failed step with no fresh
reviewer comment still follows the diagnostic and stale-label invalidation
path. The executable workflow test pins both branches.
