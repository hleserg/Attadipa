...
The rule, its query, its filter and its 134 assertions are on `main`. **The
...
| Who can finish it? | Anyone whose token may write workflows: apply the patch, delete `pending/`, commit. `.github/tests/merge-candidate-test.sh` asserts the repository is in one of the two consistent states and turns red on the third — a tested rule that nothing calls and nothing tracks |
