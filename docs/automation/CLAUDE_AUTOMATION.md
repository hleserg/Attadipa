...
| **One issue, one writer** — `agent:working` is only the visible mirror. Ownership is an atomic GitHub ref `refs/tags/attadipa-claims/<number>` carrying the run token; a loser re-reads the winner and exits `held` before checkout. Only that token can release it. The watchdog reaps a two-hour-old ref, and uses the label event for pre-ref legacy claims | `claim.sh`, `agent-queue-watchdog.yml`, `admission-claim-test.sh` |
