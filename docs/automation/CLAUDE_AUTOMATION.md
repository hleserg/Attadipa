...
| [`claude-ci-repair.yml`](../../.github/workflows/claude-ci-repair.yml) | CI failing on a `claude/*` branch of this repository; **and** a `/ci-repair reset` comment on a pull request | **yes**, to that branch only. The reset path invokes no model and edits two labels |
...
3. **One named bot, and it is this repository's own dispatcher.** On a public
   ...
   | `claude-ci-repair.yml` | `""` | the trigger of the job that runs a model is `workflow_run`; the actor is whoever pushed, today a person holding `ATTADIPA_AGENT_TOKEN`. It admits no bot, so it needs to name none — but it is one token change from the same failure. Its `issue_comment` trigger reaches no model at all, and refuses every bot in `.github/scripts/ci-repair-reset.sh` for its own reason: the workflow's give-up comment quotes the command |
...
| **And an exit from that stop that a comment cannot forge** — giving up leaves `ci:failed` + `agent:blocked`, and `agent:blocked` is a hold the sweep reads by name, so until #129 a diagnosed and fixed pull request was ineligible for ever. Clearing it takes `/ci-repair reset` as a **whole line** from a non-bot actor with write access; a plain `@claude` clears nothing, which is the same rule as the claim step refusing to strip that label from a pull request. The whole-line form is not fussiness: the give-up comment quotes the command while telling you to use it | `.github/scripts/ci-repair-reset.sh`, `.github/tests/ci-repair-reset-test.sh` |
| **A blocked outcome is never relabelled as a finished one** — `agent:blocked` at hand-over time is compared against the state the claim step *left*, re-read after its own edits, not against a snapshot taken before them. The agent's own `BLOCKED:` comment is never talked over when this run raised it, and an object carrying the label is never given `agent:review` and never taken out of draft when it did not | `.github/scripts/blocked-outcome.sh`, `.github/tests/blocked-restart-test.sh` |
