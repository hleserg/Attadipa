...
| **A page is not a set.** Every merge fact arrives in a GraphQL connection, so the shipping sweep calls `merge-facts.sh` and refuses any snapshot whose `pageInfo` cannot prove completeness. Head age and review freshness come from `merge-head-trust.sh`, bound to the current object id rather than an author-controlled commit timestamp | `merge-facts.graphql`, `merge-facts.sh`, `merge-head-trust.sh`, `pr-merge-sweep.yml`, issue [#170](https://github.com/hleserg/Attadipa/issues/170) |
