...
| **The queue has a bounded width before work starts** — zero or one active ordinary PR admits a writer, two is `full`, and three or more is `incident`. Active means an open same-repository PR without `queue:parked` or `queue:emergency`; forks do not spend a slot. `writer-admission.sh` runs before dispatch and again inside the globally leased writer job, so two gate runs cannot both consume the last slot. `unknown` from any API, CLI, JSON or schema failure refuses admission. At `incident`, only an existing PR or explicitly labelled `queue:emergency` recovery can run; ordinary research/follow-up/refactor work stops | `writer-admission.sh`, `writer-start.sh`, `wip-limit.sh`, `claude-agent.yml`, `agent-queue-watchdog.yml` |
